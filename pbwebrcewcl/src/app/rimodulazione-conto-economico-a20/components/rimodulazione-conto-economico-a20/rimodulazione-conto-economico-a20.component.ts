...
import { AltriCostiDTO } from 'src/app/rimodulazione-conto-economico/commons/dto/altri-costi-dto';
import { ContoEconomicoItem } from 'src/app/rimodulazione-conto-economico/commons/dto/conto-economico-item';
import { EsitoFindContoEconomicoDTO } from 'src/app/rimodulazione-conto-economico/commons/dto/esito-find-conto-economico-dto';
import { SalvaPropostaRimodulazioneRequest, SalvaRimodulazioneRequest } from 'src/app/rimodulazione-conto-economico/commons/request/salva-rimodulazione-request';
...
import { TabellaVociDiSpesa, Totali, VoceDiEntrataDaSalvareEntrate, VociDiEntrata, mapToTabellaVociDiSpesa, mapVociDiEntrata, ricalcolaTotali } from '../../commons/conto-economico-vo';

@Component({
  selector: 'app-rimodulazione-conto-economico-a20',
  templateUrl: './rimodulazione-conto-economico-a20.component.html',
  styleUrls: ['./rimodulazione-conto-economico-a20.component.scss']
})
@AutoUnsubscribe({ objectName: 'subscribers' })
export class RimodulazioneContoEconomicoA20Component implements OnInit {
  ...
  vociDiEntrateDaSalvare: VoceDiEntrataDaSalvareEntrate = {entrateAmmesseInRimodulazione: []};
  totaliEntrate: Totali;
  totaleRimodulazioneIniziale: number;
  visualizzaLabelContoEconomico: boolean = true;
  ...
  dataSourceEntrate: MatTableDataSource<VociDiEntrata> = new MatTableDataSource<VociDiEntrata>([]);
  displayedColumnsEntrate: string[] = ['vociDiEntrata','entrateInDomanda','entrateAmmesseInIstruttoria','entrateRendicontate'];
    ...
    if (this.isProposta) {
      ...
      this.subscribers.codice = this.rimodulazioneContoEconomicoService.inizializzaPropostaRimodulazione(this.idProgetto).subscribe(data => {
        if (data) {
          ...
          //IN CASO TORNASSE LA COLONNA
          //this.displayedColumnsEntrate = this.displayedColumnsEntrate.filter(d => d !== "entrateAmmesseInRimodulazione");
          ...
          if (!this.esitoFindContoEconomicoDTO.dataUltimaRimodulazione) {
            ...
            //IN CASO TORNASSE LA COLONNA
            //this.displayedColumnsEntrate = this.displayedColumnsEntrate.filter(d => d !== "ultimeEntrateAmmesse");
          }
      ...
      this.subscribers.codice = this.rimodulazioneContoEconomicoService.inizializzaRimodulazioneIstruttoria(this.idProgetto).subscribe(data => {
        if (data) {
          ...
          if (this.esitoFindContoEconomicoDTO.copiaModificataPresente) {
            ...
            //IN CASO TORNASSE LA COLONNA
            //this.displayedColumnsEntrate.splice(2, 0, "entrateAmmesseInRimodulazione");
      ...
      this.subscribers.codice = this.rimodulazioneContoEconomicoService.inizializzaRimodulazione(this.idProgetto).subscribe(data => {
        if (data) {
          ...
          this.righeContoEconomico = data.righeContoEconomico;
          this.dataFineIstruttoria = this.righeContoEconomico[0].dataFineIstruttoria;
          this.dataPresentazioneDomanda = this.righeContoEconomico[0].dataPresentazioneDomanda;
          this.dataUltimaProposta = this.righeContoEconomico[0].dataUltimaProposta;
          this.getEntrate();
          this.righeContoEconomico.forEach(r => { r.visible = true });
          for (let riga of this.righeContoEconomico.filter(r => r.level > 1 && !r.haFigli)) {
            let padre = this.righeContoEconomico.find(r => r.id === riga.idPadre);
  ...
  getEntrate(){
    this.loadedInizializza = false;
    this.rimodulazioneContoEconomicoService.getEntrate(this.idProgetto).subscribe(data => {
      if (data) {
        this.totaliEntrate = ricalcolaTotali(mapVociDiEntrata(data.vociDiEntrataCultura)).totali;
        this.totaleRimodulazioneIniziale = this.totaliEntrate.totaleEntrateAmmesseInRimodulazione;
        let rigaContoEconomico: VociDiEntrata = {
          descrizione: "Conto Economico",
          totali: this.totaliEntrate
        };
        let rigaDate: VociDiEntrata = {
          dataPresentazioneDomanda: this.dataPresentazioneDomanda,
          dataFineIstruttoria: this.dataFineIstruttoria
        };
        let rigaRipristina: VociDiEntrata = {
          rigaRipristina: true,
          differenzaRimodulazione: 0,
        };
        this.vociDiEntrataCultura = ricalcolaTotali(mapVociDiEntrata(data.vociDiEntrataCultura)).vociDiEntrataCultura;
        this.vociDiEntrataCultura.unshift(rigaContoEconomico);
        this.vociDiEntrataCultura.unshift(rigaRipristina);
        this.vociDiEntrataCultura.unshift(rigaDate);

        this.dataSourceEntrate = new MatTableDataSource<VociDiEntrata>(this.vociDiEntrataCultura);
       }
      this.loadedInizializza = true;
    });
  }
  ...
  controlloInputRigheSecondoLivelloImportoSpesaPreventivata(riga: any) {
		if (riga.importoEntrateAmmesseInRimodulazione < 0) {
  ...
  calcolaImporti() {
		const calcoli = ricalcolaTotali(this.vociDiEntrataCultura);
		this.vociDiEntrataCultura = calcoli.vociDiEntrataCultura;
    this.totaliEntrate = calcoli.totali;
	}
  ...
  aggiungiRigaDaModificare(riga: any) {
		...
		index = this.vociDiEntrateDaSalvare.entrateAmmesseInRimodulazione.findIndex(x =>x.idVoceDiEntrata === riga.idVoceDiEntrata);

		if (index !== -1) {
				this.vociDiEntrateDaSalvare.entrateAmmesseInRimodulazione[index].importoEntrateAmmesseInRimodulazione = riga.importoEntrateAmmesseInRimodulazione;
		}else{
			this.vociDiEntrateDaSalvare.entrateAmmesseInRimodulazione.push({
				idVoceDiEntrata:riga.idVoceDiEntrata,
				idRigoContoEconomico: riga.idRigoContoEconomico,
				importoEntrateAmmesseInRimodulazione: riga.importoEntrateAmmesseInRimodulazione,
  ...
  openCloseVoceEntrata(riga) {
    this.vociDiEntrataCultura.find(r => r.idVoceDiEntrata === riga.idVoceDiEntrata  && r.completamento === riga.completamento && r.flagEdit === riga.flagEdit).expanded = !this.vociDiEntrataCultura.find(r => r.idVoceDiEntrata  === riga.idVoceDiEntrata && r.completamento  === riga.completamento && r.flagEdit === riga.flagEdit).expanded;
  }
  ...
  rimodula() {
    ...
    if (!this.isIstruttoria) {
      ...
      let index1 = this.displayedColumnsEntrate.findIndex(d => d === "entrateRendicontate");
      //IN CASO TORNASSE LA COLONNA
      //this.displayedColumnsEntrate.splice(index1, 0, "entrateAmmesseInRimodulazione");
    } else {
      this.displayedColumns.splice(2, 0, "spesaAmmessaRimod");
      //IN CASO TORNASSE LA COLONNA
      //this.displayedColumnsEntrate.splice(2, 0, "entrateAmmesseInRimodulazione");
  ...
  concludiRimodulazione() {
    if (!this.isIstruttoria) {
      this.router.navigate([`drawer/${Constants.ID_OPERAZIONE_ATTIVITA_DA_SVOLGERE_RIMODULAZIONE_CONTO_ECONOMICO_CULTURA}/concludiRimodulazioneA20/${this.idProgetto}`
        + `/${this.idBando}/${this.esitoFindContoEconomicoDTO.contoEconomico.idContoEconomico}`]);
    } else {
      this.router.navigate([`drawer/${Constants.ID_OPERAZIONE_ATTIVITA_DA_SVOLGERE_RIMODULAZIONE_CONTO_ECONOMICO_ISTRUTTORIA}/concludiRimodulazioneA20/${this.idProgetto}`
        + `/${this.idBando}/${this.esitoFindContoEconomicoDTO.contoEconomico.idContoEconomico}`]);
    }
  }
  ...
  calcolaDifferenzaRimodulazione(){
    let differenza: number = 0
    differenza = this.totaleRimodulazioneIniziale - this.totaliEntrate.totaleEntrateAmmesseInRimodulazione;
    return differenza
  }
  ...
  ripristinaEntrateInDomanda() {
    //DA TENERE IN CONTO NEL CASO SIAMO IN PROPOSTA RIMODULAZIONE
    // for (let riga of this.vociDiEntrataCultura) {
    //   if (this.isProposta) {
    //     riga.importoRichiestoNuovaProposta = riga.importoRichiestoInDomanda;
    //   } else {
    //     riga.importoSpesaAmmessaRimodulazione = riga.importoRichiestoInDomanda;
    ...
    // }
    for (let riga of this.vociDiEntrataCultura) {
      if(riga.descrizione != "Conto Economico" && riga.importoRichiesto >= 0){
        riga.importoEntrateAmmesseInRimodulazione = riga.importoRichiesto;
      }else if(riga.descrizione != "Conto Economico"){
        riga.importoEntrateAmmesseInRimodulazione = 0;
      }
    }

    //MODIFICATO VENGONO UTILIZZATI NEGLI NGCLASS
    this.isRipristinaSpesaAmmessaIstruttoria = false;
    this.isRipristinaUltimaProposta = false;
    this.isRipristinaSpesaAmmessaUltima = false;
    this.isRipristinaRichiestoInDomanda = true;
    ...
    let totale:number = 0;
    for(let riga of this.vociDiEntrataCultura){
      if(riga.importoRichiesto > 0){
        totale += riga.importoRichiesto;
      }
    }
    this.totaliEntrate.totaleEntrateAmmesseInRimodulazione = totale;
    this.vociDiEntrataCultura[1].differenzaRimodulazione = this.calcolaDifferenzaRimodulazione();
  }
  ...
  ripristinaEntrateAmmessaIstruttoria() {
    //DA TENERE IN CONTO NEL CASO SIAMO IN PROPOSTA RIMODULAZIONE
    // for (let riga of this.righeContoEconomico) {
    //   if (this.isProposta) {
    //     riga.importoRichiestoNuovaProposta = riga.importoSpesaAmmessaInDetermina;
    ...
    // }
    for (let riga of this.vociDiEntrataCultura) {
      if(riga.descrizione != "Conto Economico" && riga.importoAmmesso >= 0){
        riga.importoEntrateAmmesseInRimodulazione = riga.importoAmmesso;
      }else if(riga.descrizione != "Conto Economico"){
        riga.importoEntrateAmmesseInRimodulazione = 0;
      }
    }
    ...
    let totale:number = 0;
    for(let riga of this.vociDiEntrataCultura){
      if(riga.importoAmmesso > 0){
        totale += riga.importoAmmesso;
      }
    }
    this.totaliEntrate.totaleEntrateAmmesseInRimodulazione = totale;
    this.vociDiEntrataCultura[1].differenzaRimodulazione = this.calcolaDifferenzaRimodulazione();
