...
import { EntrateResponse, TabellaVociDiSpesa, Totali, mapToTabellaVociDiSpesa, mapVociDiEntrata, ricalcolaTotali, ricalcolaTotaliSpese } from '../../commons/conto-economico-vo';
  ...
  aggiornaDati() {
	...
	resolve(): Observable<any> {
		...
		this.getSpesa(this.idProgetto).subscribe(data => {
			if (data) {
				this.spesaVo = mapToTabellaVociDiSpesa(data).tabellaVociDiSpesa;
	...
	getSpesa(idProgetto: any) {
		return this.rimodulazioneContoEconomicoService.getSpesa(this.idProgetto);
	}

	getEntrate(idProgetto: any) {
		return this.rimodulazioneContoEconomicoService.getEntrate(this.idProgetto);
	}

	// getPercentualeImportoAgevolato(idBandoLinea: number) {
	// 	return this.rimodulazioneContoEconomicoService.getPercentualeImportoAgevolato(this.entrateVo.vociDiEntrataCultura[0].idBando);
	// }
