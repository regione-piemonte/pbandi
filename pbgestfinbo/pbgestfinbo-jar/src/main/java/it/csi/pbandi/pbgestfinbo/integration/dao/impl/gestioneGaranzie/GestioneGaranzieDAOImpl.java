...
package it.csi.pbandi.pbgestfinbo.integration.dao.impl.gestioneGaranzie;
...
import it.csi.pbandi.pbgestfinbo.integration.dao.gestioneGaranzie.GestioneGaranzieDAO;
...
import it.csi.pbandi.pbgestfinbo.integration.vo.gestioneGaranzie.DatiGaranziaVO;
import it.csi.pbandi.pbgestfinbo.integration.vo.gestioneGaranzie.InfoDettaglioRicercaGaranzieVO;
...
@Service
public class GestioneGaranzieDAOImpl extends JdbcDaoSupport implements GestioneGaranzieDAO {
	private Logger LOG = Logger.getLogger(Constants.COMPONENT_NAME);

	@Autowired
	public GestioneGaranzieDAOImpl(DataSource dataSource) {
	...
	public GestioneGaranzieDAOImpl() {
	}
	...
	@Override
	public List<String> initRicercaGaranzie() {
		String prf = "[GestioneGaranzieDAOImpl::initRicercaGaranzie]";
		LOG.info(prf + " BEGIN");
	...
	public List<String> getSuggestions(String value, String id) throws Exception {
		String prf = "[GestioneGaranzieDAOImpl::getSuggestion]";
		LOG.info(prf + " BEGIN");
	...
	@Override
	public List<DatiGaranziaVO> ricercaGaranzie(String descrizioneBando, String codiceProgetto, String codiceFiscale, String ndg, String partitaIva, String denominazioneCognomeNome, String statoEscussione, String denominazioneBanca, String codiceFinpis,  Long idUtente) {
		String prf = "[GestioneGaranzieDAOImpl::ricercaGaranzie]";
		LOG.info(prf + " BEGIN");
		List<DatiGaranziaVO> listaGaranzie = null;
		...
		try {
			...
			query.append("WITH denom AS (\r\n"
					...
					+ "    LEFT JOIN PBANDI_R_SOGGETTO_PROGETTO prsp ON ptp.ID_PROGETTO = prsp.ID_PROGETTO\r\n"
			...
			query.append("AND rownum < 200\r\n"
					+ "ORDER BY pte.ID_ESCUSSIONE DESC");
			
			listaGaranzie = getJdbcTemplate().query(query.toString(), new RowMapper<DatiGaranziaVO>() {
				@Override
				public DatiGaranziaVO mapRow(ResultSet rs, int rowNum) throws SQLException {
					DatiGaranziaVO gar = new DatiGaranziaVO();
					
					gar.setIdProgetto(rs.getLong("ID_PROGETTO"));
					gar.setIdSoggetto(rs.getLong("ID_SOGGETTO"));
					gar.setProgrSoggettoProgetto(rs.getLong("PROGR_SOGGETTO_PROGETTO"));
					gar.setIdBando(rs.getLong("ID_BANDO"));
					gar.setIdEscussione(rs.getLong("ID_ESCUSSIONE"));
					
					gar.setTitoloBando(rs.getString("TITOLO_BANDO"));
					gar.setCodiceProgetto(rs.getString("CODICE_VISUALIZZATO"));
					gar.setNdg(rs.getString("NDG"));
					gar.setDenomBeneficiario(rs.getString("DENOMINAZIONE"));
					gar.setDenomBanca(rs.getString("DESC_BANCA"));
					gar.setDataRichiestaEscussione(rs.getDate("DT_RIC_RICH_ESCUSSIONE"));
					gar.setIdTipoEscussione(rs.getInt("ID_TIPO_ESCUSSIONE"));
					gar.setDescTipoEscussione(rs.getString("DESC_TIPO_ESCUSSIONE"));
					gar.setIdStatoEscussione(rs.getInt("ID_STATO_ESCUSSIONE"));
					gar.setDescStatoEscussione(rs.getString("DESC_STATO_ESCUSSIONE"));
					gar.setDataStato(rs.getDate("DT_INIZIO_VALIDITA"));
					gar.setImportoRichiesto(rs.getBigDecimal("IMP_RICHIESTO"));
					gar.setImportoApprovato(rs.getBigDecimal("IMP_APPROVATO"));
					gar.setIdModalitaAgevolazione(rs.getInt("ID_MODALITA_AGEVOLAZIONE"));
					gar.setIdModalitaAgevolazioneRif(rs.getInt("ID_MODALITA_AGEVOLAZIONE_RIF"));
					...
					// Recupero i dettagli della garanzia (tabella interna)
					gar.setListaDettagli(getDettaglioGaranzia(gar.getIdProgetto(), gar.getIdEscussione()));
	...
	List<DatiDettaglioGaranziaVO> getDettaglioGaranzia(Long idProgetto, Long idEscussione) {
		String prf = "[GestioneGaranzieDAOImpl::ricercaGaranzie::getDettaglioGaranzia]";
		LOG.info(prf + " BEGIN");
		List<DatiDettaglioGaranziaVO> listaDettaglio = null;

		try {
			StringBuilder queryDettaglio = new StringBuilder();
			queryDettaglio.append("SELECT DISTINCT\r\n"
					+ "	ptp.ID_PROGETTO,\r\n"
					+ "	prcema.ID_MODALITA_AGEVOLAZIONE,\r\n"
					+ "	pdma.ID_MODALITA_AGEVOLAZIONE_RIF,\r\n"
					+ "	pdma.DESC_MODALITA_AGEVOLAZIONE,\r\n"
					+ "	pdma.DESC_BREVE_MODALITA_AGEVOLAZ,\r\n"
					+ "	prcema.IMPORTO_AMMESSO_FINPIS,\r\n"
					+ "	(SELECT SUM(prpsf.IMP_QUOTA_SOGG_FINANZIATORE)\r\n"
					+ "	FROM PBANDI_R_PROG_SOGG_FINANZIAT prpsf\r\n"
					+ "	LEFT JOIN PBANDI_D_SOGGETTO_FINANZIATORE pdsf ON prpsf.ID_SOGGETTO_FINANZIATORE = pdsf.ID_SOGGETTO_FINANZIATORE\r\n"
					+ "	WHERE prpsf.ID_PROGETTO = ptp.id_progetto AND pdsf.FLAG_AGEVOLATO = 'S') AS totale_fondo,\r\n"
					+ "	ptce.IMPORTO_FINANZIAMENTO_BANCA AS totale_banca,\r\n"
					+ "	ptp.DT_CONCESSIONE,\r\n"
					+ "	(SELECT MAX(DT_CONTABILE)\r\n"
					+ "	FROM PBANDI_T_EROGAZIONE\r\n"
					+ "	WHERE ID_PROGETTO = ptp.id_progetto\r\n"
					+ "	AND ID_MODALITA_AGEVOLAZIONE = pdma.ID_MODALITA_AGEVOLAZIONE\r\n"
					+ "	GROUP BY ID_MODALITA_AGEVOLAZIONE) AS data_erogazione,\r\n"
					+ "	pte.ID_ESCUSSIONE,\r\n"
					+ "	pte.ID_TIPO_ESCUSSIONE,\r\n"
					+ "	pte.ID_STATO_ESCUSSIONE,\r\n"
					+ "	pte.IMP_APPROVATO AS importo_escusso,\r\n"
					+ "	pdscf.DESC_STATO_CREDITO_FP,\r\n"
					+ "	CASE WHEN ptp.ID_PROGETTO IN (\r\n"
					+ "		SELECT DISTINCT ptrb.ID_PROGETTO\r\n"
					+ "        FROM PBANDI_T_REVOCA_BANCARIA ptrb\r\n"
					+ "        WHERE ptrb.DT_FINE_VALIDITA IS NULL) THEN 'Si' ELSE 'No'\r\n"
					+ "    END AS revoca_bancaria,\r\n"
					+ "    CASE WHEN ptp.ID_PROGETTO IN (\r\n"
					+ "		SELECT DISTINCT ptarb.ID_PROGETTO\r\n"
					+ "        FROM PBANDI_T_AZIONE_RECUP_BANCA ptarb\r\n"
					+ "        WHERE ptarb.DT_FINE_VALIDITA IS NULL) THEN 'Si' ELSE 'No'\r\n"
					+ "    END AS azioni_recupero_banca,\r\n"
					+ "CASE WHEN ptp.ID_PROGETTO IN (\r\n"
					+ "		SELECT DISTINCT ptss.ID_PROGETTO\r\n"
					+ "        FROM PBANDI_T_SALDO_STRALCIO ptss\r\n"
					+ "        WHERE ptss.DT_FINE_VALIDITA IS NULL) THEN 'Si' ELSE 'No'\r\n"
					+ "    END AS saldo_stralcio\r\n"
					+ "FROM PBANDI_T_PROGETTO ptp\r\n"
					+ "LEFT JOIN PBANDI_T_CONTO_ECONOMICO ptce ON ptp.ID_DOMANDA = ptce.ID_DOMANDA\r\n"
					+ "LEFT JOIN PBANDI_D_STATO_CONTO_ECONOMICO pdsce ON ptce.ID_STATO_CONTO_ECONOMICO = pdsce.ID_STATO_CONTO_ECONOMICO\r\n"
					+ "LEFT JOIN PBANDI_R_CONTO_ECONOM_MOD_AGEV prcema ON ptce.ID_CONTO_ECONOMICO = prcema.ID_CONTO_ECONOMICO\r\n"
					...
					+ "LEFT JOIN PBANDI_D_STATO_CREDITO_FP pdscf ON prspscf.ID_STATO_CREDITO_FP = pdscf.ID_STATO_CREDITO_FP\r\n"
					+ "WHERE ptp.ID_PROGETTO = " + idProgetto + "\r\n");
			if(idEscussione != null && idEscussione != 0) {
				queryDettaglio.append(" AND pte.ID_ESCUSSIONE = " + idEscussione + "\r\n");
			}
					queryDettaglio.append("AND ptp.DT_CHIUSURA_PROGETTO IS NULL\r\n"
					+ "AND ptce.DT_FINE_VALIDITA IS NULL\r\n"
					+ "AND pte.DT_FINE_VALIDITA IS NULL\r\n"
					...
					+ "AND pdma.ID_MODALITA_AGEVOLAZIONE = 10");

			
			listaDettaglio = getJdbcTemplate().query(queryDettaglio.toString(), new RowMapper<DatiDettaglioGaranziaVO>() {

				@Override
				public DatiDettaglioGaranziaVO mapRow(ResultSet rs, int rowNum) throws SQLException {
					DatiDettaglioGaranziaVO dettGar = new DatiDettaglioGaranziaVO();
					
					dettGar.setIdProgetto(rs.getLong("ID_PROGETTO"));
					dettGar.setIdModalitaAgevolazione(rs.getInt("ID_MODALITA_AGEVOLAZIONE"));
					dettGar.setIdModalitaAgevolazioneRif(rs.getInt("ID_MODALITA_AGEVOLAZIONE_RIF"));
					dettGar.setDescModalitaAgevolazione(rs.getString("DESC_MODALITA_AGEVOLAZIONE"));
					dettGar.setDescBreveModalitaAgevolazione(rs.getString("DESC_BREVE_MODALITA_AGEVOLAZ"));
					dettGar.setImportoAmmesso(rs.getBigDecimal("IMPORTO_AMMESSO_FINPIS"));
					dettGar.setTotaleFondo(rs.getBigDecimal("TOTALE_FONDO"));
					dettGar.setTotaleBanca(rs.getBigDecimal("TOTALE_BANCA"));
					dettGar.setDataConcessione(rs.getDate("DT_CONCESSIONE"));
					dettGar.setDataErogazione(rs.getDate("DATA_EROGAZIONE"));
					dettGar.setIdEscussione(rs.getLong("ID_ESCUSSIONE"));
					dettGar.setIdTipoEscussione(rs.getLong("ID_TIPO_ESCUSSIONE"));
					dettGar.setIdStatoEscussione(rs.getLong("ID_STATO_ESCUSSIONE"));
					dettGar.setImportoEscusso(rs.getBigDecimal("IMPORTO_ESCUSSO"));
					dettGar.setStatoCredito(rs.getString("DESC_STATO_CREDITO_FP"));
					dettGar.setRevocaBancaria(rs.getString("REVOCA_BANCARIA"));
					dettGar.setAzioniRecuperoBanca(rs.getString("AZIONI_RECUPERO_BANCA"));
					dettGar.setSaldoStralcio(rs.getString("SALDO_STRALCIO"));
			...
			LOG.error(prf + "Exception in getDettaglioGaranzia", e);
		} finally {
			LOG.info(prf + " END");
		}
	...
	public InfoDettaglioRicercaGaranzieVO getDettaglioRevocaBancaria(Long idProgetto, Long idUtente) throws Exception{
		String prf = "[GestioneGaranzieDAOImpl::getDettaglioRevocaBancaria]";
		LOG.info(prf + " BEGIN");
		
		List<InfoDettaglioRicercaGaranzieVO> listaRB = new ArrayList<InfoDettaglioRicercaGaranzieVO>();
		
		try {
			String sql = "SELECT DISTINCT \r\n"
					+ "ptrb.ID_REVOCA_BANCARIA,\r\n"
					+ "ptrb.DT_RICEZIONE_COMUNICAZ,\r\n"
					+ "ptrb.DT_REVOCA,\r\n"
					+ "ptrb.IMP_DEBITO_RESIDUO_BANCA,\r\n"
					+ "ptrb.IMP_DEBITO_RESIDUO_FP,\r\n"
					+ "ptrb.NUM_PROTOCOLLO,\r\n"
					+ "ptrb.NOTE,\r\n"
					+ "ptrb.DT_INIZIO_VALIDITA,\r\n"
					+ "ptrb.DT_AGGIORNAMENTO,\r\n"
					+ "ptpf1.NOME || ' ' || ptpf1.COGNOME AS utente_ins,\r\n"
					+ "ptpf2.NOME || ' ' || ptpf2.COGNOME AS utente_agg\r\n"
					+ "FROM PBANDI_T_REVOCA_BANCARIA ptrb\r\n"
					+ "LEFT JOIN PBANDI_T_UTENTE ptu1 ON ptu1.ID_UTENTE = ptrb.ID_UTENTE_INS\r\n"
					+ "LEFT JOIN PBANDI_T_UTENTE ptu2 ON ptu2.ID_UTENTE = ptrb.ID_UTENTE_AGG\r\n"
					+ "LEFT JOIN PBANDI_T_PERSONA_FISICA ptpf1 ON ptu1.ID_SOGGETTO = PTPF1.ID_SOGGETTO\r\n"
					+ "LEFT JOIN PBANDI_T_PERSONA_FISICA ptpf2 ON ptu2.ID_SOGGETTO = PTPF2.ID_SOGGETTO\r\n"
					+ "WHERE ID_PROGETTO = " + idProgetto + "\r\n"
					+ "AND ID_MODALITA_AGEVOLAZIONE = 10\r\n"
					+ "ORDER BY ID_REVOCA_BANCARIA DESC ";
			listaRB = getJdbcTemplate().query(sql, new RowMapper<InfoDettaglioRicercaGaranzieVO>() {
				@Override
				public InfoDettaglioRicercaGaranzieVO mapRow(ResultSet rs, int rowNum) throws SQLException {
					InfoDettaglioRicercaGaranzieVO rb = new InfoDettaglioRicercaGaranzieVO(); 
					rb.setRevoca_idRevocaBancaria(rs.getLong("ID_REVOCA_BANCARIA"));
					rb.setRevoca_dataRicezioneComunicazione(rs.getDate("DT_RICEZIONE_COMUNICAZ"));
					rb.setRevoca_dataRevoca(rs.getDate("DT_REVOCA"));
					rb.setRevoca_importoDebitoResiduoBanca(rs.getBigDecimal("IMP_DEBITO_RESIDUO_BANCA"));
					rb.setRevoca_importoDebitoResiduoFinpiemonte(rs.getBigDecimal("IMP_DEBITO_RESIDUO_FP"));
					rb.setRevoca_numeroProtocollo(rs.getString("NUM_PROTOCOLLO"));
					rb.setRevoca_note(rs.getString("NOTE"));
					rb.setCom_dataInizioValidita(rs.getDate("DT_INIZIO_VALIDITA"));
					rb.setCom_utenteIns(rs.getString("UTENTE_INS"));
					return rb;
				}
			});
		...
		return listaRB.isEmpty() ? null : listaRB.get(0);	
	}
	
	public List<InfoDettaglioRicercaGaranzieVO> getDettaglioAzioneRecuperoBanca(Long idProgetto, Long idUtente) throws Exception{
		String prf = "[GestioneGaranzieDAOImpl::getDettaglioAzioneRecuperoBanca]";
		LOG.info(prf + " BEGIN");
		
		List<InfoDettaglioRicercaGaranzieVO> listaARB = new ArrayList<InfoDettaglioRicercaGaranzieVO>();
		
		try {
			String sql = "SELECT DISTINCT\r\n"
					+ "ptarb.ID_AZIONE_RECUPERO_BANCA,\r\n"
					+ "ptarb.ID_ATTIVITA_AZIONE,\r\n"
					+ "pdamc.DESC_ATT_MONIT_CRED AS desc_attivita_azione,\r\n"
					+ "ptarb.DT_AZIONE,\r\n"
					+ "ptarb.NUM_PROTOCOLLO,\r\n"
					+ "ptarb.NOTE,\r\n"
					+ "ptarb.DT_INIZIO_VALIDITA,\r\n"
					+ "ptarb.DT_INSERIMENTO,\r\n"
					+ "ptarb.DT_AGGIORNAMENTO,\r\n"
					+ "ptpf1.NOME || ' ' || ptpf1.COGNOME AS utente_ins,\r\n"
					+ "ptpf2.NOME || ' ' || ptpf2.COGNOME AS utente_agg\r\n"
					+ "FROM PBANDI_T_AZIONE_RECUP_BANCA ptarb\r\n"
					+ "LEFT JOIN PBANDI_D_ATTIVITA_MONIT_CRED pdamc ON ptarb.ID_ATTIVITA_AZIONE = pdamc.ID_ATTIVITA_MONIT_CRED\r\n"
					+ "LEFT JOIN PBANDI_T_UTENTE ptu1 ON ptu1.ID_UTENTE = ptarb.ID_UTENTE_INS\r\n"
					+ "LEFT JOIN PBANDI_T_UTENTE ptu2 ON ptu2.ID_UTENTE = ptarb.ID_UTENTE_AGG\r\n"
					+ "LEFT JOIN PBANDI_T_PERSONA_FISICA ptpf1 ON ptu1.ID_SOGGETTO = PTPF1.ID_SOGGETTO\r\n"
					+ "LEFT JOIN PBANDI_T_PERSONA_FISICA ptpf2 ON ptu2.ID_SOGGETTO = PTPF2.ID_SOGGETTO\r\n"
					+ "WHERE ptarb.ID_PROGETTO = " + idProgetto + "\r\n"
					+ "AND ptarb.ID_MODALITA_AGEVOLAZIONE = 10\r\n"
					+ "AND ptarb.DT_FINE_VALIDITA IS NULL\r\n"
					+ "ORDER BY ptarb.ID_AZIONE_RECUPERO_BANCA DESC";
			listaARB = getJdbcTemplate().query(sql, new RowMapper<InfoDettaglioRicercaGaranzieVO>() {
				@Override
				public InfoDettaglioRicercaGaranzieVO mapRow(ResultSet rs, int rowNum) throws SQLException {
					InfoDettaglioRicercaGaranzieVO arb = new InfoDettaglioRicercaGaranzieVO(); 
					arb.setAzione_idAzioneRecuperoBanca(rs.getLong("ID_AZIONE_RECUPERO_BANCA"));
					arb.setAzione_idAttivitaAzione(rs.getLong("ID_ATTIVITA_AZIONE"));
					arb.setAzione_descAttivitaAzione(rs.getString("DESC_ATTIVITA_AZIONE"));
					arb.setAzione_dataAzione(rs.getDate("DT_AZIONE"));
					arb.setAzione_numeroProtocollo(rs.getString("NUM_PROTOCOLLO"));
					arb.setAzione_note(rs.getString("NOTE"));
					arb.setCom_dataInizioValidita(rs.getDate("DT_INIZIO_VALIDITA"));
					arb.setCom_dataInserimento(rs.getDate("DT_INSERIMENTO"));
					//arb.setCom_dataAggiornamento(rs.getDate("DT_AGGIORNAMENTO"));
					arb.setCom_utenteIns(rs.getString("UTENTE_INS"));
					//arb.setCom_utenteAgg(rs.getString("UTENTE_AGG"));
					return arb;
				}
			});
	...
	public List<InfoDettaglioRicercaGaranzieVO> getDettaglioSaldoStralcio(Long idProgetto, Long idUtente) throws Exception{
		String prf = "[GestioneGaranzieDAOImpl::getDettaglioSaldoStralcio]";
		LOG.info(prf + " BEGIN");
		...
		try {
			String sql = "SELECT DISTINCT\r\n"
					...
					+ "ptss.ID_ATTIVITA_SALDO_STRALCIO,\r\n"
					+ "pdamc1.DESC_BREVE_ATT_MONIT_CRED AS desc_breve_attivita,\r\n"
					+ "pdamc1.DESC_ATT_MONIT_CRED AS desc_attivita,\r\n"
					+ "ptss.DT_PROPOSTA,\r\n"
					+ "ptss.DT_ACCETTAZIONE,\r\n"
					+ "ptss.IMP_DEBITORE,\r\n"
					+ "ptss.IMP_CONFIDI,\r\n"
					+ "ptss.ID_ATTIVITA_ESITO,\r\n"
					+ "pdamc2.DESC_BREVE_ATT_MONIT_CRED AS desc_breve_esito,\r\n"
					+ "pdamc2.DESC_ATT_MONIT_CRED AS desc_esito,\r\n"
					+ "ptss.DT_ESITO,\r\n"
					+ "ptss.DT_PAGAM_DEBITORE,\r\n"
					+ "ptss.DT_PAGAM_CONFIDI,\r\n"
					+ "ptss.ID_ATTIVITA_RECUPERO,\r\n"
					+ "pdamc3.DESC_BREVE_ATT_MONIT_CRED AS desc_breve_recupero,\r\n"
					+ "pdamc3.DESC_ATT_MONIT_CRED AS desc_recupero,\r\n"
					+ "ptss.IMP_RECUPERATO,\r\n"
					+ "ptss.IMP_PERDITA,\r\n"
					+ "ptss.NOTE,\r\n"
					+ "ptss.FLAG_AGEVOLAZIONE,\r\n"
					+ "ptss.IMP_DISIMPEGNO,\r\n"
					+ "ptss.DT_INIZIO_VALIDITA,\r\n"
					+ "ptss.DT_INSERIMENTO,\r\n"
					+ "ptpf.NOME || ' ' || ptpf.COGNOME AS utente_ins\r\n"
					+ "FROM PBANDI_T_SALDO_STRALCIO ptss\r\n"
					+ "LEFT JOIN PBANDI_D_ATTIVITA_MONIT_CRED pdamc1 ON ptss.ID_ATTIVITA_SALDO_STRALCIO = pdamc1.ID_ATTIVITA_MONIT_CRED\r\n"
					+ "LEFT JOIN PBANDI_D_ATTIVITA_MONIT_CRED pdamc2 ON ptss.ID_ATTIVITA_ESITO = pdamc2.ID_ATTIVITA_MONIT_CRED\r\n"
					+ "LEFT JOIN PBANDI_D_ATTIVITA_MONIT_CRED pdamc3 ON ptss.ID_ATTIVITA_RECUPERO = pdamc3.ID_ATTIVITA_MONIT_CRED\r\n"
					...
					+ "WHERE ptss.ID_PROGETTO = " + idProgetto + "\r\n"
					+ "AND ptss.ID_MODALITA_AGEVOLAZIONE = 10 AND ptss.DT_FINE_VALIDITA IS NULL\r\n"
					+ "ORDER BY ptss.ID_SALDO_STRALCIO DESC"; 
			listaRB = getJdbcTemplate().query(sql, new RowMapper<InfoDettaglioRicercaGaranzieVO>() {

				@Override
				public InfoDettaglioRicercaGaranzieVO mapRow(ResultSet rs, int rowNum) throws SQLException {
					InfoDettaglioRicercaGaranzieVO ss = new InfoDettaglioRicercaGaranzieVO(); 
					ss.setSaldo_idSaldoStralcio(rs.getLong("ID_SALDO_STRALCIO"));
					ss.setSaldo_idAttivita(rs.getLong("ID_ATTIVITA_SALDO_STRALCIO"));
					ss.setSaldo_descBreveAttivita(rs.getString("DESC_BREVE_ATTIVITA"));
					ss.setSaldo_descAttivita(rs.getString("DESC_ATTIVITA"));
					ss.setSaldo_dataProposta(rs.getDate("DT_PROPOSTA"));
					ss.setSaldo_dataAccettazione(rs.getDate("DT_ACCETTAZIONE"));
					ss.setSaldo_importoDebitore(rs.getBigDecimal("IMP_DEBITORE"));
					ss.setSaldo_importoConfidi(rs.getBigDecimal("IMP_CONFIDI"));
					ss.setSaldo_idAttivitaEsito(rs.getLong("ID_ATTIVITA_ESITO"));
					ss.setSaldo_descBreveEsito(rs.getString("DESC_BREVE_ESITO"));
					ss.setSaldo_descEsito(rs.getString("DESC_ESITO"));
					ss.setSaldo_dataEsito(rs.getDate("DT_ESITO"));
					ss.setSaldo_dataPagamentoDebitore(rs.getDate("DT_PAGAM_DEBITORE"));
					ss.setSaldo_dataPagamentoConfidi(rs.getDate("DT_PAGAM_CONFIDI"));
					ss.setSaldo_idAttivitaRecupero(rs.getLong("ID_ATTIVITA_RECUPERO"));
					ss.setSaldo_descBreveRecupero(rs.getString("DESC_BREVE_RECUPERO"));
					ss.setSaldo_descRecupero(rs.getString("DESC_RECUPERO"));
					ss.setSaldo_importoRecuperato(rs.getBigDecimal("IMP_RECUPERATO"));
					ss.setSaldo_importoPerdita(rs.getBigDecimal("IMP_PERDITA"));
					ss.setSaldo_note(rs.getString("NOTE"));
					String flagTemp = rs.getString("FLAG_AGEVOLAZIONE");
					if("S".equals(flagTemp)) {
						ss.setSaldo_flagAgevolazione(true);
					} else {
						ss.setSaldo_flagAgevolazione(false);
					}
					ss.setSaldo_importoDisimpegno(rs.getBigDecimal("IMP_DISIMPEGNO"));
					ss.setCom_dataInizioValidita(rs.getDate("DT_INIZIO_VALIDITA"));
					ss.setCom_dataInserimento(rs.getDate("DT_INSERIMENTO"));
					ss.setCom_utenteIns(rs.getString("UTENTE_INS"));
					return ss;
				}
			});
	...
	@Override
	public List<GestioneAllegatiVO> getAllegatiEscussione(Long idEscussione) throws Exception {
		String prf = "[GestioneGaranzieDAOImpl::getAllegatiEscussione]";
		LOG.info(prf + " BEGIN");
		List<GestioneAllegatiVO> allegati = new ArrayList<GestioneAllegatiVO>();
		try {
			/*String query = "select nome_file, ID_DOCUMENTO_INDEX from PBANDI_T_DOCUMENTO_INDEX   \r\n"
			...
			allegati = getJdbcTemplate().query(query, args, types, new ResultSetExtractor<List<GestioneAllegatiVO>>() {
				@Override
	...
	private List<DettaglioGaranziaVO> OLDgetDettagliGaranza(String idProgetto, BigDecimal idEscussione) {
		...
		try {
			...
			det.setImportoEscusso(getImportoEscusso(idProgetto, idEscussione));
	...
	public GaranziaVO getGaranzia(BigDecimal idEscussione, BigDecimal idProgetto) {
		...
		try {
			...
			garanzia.setListaDettagli(OLDgetDettagliGaranza(garanzia.getIdProgetto().toString(), idEscussione));
