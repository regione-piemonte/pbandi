...
package it.csi.pbandi.pbgestfinbo.integration.dao.impl.boxAttivitaIstruttore;
...
@Component
public class BoxSaldoStralcioDAOImpl extends JdbcDaoSupport implements BoxSaldoStralcioDAO {
	
	private Logger LOG = Logger.getLogger(Constants.COMPONENT_NAME);
	
	@Autowired
	public BoxSaldoStralcioDAOImpl(DataSource dataSource) {
	...
	@Override
	public List<BoxSaldoStralcioVO> getSaldoStralcio(Long idProgetto, int idModalitaAgevolazione) {
		String prf = "[BoxSaldoStralcioDAOImpl::getNEWSaldoStralcio]";
		LOG.info(prf + " BEGIN");
		
		List<BoxSaldoStralcioVO> listaSaldo =  new ArrayList<BoxSaldoStralcioVO>();
		
		try {
			
			String queryGet ="SELECT DISTINCT\r\n"
					+ "	ptss.ID_SALDO_STRALCIO,\r\n"
					+ "	ptss.ID_PROGETTO,\r\n"
					+ "	ptss.ID_MODALITA_AGEVOLAZIONE,\r\n"
					+ "	ptss.ID_ATTIVITA_SALDO_STRALCIO,\r\n"
					+ "	pdamc.DESC_BREVE_ATT_MONIT_CRED AS DESC_BREVE_ATTIVITA,\r\n"
					+ "	pdamc.DESC_ATT_MONIT_CRED AS DESC_ATTIVITA,\r\n"
					+ "	ptss.DT_PROPOSTA,\r\n"
					+ "	ptss.DT_ACCETTAZIONE,\r\n"
					+ "	ptss.IMP_DEBITORE,\r\n"
					+ "	ptss.IMP_CONFIDI,\r\n"
					+ "	ptss.ID_ATTIVITA_ESITO,\r\n"
					+ "	pdamc2.DESC_BREVE_ATT_MONIT_CRED AS DESC_BREVE_ATTIVITA_ESITO,\r\n"
					+ "	pdamc2.DESC_ATT_MONIT_CRED AS DESC_ATTIVITA_ESITO,\r\n"
					+ "	ptss.DT_ESITO,\r\n"
					+ "	ptss.DT_PAGAM_DEBITORE,\r\n"
					+ "	ptss.DT_PAGAM_CONFIDI,\r\n"
					+ "	ptss.ID_ATTIVITA_RECUPERO,\r\n"
					+ "	pdamc3.DESC_BREVE_ATT_MONIT_CRED AS DESC_BREVE_ATTIVITA_RECUPERO,\r\n"
					+ "	pdamc3.DESC_ATT_MONIT_CRED AS DESC_ATTIVITA_RECUPERO,\r\n"
					+ "	ptss.IMP_RECUPERATO,\r\n"
					+ "	ptss.IMP_PERDITA,\r\n"
					+ "	ptss.NOTE,\r\n"
					...
					+ "	ptss.ID_UTENTE_INS,\r\n"
					+ "	ptpf1.COGNOME || ' ' || ptpf1.NOME AS nome_utente_ins,\r\n"
					+ "	ptss.DT_AGGIORNAMENTO,\r\n"
					+ "	ptss.ID_UTENTE_AGG,\r\n"
					+ "	ptpf2.COGNOME || ' ' || ptpf2.NOME AS nome_utente_agg\r\n"
					+ "FROM PBANDI_T_SALDO_STRALCIO ptss\r\n"
					+ "-- attivita saldo stralcio --\r\n"
					+ "LEFT JOIN PBANDI_D_ATTIVITA_MONIT_CRED pdamc ON ptss.ID_ATTIVITA_SALDO_STRALCIO = pdamc.ID_ATTIVITA_MONIT_CRED \r\n"
					+ "-- attivita esito --\r\n"
					...
					+ "-- utente inserito --\r\n"
					+ "LEFT JOIN PBANDI_T_UTENTE ptu1 ON ptss.ID_UTENTE_INS = ptu1.ID_UTENTE\r\n"
					+ "LEFT JOIN PBANDI_T_PERSONA_FISICA ptpf1 ON ptpf1.ID_SOGGETTO = ptu1.ID_SOGGETTO\r\n"
					+ "-- utente aggiornato --\r\n"
					+ "LEFT JOIN PBANDI_T_UTENTE ptu2 ON ptss.ID_UTENTE_AGG = ptu2.ID_UTENTE\r\n"
					+ "LEFT JOIN PBANDI_T_PERSONA_FISICA ptpf2 ON ptpf2.ID_SOGGETTO = ptu2.ID_SOGGETTO\r\n"
					+ "WHERE PTSS.DT_FINE_VALIDITA IS NULL\r\n"
					+ "    AND ptss.ID_PROGETTO = ?\r\n"
					+ "    AND ptss.ID_MODALITA_AGEVOLAZIONE = ?\r\n"
					+ "ORDER BY DT_ESITO DESC, ptss.ID_SALDO_STRALCIO DESC";			
			listaSaldo = getJdbcTemplate().query(queryGet, new RowMapper<BoxSaldoStralcioVO>() {
				
				@Override
				public BoxSaldoStralcioVO mapRow(ResultSet rs, int rowNum) throws SQLException {
					BoxSaldoStralcioVO ss = new BoxSaldoStralcioVO();

					ss.setIdSaldoStralcio(rs.getLong("ID_SALDO_STRALCIO"));
					ss.setIdProgetto(rs.getLong("ID_PROGETTO"));
					ss.setIdModalitaAgevolazione(rs.getInt("ID_MODALITA_AGEVOLAZIONE"));
					
					ss.setIdAttivitaSaldoStralcio(rs.getInt("ID_ATTIVITA_SALDO_STRALCIO"));
					ss.setDescBreveAttivitaSaldoStralcio(rs.getString("DESC_BREVE_ATTIVITA"));
					ss.setDescAttivitaSaldoStralcio(rs.getString("DESC_ATTIVITA"));
					ss.setDataProposta(rs.getDate("DT_PROPOSTA"));
					ss.setDataAccettazione(rs.getDate("DT_ACCETTAZIONE"));
					ss.setImportoDebitore(rs.getBigDecimal("IMP_DEBITORE"));
					ss.setImportoConfidi(rs.getBigDecimal("IMP_CONFIDI"));
					ss.setIdAttivitaEsito(rs.getInt("ID_ATTIVITA_ESITO"));
					ss.setDescBreveAttivitaEsito(rs.getString("DESC_BREVE_ATTIVITA_ESITO"));
					ss.setDescAttivitaEsito(rs.getString("DESC_ATTIVITA_ESITO"));
					ss.setDataEsito(rs.getDate("DT_ESITO"));
					ss.setDataPagamentoDebitore(rs.getDate("DT_PAGAM_DEBITORE"));
					ss.setDataPagamentoConfidi(rs.getDate("DT_PAGAM_CONFIDI"));
					ss.setIdAttivitaRecupero(rs.getInt("ID_ATTIVITA_RECUPERO"));
					ss.setDescBreveAttivitaRecupero(rs.getString("DESC_BREVE_ATTIVITA_RECUPERO"));
					ss.setDescAttivitaRecupero(rs.getString("DESC_ATTIVITA_RECUPERO"));
					ss.setImportoRecuperato(rs.getBigDecimal("IMP_RECUPERATO"));
					ss.setImportoPerdita(rs.getBigDecimal("IMP_PERDITA"));
					ss.setNote(rs.getString("NOTE"));
					//String temp = rs.getString("FLAG_AGEVOLAZIONE");
					ss.setFlagAgevolazione("S".equals(rs.getString("FLAG_AGEVOLAZIONE")) ? true : false);
					ss.setImportoDisimpegno(rs.getBigDecimal("IMP_DISIMPEGNO"));
					
					ss.setDataInizioValidita(rs.getDate("DT_INIZIO_VALIDITA"));
					ss.setIdUtenteIns(rs.getLong("ID_UTENTE_INS"));
					ss.setNomeUtenteIns(rs.getString("NOME_UTENTE_INS"));
					ss.setDataAggiornamento(rs.getDate("DT_AGGIORNAMENTO"));
					ss.setIdUtenteAgg(rs.getLong("ID_UTENTE_AGG"));
					ss.setNomeUtenteAgg(rs.getString("NOME_UTENTE_AGG"));
					
					return ss;
				}}, new Object[] { idProgetto, idModalitaAgevolazione }
			);
	...
	@Override
	public InitBoxSaldoStralcioVO initSaldoStralcio() {
		String prf = "[BoxSaldoStralcioDAOImpl::initSaldoStralcio]";
		LOG.info(prf + " BEGIN");

		InitBoxSaldoStralcioVO liste = new InitBoxSaldoStralcioVO(); 

		// Ottengo la lista dei tipi saldo e stralcio
		String queryTipi = "SELECT DISTINCT DESC_ATT_MONIT_CRED\r\n"
				+ "FROM PBANDI_D_ATTIVITA_MONIT_CRED pdamc\r\n"
				+ "WHERE pdamc.ID_TIPO_MONIT_CRED = 10 AND pdamc.DT_FINE_VALIDITA IS NULL";
		liste.setListaTipiSaldoStralcio(getJdbcTemplate().queryForList(queryTipi, String.class));

		// Ottengo la lista degli esiti
		String queryEsiti = "SELECT DISTINCT DESC_ATT_MONIT_CRED\r\n"
				+ "FROM PBANDI_D_ATTIVITA_MONIT_CRED pdamc\r\n"
				+ "WHERE pdamc.ID_TIPO_MONIT_CRED = 4 AND pdamc.DT_FINE_VALIDITA IS NULL";
		liste.setListaEsiti(getJdbcTemplate().queryForList(queryEsiti, String.class));
		
		// Ottengo la lista dei recuperi
		String queryRecuperi = "SELECT DISTINCT DESC_ATT_MONIT_CRED\r\n"
				+ "FROM PBANDI_D_ATTIVITA_MONIT_CRED pdamc\r\n"
				+ "WHERE pdamc.ID_TIPO_MONIT_CRED = 5 AND pdamc.DT_FINE_VALIDITA IS NULL";
		liste.setListaRecuperi(getJdbcTemplate().queryForList(queryRecuperi, String.class));
	...
	private boolean checkCallToAmmContabile(Long idProgetto, Long idSaldoStralcio) {
		...
		try {
			...
			String checkAmmContabile ="SELECT ptmsac.ID_MONIT_AMMVO_CONT\r\n"
					+ "FROM PBANDI_T_MON_SERV_AMMVO_CONTAB ptmsac \r\n"
