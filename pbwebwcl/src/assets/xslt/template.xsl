...
<!--  <xsl:decimal-format name="euro" decimal-separator="," grouping-separator="."/> -->
 ...
 <xsl:template name="FormattaNumeri"> <!-- '###.###.##0,00###### -->
  <xsl:param name="numero" />

  <xsl:variable name="parteIntera" select="substring-before($numero, '.')" />
  <xsl:variable name="parteDecimale" select="substring-after($numero, '.')" />
  <xsl:variable name="parteDecimaleNew"  >
      <xsl:choose>
        <xsl:when test="string-length($parteDecimale)=2">
          <xsl:value-of select="$parteDecimale" />
        </xsl:when>
        <xsl:when test="string-length($parteDecimale)=1">
        <xsl:value-of select="$parteDecimale" />0
        </xsl:when>
        <xsl:otherwise>
          00
        </xsl:otherwise>
  ...
  <xsl:variable name="parteInteraNew"><xsl:call-template name="FormatParteIntera"><xsl:with-param name="numero" select="$parteIntera" /></xsl:call-template></xsl:variable>
  <xsl:value-of select="$parteInteraNew"/>,<xsl:value-of select="$parteDecimaleNew" />

 </xsl:template>

<xsl:template name="FormatParteIntera"> <!-- '###.###.##0,00###### -->
  <xsl:param name="numero" />
    <xsl:choose>
      <xsl:when test="string-length($numero)&lt;4"><xsl:value-of select="$numero"/></xsl:when>
      <xsl:otherwise><xsl:call-template name="FormatParteIntera"><xsl:with-param name="numero" select="substring($numero, 0, string-length($numero)-2)"/></xsl:call-template>.<xsl:value-of select="substring($numero,string-length($numero)-2)"/></xsl:otherwise>
    </xsl:choose>
</xsl:template>
...
<!--          <xsl:value-of select="format-number($IVA,  '###.###.##0,00', 'euro')" /> -->
              <xsl:call-template name="FormattaNumeri">
                <xsl:with-param name="numero" select="$IVA" />
              </xsl:call-template>
        </xsl:if>
      </xsl:otherwise>
    </xsl:choose>
  ...
  <!--        <xsl:value-of select="format-number($importo,  '###.###.##0,00', 'euro')" /> -->
              <xsl:call-template name="FormattaNumeri">
                <xsl:with-param name="numero" select="$importo" />
              </xsl:call-template>
        </xsl:if>
      </xsl:otherwise>
    </xsl:choose>
  ...
  <!--                  <xsl:value-of select="format-number(RiferimentoNumero,  '###.###.##0,00######', 'euro')" /> -->
                        <xsl:call-template name="FormattaNumeri">
                          <xsl:with-param name="numero" select="RiferimentoNumero" />
                        </xsl:call-template>
                  </div>
                </xsl:if>
...
<!--                <xsl:value-of select="format-number(Quantita,  '###.###.##0,00######', 'euro')" />-->
                 <xsl:call-template name="FormattaNumeri">
                    <xsl:with-param name="numero" select="Quantita" />
                </xsl:call-template>
              </xsl:if>
            </xsl:if>
          </td>
...
<!--            <xsl:value-of select="format-number(PrezzoUnitario,  '###.###.##0,00######', 'euro')" /> -->
                <xsl:call-template name="FormattaNumeri">
                    <xsl:with-param name="numero" select="PrezzoUnitario" />
                </xsl:call-template>
              </xsl:if>
            </xsl:if>
          </td>
...
<!--             <xsl:value-of select="format-number(PrezzoTotale,  '###.###.##0,00######', 'euro')" /> -->
                <xsl:call-template name="FormattaNumeri">
                    <xsl:with-param name="numero" select="PrezzoTotale" />
                </xsl:call-template>
                </div>
              </xsl:if>
...
<!--              <xsl:value-of select="format-number(AliquotaRitenuta,  '###.###.##0,00', 'euro')" /> -->
                  <xsl:call-template name="FormattaNumeri">
                      <xsl:with-param name="numero" select="AliquotaRitenuta" />
                   </xsl:call-template>
            </xsl:if>
   ...
   <!--           <xsl:value-of select="format-number(ImportoRitenuta,  '###.###.##0,00', 'euro')" /> -->
                  <xsl:call-template name="FormattaNumeri">
                     <xsl:with-param name="numero" select="ImportoRitenuta" />
                  </xsl:call-template>
  ...
  <!--      <xsl:value-of select="format-number(ScontoPagamentoAnticipato,  '###.###.##0,00', 'euro')" /> -->
            <xsl:call-template name="FormattaNumeri">
                <xsl:with-param name="numero" select="ScontoPagamentoAnticipato" />
            </xsl:call-template>
      </xsl:if>
      <br/>
    </xsl:if>
  ...
  <!--      <xsl:value-of select="format-number(PenalitaPagamentiRitardati,  '###.###.##0,00', 'euro')" /> -->
            <xsl:call-template name="FormattaNumeri">
                <xsl:with-param name="numero" select="PenalitaPagamentiRitardati" />
            </xsl:call-template>
      </xsl:if>
  ...
  <xsl:template name="FatturaElettronica">
    ...
    <xsl:if test="$TipoFattura">
      ...
      <div id="fattura-elettronica" class="page">
	      <div class="versione">Versione <xsl:value-of select="$TipoFattura/@versione"/></div>
          ...
          <table id="tbHeader" class="tbHeader">
            <tr>
                            ...
                            <xsl:when test="DatiAnagrafici">
                              ...
                              <xsl:for-each select="$TipoFattura/FatturaElettronicaHeader/CedentePrestatore/DatiAnagrafici">
                                ...
                                <div class="headContent mt5">
                                  <xsl:if test="IdFiscaleIVA">
                                    ...
                                    <span>
                                      ...
                                      <xsl:value-of select="IdFiscaleIVA/IdCodice" />                                      </span>
                                  </xsl:if>
                                </div>
                                ...
                                <div class="headContent" >
                                  ...
                                  <xsl:if test="CodiceFiscale">
                                    ...
                                    Codice fiscale:
                                    <span>
                                      <xsl:value-of select="CodiceFiscale" />                                      </span>
                                  </xsl:if>
                                </div>
                                ...
                                <div class="headContent" >
                                  ...
                                  <xsl:if test="Anagrafica/Denominazione">
                                    ...
                                    Denominazione:
                                    <span>
                                      <xsl:value-of select="Anagrafica/Denominazione" />                                      </span>
                                  </xsl:if>
                                </div>
                                ...
                                <div class="headContent" >
                                  ...
                                  <xsl:if test="Anagrafica/Nome | Anagrafica/Cognome">
                                    ...
                                    <xsl:if test="Anagrafica/Nome">
                                      <span>
                                        <xsl:value-of select="Anagrafica/Nome" />                                        </span>
                                    </xsl:if>
                                  </xsl:if>
                                </div>
                                ...
                                <div class="headContent" >
                                  ...
                                  <xsl:if test="RegimeFiscale">
                                    Regime fiscale:
                                    <span>
                                      <xsl:value-of select="RegimeFiscale" />                                      </span>
                                    <xsl:variable name="RF">
                                      <xsl:value-of select="RegimeFiscale" />                                      </xsl:variable>
                                    <xsl:choose>
                                ...
                                <div class="headContent" >
                                  ...
                                  <xsl:if test="Indirizzo">
                                    ...
                                    <span>
                                      ...
                                      <xsl:text> </xsl:text>
                                      <xsl:value-of select="NumeroCivico" />                                    </span>
                                  </xsl:if>
                                </div>
                                ...
                                <div class="headContent" >
                                  <span>
                                    <xsl:if test="Comune">
                                      ...
                                      Comune:
                                      <span>
                                        <xsl:value-of select="Comune" />                                      </span>
                                    </xsl:if>
                                    <xsl:if test="Provincia">

                                      Provincia:
                                      <span>
                                        <xsl:value-of select="Provincia" />                                      </span>
                                    </xsl:if>
                                  </span>                                </div>
                                <div class="headContent" >
                                  ...
                                  <span>
                                    ...
                                    <xsl:if test="Nazione">
                                      ...
                                      Nazione:
                                      <span>
                                        <xsl:value-of select="Nazione" />                                      </span>
                                    </xsl:if>
                                  </span>                                </div>
                              </xsl:for-each>
                              <div class="headContent" >
                                ...
                                <xsl:if test="$TipoFattura/FatturaElettronicaHeader/CedentePrestatore/Contatti/Telefono">
                                  ...
                                  Telefono:
                                  <span>
                                    <xsl:value-of select="$TipoFattura/FatturaElettronicaHeader/CedentePrestatore/Contatti/Telefono" />                                  </span>
                                </xsl:if>
                              </div>
                              ...
                              <div class="headContent" >
                                ...
                                <xsl:if test="$TipoFattura/FatturaElettronicaHeader/CedentePrestatore/Contatti/Email">
                                  ...
                                  Email:
                                  <span>
                                    <xsl:value-of select="$TipoFattura/FatturaElettronicaHeader/CedentePrestatore/Contatti/Email" />                                    </span>
                                </xsl:if>
                              </div>
                              ...
                              <div class="headContent" >
                                ...
                                <xsl:if test="$TipoFattura/FatturaElettronicaHeader/CedentePrestatore/RiferimentoAmministrazione">
                                  ...
                                  <span>
                                    <xsl:value-of select="$TipoFattura/FatturaElettronicaHeader/CedentePrestatore/RiferimentoAmministrazione" />                                  </span>
                                </xsl:if>
                              </div>
                            </xsl:when>
                            <xsl:otherwise>
                              ...
                              <xsl:if test="Denominazione">
                                <div class="headContent">
                                  ...
                                  Denominazione:
                                  <span>
                                    <xsl:value-of select="Denominazione" />                                    </span>                                 </div>
                              </xsl:if>
              ...
              <td class="tdHead">
                              ...
                              <xsl:for-each select="$TipoFattura/FatturaElettronicaHeader/CessionarioCommittente/AltriDatiIdentificativi">
                                <div class="headContent" >
                                  ...
                                  <xsl:if test="Denominazione">
                                    ...
                                    Denominazione:
                                    <span>
                                      <xsl:value-of select="Denominazione" />                                      </span>
                                  </xsl:if>
                                </div>
                ...
                <!--FINE CESSIONARIO COMMITTENTE-->                </td>
            </tr>
          </table>
        </xsl:if>
        ...
        <xsl:for-each select="$TipoFattura/FatturaElettronicaBody" >
                ...
                <td>
                  <xsl:if test="DatiGenerali/DatiGeneraliDocumento/TipoDocumento">
                    ...
                    <xsl:variable name="TD">
                      <xsl:value-of select="DatiGenerali/DatiGeneraliDocumento/TipoDocumento" />                      </xsl:variable>
                    <xsl:choose>
                    ...
                    <tr>
                      <td>
                        ...
                        <xsl:if test="RiferimentoNormativo">
                          <div class="tx-xsmall">
                            RIF.NORM. <xsl:value-of select="RiferimentoNormativo" />                            </div>
                        </xsl:if>                        </td>
                      <td class="import" >
         ...
         <!--                 <xsl:value-of select="format-number(DatiIVA/Imposta,  '###.###.##0,00', 'euro')" /> -->
                            <xsl:call-template name="FormattaNumeri">
                              <xsl:with-param name="numero" select="DatiIVA/Imposta" />
                            </xsl:call-template>
                        </xsl:if>                        </td>
                      <td class="import" >
        ...
        <!--                  <xsl:value-of select="format-number(Importo,  '###.###.##0,00', 'euro')" /> -->
            ...
            <xsl:otherwise>
              ...
              <table class="tbFoglio"  >
                ...
                <thead>
                  <tr>
                    <th width="codarticolo">Cod. articolo</th>
                    <th>Descrizione</th>
              ...
              <xsl:if test="DatiGenerali/DatiGeneraliDocumento/DatiCassaPrevidenziale">
                ...
                <table class="tbFoglio">
                      ...
                      <tr>
                        <td>
                          <xsl:if test="TipoCassa">
                            ...
                            <span>
                              <xsl:value-of select="TipoCassa" />                              </span>
                            <xsl:variable name="TC">
                              <xsl:value-of select="TipoCassa" />                              </xsl:variable>
                            <xsl:choose>
                 ...
                 <!--          <xsl:value-of select="format-number(ImponibileCassa,  '###.###.##0,00', 'euro')" /> -->
                            <xsl:call-template name="FormattaNumeri">
                                <xsl:with-param name="numero" select="ImponibileCassa" />
                            </xsl:call-template>
                          </xsl:if>                          </td>
                 ...
                 <!--           <xsl:value-of select="format-number(AlCassa,  '###.###.##0,00', 'euro')" />-->
                            <xsl:call-template name="FormattaNumeri">
                                <xsl:with-param name="numero" select="AlCassa" />
                            </xsl:call-template>
                          </xsl:if>                          </td>
                  ...
                  <!--              <xsl:value-of select="format-number(AliquotaIVA,  '###.###.##0,00', 'euro')" /> -->
                                  <xsl:call-template name="FormattaNumeri">
                                      <xsl:with-param name="numero" select="AliquotaIVA" />
                                  </xsl:call-template>
                              </xsl:if>
                            </xsl:otherwise>
                          </xsl:choose>                          </td>
                  ...
                  <!--          <xsl:value-of select="format-number(ImportoContributoCassa,  '###.###.##0,00', 'euro')" /> -->
                              <xsl:call-template name="FormattaNumeri">
                                  <xsl:with-param name="numero" select="ImportoContributoCassa" />
                              </xsl:call-template>
                          </xsl:if>                          </td>
                      </tr>
                    </xsl:for-each>
                ...
                <tbody>
                  ...
                  <xsl:for-each select="DatiBeniServizi/DatiRiepilogo" >
                        ...
                        <td colspan="3" >
                          <xsl:choose>
                            <xsl:when test="EsigibilitaIVA">
                              ...
                              <span>
                                <xsl:value-of select="EsigibilitaIVA" />                                </span>
                              <xsl:variable name="EI">
                                <xsl:value-of select="EsigibilitaIVA" />                                </xsl:variable>
                              <xsl:choose>
                                <xsl:when test="$EI='I'">
                          ...
                          <xsl:if test="RiferimentoNormativo">
                            <div class="tx-xsmall">
                              <xsl:value-of select="RiferimentoNormativo" />                              </div>
                          </xsl:if>                          </td>
                   ...
                   <!--         <xsl:value-of select="format-number(SpeseAccessorie,  '###.###.##0,00', 'euro')" /> -->
                              <xsl:call-template name="FormattaNumeri">
                                  <xsl:with-param name="numero" select="SpeseAccessorie" />
                              </xsl:call-template>
                          </xsl:if>                          </td>
         ...
         <!--                 <xsl:value-of select="format-number(Arrotondamento,  '###.###.##0,00', 'euro')" /> -->
                              <xsl:call-template name="FormattaNumeri">
                                  <xsl:with-param name="numero" select="Arrotondamento" />
                              </xsl:call-template>
                        </xsl:if>                        </td>
        ...
        <!--                    <xsl:value-of select="format-number(ImponibileImporto,  '###.###.##0,00', 'euro')" /> -->
                              <xsl:call-template name="FormattaNumeri">
                                  <xsl:with-param name="numero" select="ImponibileImporto" />
                              </xsl:call-template>
                          </xsl:if>                          </td>
               ...
               <!--                 <xsl:value-of select="format-number(Imposta,  '###.###.##0,00', 'euro')" /> -->
                                <xsl:call-template name="FormattaNumeri">
                                  <xsl:with-param name="numero" select="Imposta" />
                               </xsl:call-template>
                              </xsl:otherwise>
                            </xsl:choose>
                          </xsl:if>                          </td>
          ...
          <!--              <xsl:value-of select="format-number(DatiGenerali/DatiGeneraliDocumento/DatiBollo/ImportoBollo,  '###.###.##0,00', 'euro')" /> -->
                            <xsl:call-template name="FormattaNumeri">
                              <xsl:with-param name="numero" select="DatiGenerali/DatiGeneraliDocumento/DatiBollo/ImportoBollo" />
                            </xsl:call-template>
                      </xsl:if>                    </td>
     ...
     <!--                   <xsl:value-of select="format-number(DatiGenerali/DatiGeneraliDocumento/Arrotondamento,  '###.###.##0,00', 'euro')" /> -->
                          <xsl:call-template name="FormattaNumeri">
                            <xsl:with-param name="numero" select="DatiGenerali/DatiGeneraliDocumento/Arrotondamento" />
                          </xsl:call-template>
                      </xsl:if>                      </td>
      ...
      <!--                  <xsl:value-of select="format-number(DatiGenerali/DatiGeneraliDocumento/ImportoTotaleDocumento,  '###.###.##0,00', 'euro')" /> -->
                          <xsl:call-template name="FormattaNumeri">
                            <xsl:with-param name="numero" select="DatiGenerali/DatiGeneraliDocumento/ImportoTotaleDocumento" />
                          </xsl:call-template>
                      </xsl:if>                      </td>
                  </tr>
                  <!-- FINE Importo Totale  -->
                ...
                <tbody>
                  <xsl:for-each select="DatiPagamento" >
                    ...
                    <xsl:variable name="CountDettaglioPagamento">
                      <xsl:value-of select="count(DettaglioPagamento)" />                      </xsl:variable>
                      ...
                      <tr>
                        <td>
                          <xsl:if test="ModalitaPagamento">
                            <span>
                              <xsl:value-of select="ModalitaPagamento" />                              </span>
                            <xsl:variable name="MP">
                              <xsl:value-of select="ModalitaPagamento" />                              </xsl:variable>
                            <xsl:choose>
          ...
          <!--                  <xsl:value-of select="format-number(ImportoPagamento,  '###.###.##0,00', 'euro')" /> -->
                              <xsl:call-template name="FormattaNumeri">
                                <xsl:with-param name="numero" select="ImportoPagamento" />
                              </xsl:call-template>
                          </xsl:if>
                          </td>
                      </tr>
              ...
              <xsl:for-each select="OpzRiepilogoIVA"  >
                <div class="tx-xsmall">* <xsl:value-of select="." /></div>
              </xsl:for-each>
