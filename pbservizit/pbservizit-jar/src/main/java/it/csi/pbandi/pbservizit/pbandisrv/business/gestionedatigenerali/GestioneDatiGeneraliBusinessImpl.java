...
package it.csi.pbandi.pbservizit.pbandisrv.business.gestionedatigenerali;
...
import it.csi.pbandi.pbservizit.pbandisrv.business.manager.CertificazioneManager;
...
import it.csi.pbandi.pbservizit.pbandisrv.business.richiestaerogazione.RichiestaErogazioneBusinessImpl;
...
import it.csi.pbandi.pbservizit.pbandisrv.dto.gestionedatigenerali.DatiCumuloDeMinimisDTO;
...
import it.csi.pbandi.pbservizit.pbandisrv.dto.gestionedatigenerali.DichiarazioneSpesaDTO;
...
import it.csi.pbandi.pbservizit.pbandisrv.dto.gestionedatigenerali.RigoAttivitaPregresseDTO;
import it.csi.pbandi.pbservizit.pbandisrv.dto.gestionedatigenerali.SedeDTO;
...
import it.csi.pbandi.pbservizit.pbandisrv.integration.db.jdbctemplate.vo.AllegatoVO;
import it.csi.pbandi.pbservizit.pbandisrv.integration.db.jdbctemplate.vo.AttivitaPregresseVO;
import it.csi.pbandi.pbservizit.pbandisrv.integration.db.jdbctemplate.vo.BeneficiarioEnteGiuridicoVO;
import it.csi.pbandi.pbservizit.pbandisrv.integration.db.jdbctemplate.vo.DelegatoVO;
import it.csi.pbandi.pbservizit.pbandisrv.integration.db.jdbctemplate.vo.EconomiaPerDatiGeneraliVO;
import it.csi.pbandi.pbservizit.pbandisrv.integration.db.jdbctemplate.vo.ErogazioneCausaleModalitaAgevolazioneVO;
import it.csi.pbandi.pbservizit.pbandisrv.integration.db.jdbctemplate.vo.EstremiBancariVO;
import it.csi.pbandi.pbservizit.pbandisrv.integration.db.jdbctemplate.vo.ImportoImpegnoAppaltiVO;
import it.csi.pbandi.pbservizit.pbandisrv.integration.db.jdbctemplate.vo.LiquidazioneModalitaAgevolazioneVO;
import it.csi.pbandi.pbservizit.pbandisrv.integration.db.jdbctemplate.vo.ProgettoBandoLineaVO;
import it.csi.pbandi.pbservizit.pbandisrv.integration.db.jdbctemplate.vo.RigoContoEconomicoProgettoCulturaVO;
import it.csi.pbandi.pbservizit.pbandisrv.integration.db.jdbctemplate.vo.RigoContoEconomicoProgettoVO;
...
import it.csi.pbandi.pbservizit.pbandisrv.integration.db.jdbctemplate.vo.SoggettoFinanziatoreProgettoVO;
...
import it.csi.pbandi.pbservizit.pbandisrv.integration.db.jdbctemplate.vo.TipoAllegatoVO;
...
import it.csi.pbandi.pbservizit.pbandisrv.integration.db.jdbctemplate.vo.revoca.TotaleRevocaProgettoVO;
...
import it.csi.pbandi.pbservizit.pbandisrv.integration.db.vo.PbandiDMacroSezioneModuloVO;
...
import it.csi.pbandi.pbservizit.pbandisrv.integration.db.vo.PbandiTDocProtocolloVO;
...
import it.csi.pbandi.pbservizit.pbandisrv.integration.db.vo.PbandiTNotificaProcessoVO;
...
//import it.csi.pbandi.pbservizit.pbandisrv.integration.services.moni.MoniDAO;
//import it.csi.pbandi.pbservizit.pbandisrv.integration.services.moni.exception.CumuloDeMinimisException;
...
import it.csi.pbandi.pbservizit.pbandisrv.interfacecsi.gestionedatigenerali.GestioneDatiGeneraliSrv;
...
//	}
    ...
    public it.csi.pbandi.pbservizit.pbandisrv.dto.gestionedatigenerali.DatiGeneraliDTO caricaDatiGenerali(
            java.lang.Long idUtente, java.lang.String identitaDigitale,
            java.lang.Long idProgetto, java.lang.Long idSoggettoBeneficiario)
            throws it.csi.pbandi.pbservizit.pbandisrv.exception.gestionedatigenerali.GestioneDatiGeneraliException,
    ...
    private DatiGeneraliDTO caricaDatiGenerali(Long idProgetto) throws GestioneDatiGeneraliException {
        ...
        if (idProcesso != null)
            datiGeneraliDTO.setIdProcesso(idProcesso.longValue());

        PbandiDLineaDiInterventoVO pbandiDLineaDiInterventoVO = progettoManager
                .getLineaDiInterventoNormativa(new BigDecimal(progettoBandoLineaVO.getIdBandoLinea()));// idBandoLinea
                                                                                                       // qui � il
        ...
        // Cultura.
        boolean isBandoCultura = (pbandiDLineaDiInterventoVO.getIdProcesso() != null
                && pbandiDLineaDiInterventoVO.getIdProcesso().intValue() == 3);
        ...
        logger.info("descBreveLineaNormativa post isCapofila : " + datiGeneraliDTO.getIsCapofila());
        ...
        logger.info("descBreveLineaNormativa post findImportoAgevolato : " + datiGeneraliDTO.getImportoAgevolato());

        datiGeneraliDTO.setDescrizioneBeneficiario(progettoManager
                .getDenominazioneBeneficiario(idProgettoB));

        logger.info("descBreveLineaNormativa post setDescrizioneBeneficiario : "
                + datiGeneraliDTO.getDescrizioneBeneficiario());
        ...
        try {
            ...
            logger.warn("Eccezione gestita. Non trovata la sede di intervento per il progetto[" + idProgetto
                    + "] ed il beneficiario[" + idSoggettoBeneficiario + "]" + e.getMessage());
        }
        ...
        try {
            ...
            // considera anche il numero civico, ma solo se l'indirizzo non finisce gia' con
            // un numero.
            ...
            logger.warn("Eccezione gestita. Non trovata la sede legale per il progetto[" + idProgetto
                    + "] ed il beneficiario[" + idSoggettoBeneficiario + "]" + e.getMessage());
         ...
         */
        TotaleQuietanzatoValidatoProgettoVO totQuietanzatoValidato = progettoManager
                .getTotaliQuietanzatoValidato(idProgetto);
        BigDecimal totaleQuietanzato = totQuietanzatoValidato.getImportoQuietanzato() == null ? new BigDecimal(0d)
                : totQuietanzatoValidato.getImportoQuietanzato();
        BigDecimal totaleValidato = totQuietanzatoValidato.getImportoValidato() == null ? new BigDecimal(0d)
                : totQuietanzatoValidato.getImportoValidato();
        datiGeneraliDTO.setImportoQuietanzato(NumberUtil.toDouble(totaleQuietanzato));
        datiGeneraliDTO.setImportoValidatoNettoRevoche(
                NumberUtil.subtract(NumberUtil.toDouble(totaleValidato), NumberUtil.toDouble(totaleRevocato)));

        /*
         * Rendicontato
         */
        // PBANDI-3270: commentato poichè getTotaleRendicontazione() restituisce il
        // "rendicontabile", anzichè il "rendicontato".
        // BigDecimal totaleRendicontato =
        // progettoManager.getTotaleRendicontazione(idProgetto);
        BigDecimal totaleRendicontato = progettoManager.getTotaleRendicontato(idProgetto);
        ...
        /*
         * JIRA 2120: Se il bando del progetto ha la regola BR37 allora calcolo le
         * liquidazioni
         * altrimenti calcolo le erogazioni.
         */
        Boolean isRegolaBR37 = null;
        try {
            isRegolaBR37 = regolaManager.isRegolaApplicabileForProgetto(idProgetto,
                    RegoleConstants.BR37_ATTIVITA_LIQUIDAZIONE_CONTRIBUTO);
        } catch (Exception e) {
            logger.warn("Eccezione gestita. Errore nella verifica della regola "
                    + RegoleConstants.BR37_ATTIVITA_LIQUIDAZIONE_CONTRIBUTO + " per il progetto[" + idProgetto + "] "
                    + e.getMessage());
        }

        logger.debug("isRegolaBR37=" + isRegolaBR37);

        Boolean isRegolaDematAttiva = null;
        try {
            isRegolaDematAttiva = regolaManager.isRegolaApplicabileForBandoLinea(progettoBandoLineaVO.getIdBandoLinea(),
                    RegoleConstants.BR42_ABILITAZIONE_ALLEGA_FILE);
        } catch (Exception e) {
            logger.warn("Eccezione gestita. Errore nella verifica della regola "
                    + RegoleConstants.BR42_ABILITAZIONE_ALLEGA_FILE + " per il progettoBandoLineaVO.getIdBandoLinea():"
                    + progettoBandoLineaVO.getIdBandoLinea() + " " + e.getMessage());
        }
        logger.debug("isRegolaDematAttiva=" + isRegolaDematAttiva);

        datiGeneraliDTO.setRegolaDematAttiva(isRegolaDematAttiva);

        if (isRegolaBR37 != null) {
            if (isRegolaBR37) {
                logger.debug("carico liquidazioni");

                List<LiquidazioneModalitaAgevolazioneVO> liquidazioni = progettoManager
                        .getTotaleLiquidazioniPerModalitaAgevolazione(idProgetto);
                logger.debug("liquidazioni caricate");
                ...
                logger.debug("carico erogazioni");

                /*
                 * Erogazioni
                 */
                List<ErogazioneCausaleModalitaAgevolazioneVO> erogazioni = progettoManager
                        .getTotaleErogazioniPerModalitaAgevolazione(idProgetto);
                logger.debug("post erogazioni");
...
//			AND PA.ID_PROGETTO = XXX;
            ...
            boolean isTipoOperazioneOk = progettoBandoLineaVO.getIdTipoOperazione().compareTo(new BigDecimal(1)) == 0
                    || progettoBandoLineaVO.getIdTipoOperazione().compareTo(new BigDecimal(2)) == 0;
            boolean isLinea1420Ok = pbandiDLineaDiInterventoVO.getIdLineaDiIntervento()
                    .compareTo(ID_LINEA_DI_INTERVENTO_POR_FESR_14_20) == 0;
        ...
        logger.info("[GestioneDatiGeneraliBusinessImpl :: caricaDatiGenerali()] - importo impegno = "
                + NumberUtil.toDouble(importoImpegnoVincolante));
        ...
        BeneficiarioEnteGiuridicoVO enteGiuridico = soggettoManager.findEnteGiuridicoBeneficiario(idProgetto,
                idSoggettoBeneficiario);

        if (enteGiuridico != null) {
        ...
        List<SedeProgettoVO> sediProgetto = sedeManager.findSediProgetto(new BigDecimal(idProgetto),
         ...
         * Soppressioni
         */
        // BigDecimal totaleSoppressioni =
        // progettoManager.getTotaleSoppressioni(idProgetto);
            ...
            datiGeneraliDTO
                    .setImportoCertificatoNettoUltimaPropAppr(certificazioneManager
                            .findImportoUltimaPropostaCertificazionePerProgetto(
                                    idProgetto,
                                    it.csi.pbandi.pbservizit.pbandiutil.common.Constants.STATO_PROPOSTA_CERTIFICAZIONE_APPROVATA));
        } catch (Exception e) {
        ...
        // importo totale banca / premialita
        datiGeneraliDTO.setImpTotBanca(getDichiarazioneDiSpesaManager().getImportoTotaleBanca(idProgetto));
        logger.end();
        return datiGeneraliDTO;
    }
    ...
    public it.csi.pbandi.pbservizit.pbandisrv.dto.gestionedatigenerali.DatiGeneraliDTO caricaDatiGeneraliPerValidazione(
            java.lang.Long idUtente, java.lang.String identitaDigitale,
            java.lang.Long idProgetto, java.lang.Long idSoggettoBeneficiario,
        ...
        PbandiTDichiarazioneSpesaVO dichiarazioneDiSpesaVO = new PbandiTDichiarazioneSpesaVO(
                new BigDecimal(idDichiarazione));
        ...
        if (dichiarazioniDiSpesaVO.length == 1) {
            ...
            DecodificaDTO decodificaTipoDich = decodificheManager
                    .findDecodifica(GestioneDatiDiDominioSrv.TIPO_DICHIARAZ_SPESA, idTipoDichiarazSpesa);
            ...
            if (decodificaTipoDich.getDescrizioneBreve().equals("FC")) {
                ...
                if (!list.isEmpty()) {
                    ...
                    datiGeneraliDTO.setIdDocumentoIndexDichiarazione(
                            dichiarazioniDiSpesaVO[0].getIdDichiarazioneSpesa().longValue());
                    ...
                    if (pbandiTDocProtocolloVO != null)
                        datiGeneraliDTO.setProtocollo(pbandiTDocProtocolloVO.getNumProtocollo());
                    if (docIndex.getIdStatoDocumento() != null
                            && (docIndex.getIdStatoDocumento().longValue() == statoInviato.getId().longValue()
                                    ...
                                    ||
                                    docIndex.getIdStatoDocumento().longValue() == statoProtocollato.getId()
                                            .longValue())) {
                    ...
                    logger.info("docIndex.getIdDocumentoIndex() trovato per comunicazione fine progetto : "
                            + docIndex.getIdDocumentoIndex()
                            + " , dichiarazioniDiSpesaVO[0].getIdDichiarazioneSpesa() : "
                            + dichiarazioniDiSpesaVO[0].getIdDichiarazioneSpesa());

                    // Aggiunto perch� nella finestra di validazione si vuole visualizzare
                    // id+data della comunicazione fine progetto al posto di id+data della
                ...
                logger.info("\n\n\n\ntipo dich NON FC, idDichiarazione "
                        + dichiarazioniDiSpesaVO[0].getIdDichiarazioneSpesa());
                ...
                LikeStartsWithCondition<PbandiTDocumentoIndexVO> startsWithCondition = new LikeStartsWithCondition<PbandiTDocumentoIndexVO>(
                        documentoIndexVO);
            ...
            integraDatiPiuGreen(idUtente, identitaDigitale, idProgetto, datiGeneraliDTO, statoInviato,
                    statoClassificato, statoProtocollato, decodificaTipoDich.getDescrizioneBreve(),
                    dichiarazioniDiSpesaVO[0].getIdTipoDichiarazSpesa());
    ...
    private void integraDatiPiuGreen(Long idUtente, String identitaDigitale, Long idProgetto,
            DatiGeneraliDTO datiGeneraliDTO,
        ...
        try {
            ...
            throw new GestioneDatiGeneraliException(
                    "Errore nella ricerca del progetto a contributo del progetto " + idProgetto);
        }
        ...
        if ("FC".equalsIgnoreCase(descrizioneBreveTipoDich)) {
            ...
            if (listaDS.size() > 0) {
                ...
                logger.info(
                        "+Green: trovato id dichSpesaContributo = " + dichSpesaContributo.getIdDichiarazioneSpesa());

                // NOTA: uso questi campi, inizialmente previsti solo per le CFP,
                // per metterci i dati della DS contributo.
                datiGeneraliDTO
                        .setIdComunicazFineProgPiuGreen(dichSpesaContributo.getIdDichiarazioneSpesa().longValue());
                if (dichSpesaContributo.getDtDichiarazione() != null)
                    datiGeneraliDTO.setDtComunicazionePiuGreen(
                            "" + DateUtil.getData(dichSpesaContributo.getDtDichiarazione().getTime()));
                ...
                if (!list.isEmpty()) {
                    ...
                    // ID DOC INDEX
                    // NOTA: metto l'id doc index della DS Contributo nel campo
                    // IdDocIndexComunicFineProgettoPiuGreen
                    // nonostante il nome di quest'ultimo sia riferito alla CFP.
                    datiGeneraliDTO
                            .setIdDocIndexComunicFineProgettoPiuGreen(docIndex.getIdDocumentoIndex().longValue());
                    logger.info("+Green: trovato documento index con id = "
                    ...
                    logger.info(
                            "+Green: StatoDichiarazionePiuGreen = " + datiGeneraliDTO.getStatoDichiarazionePiuGreen());

                    // FLAG FIRMA CARTACEA.
                    datiGeneraliDTO.setFlagFirmaCartaceaPiuGreen(docIndex.getFlagFirmaCartacea());
                    logger.info(
                            "+Green: FlagFirmaCartaceaPiuGreen = " + datiGeneraliDTO.getFlagFirmaCartaceaPiuGreen());
    ...
    private BigDecimal getTotaleCofinanzimentoPubblico(Long idProgetto) {
        List<SoggettoFinanziatoreProgettoVO> soggettiFinanziatoriPubblici = progettoManager
                .findSoggettiFinanziatoriPubblici(idProgetto);
    ...
    private BigDecimal getTotaleCofinanziamentoPrivato(Long idProgetto) {
        List<SoggettoFinanziatoreProgettoVO> soggettiFinanziatoriPrivati = progettoManager
                .findSoggettiFinanziatoriPrivati(idProgetto);
        BigDecimal totale = new BigDecimal(0d);
    ...
    private BigDecimal getTotaleAmmesso(Long idProgetto) {
        ...
        if (!ObjectUtil.isEmpty(righi)) {
            for (RigoContoEconomicoProgettoVO r : righi) {
                ...
                logger.info("getTotaleAmmesso(): ImportoAmmessoFinanziamento = " + r.getImportoAmmessoFinanziamento()
                        + "; totaleAmmesso = " + totaleAmmesso);
            }
        }
    ...
    private BigDecimal getTotaleAmmessoCultura(Long idProgetto) {
        List<RigoContoEconomicoProgettoCulturaVO> righi = progettoManager
                .getRigheContoEconomicoMasterCultura(idProgetto);
        BigDecimal totaleAmmesso = new BigDecimal(0D);
        if (!ObjectUtil.isEmpty(righi)) {
            for (RigoContoEconomicoProgettoCulturaVO r : righi) {
                if (r.getIdVoceDiSpesa() != null) {
                    ...
                    logger.info("getTotaleAmmessoCultura(): ImportoAmmessoFinanziamento = "
                            + r.getImportoAmmessoFinanziamento() + "; totaleAmmesso = " + totaleAmmesso);
                }
            }
        }
    ...
    public DatiCumuloDeMinimisDTO caricaDatiCumuloDeMinimis(
        ...
        // PK : Pacilio dice che non serve a nulla.... non cancello il metodo per non
        // dover modificare l'interfaccia
//		 
//		ValidatorInput.verifyNullValue(nameParameter, idUtente,
...
//		}
    ...
    private DatiCumuloDeMinimisDTO setDefaultValueIfDatiCumuloDeMinimisIsNull(
            DatiCumuloDeMinimisDTO datiCumuloDeMinimis) {
    ...
    public AttivitaPregresseDTO[] caricaAttivitaPregresse(UserInfoSec userInfo, Long idProgetto) throws CSIException,
        ...
        ValidatorInput.verifyNullValue(nameParameter, userInfo.getIdUtente(),
        ...
        if (!ObjectUtil.isEmpty(attivita)) {
            ...
            for (AttivitaPregresseVO vo : attivita) {
                if (dto == null) {
                    ...
                    if (!userInfo.getCodiceRuolo().equals("BEN-MASTER")
                            && !userInfo.getCodiceRuolo().equals("PERSONA-FISICA")) {
                        dto.setIdDocumentoIndex(
                                vo.getIdDocumentoIndex() == null ? null : "" + vo.getIdDocumentoIndex());
                        dto.setNomeDocumento(vo.getNomeDocumento());
                    }

                    if (vo.getIdDocumentoIndex() != null && vo.getCodElemento().equals("H")) {
                        PbandiTDocumentoIndexVO reportValidazioneVO = getReportByIdDocIndexChecklist(
                                vo.getIdDocumentoIndex());
                        if (reportValidazioneVO != null) {
                            dto.setIdDocumentoIndexReport(reportValidazioneVO.getIdDocumentoIndex() == null ? null
                                    : "" + reportValidazioneVO.getIdDocumentoIndex());
                            dto.setNomeDocumentoReport(reportValidazioneVO.getNomeDocumento());
                        }
                    }
                ...
                if (dto.getDescAttivita().equals(vo.getDescElemento()) && dto.getData() != null
                        && dto.getData().equals(vo.getData())) {
                    /*
    ...
    private PbandiTDocumentoIndexVO getReportByIdDocIndexChecklist(BigDecimal idDocIndexChecklist) {
        PbandiTDocumentoIndexVO pbandiTDocumentoIndexVO = new PbandiTDocumentoIndexVO();
        pbandiTDocumentoIndexVO.setIdDocumentoIndex(idDocIndexChecklist);
        List<PbandiTDocumentoIndexVO> pbandiTDocumentoIndexVOs = genericDAO.findListWhere(pbandiTDocumentoIndexVO);

        if (pbandiTDocumentoIndexVOs != null && pbandiTDocumentoIndexVOs.size() > 0) {
            pbandiTDocumentoIndexVO = new PbandiTDocumentoIndexVO();
            pbandiTDocumentoIndexVO.setIdTarget(pbandiTDocumentoIndexVOs.get(0).getIdTarget());
            pbandiTDocumentoIndexVO.setIdTipoDocumentoIndex(new BigDecimal(24));
            pbandiTDocumentoIndexVOs = genericDAO.findListWhere(pbandiTDocumentoIndexVO);

            if (pbandiTDocumentoIndexVOs != null && pbandiTDocumentoIndexVOs.size() > 0) {
                return pbandiTDocumentoIndexVOs.get(0);
            }
        }
        return null;
    }

    // PBANDI_R_SOGGETTI_CORRELATI --> settare ID_TIPO_SOGGETTO_CORRELATO = 21
    // (delegato). la tab � in join con :
    ...
    public TipoAllegatoDTO[] findTipoAllegati(Long idUtente, String identitaIride, Long progrBandoLineaIntervento,
        ...
        ValidatorInput.verifyNullValue(nameParameter, idUtente, identitaIride, progrBandoLineaIntervento,
                codTipoDocIndex);
        ...
        if (idDichiarazione != null && idDichiarazione > 0) {
            ...
            tipoAllegatiDTOResult = beanUtil.transform(genericDAO.findListWhere(filtroTipoAllegato),
                    TipoAllegatoDTO.class);
            ...
            tipoAllegatiDTOResult = beanUtil.transform(genericDAO.findListWhere(filtroTipoAllProg),
                    TipoAllegatoDTO.class);
        }

        if (tipoAllegatiDTOResult == null || tipoAllegatiDTOResult.length == 0) {
            // Non esistono allegati gi� fleggati allora carico i tipi allegati per un
            // particolare bando linea
    ...
    public TipoAllegatoDTO[] findTipoAllegatiErogazione(Long idUtente, String identitaIride,
        ...
        BigDecimal idCausaleErogazione = decodificheManager.decodeDescBreve(PbandiDCausaleErogazioneVO.class,
                codCausaleErogazione);
        ...
        List<PbandiWProgettoDocAllegVO> progettoDocAlleg = beanUtil.transformToArrayList(resultList,
                PbandiWProgettoDocAllegVO.class);

        if (progettoDocAlleg != null && progettoDocAlleg.size() > 0) {

            List<TipoAllegatoProgettoVO> arrayFiltroVO = beanUtil.transformToArrayList(resultList,
                    TipoAllegatoProgettoVO.class);
    ...
    public EsitoOperazioneSalvaTipoAllegati salvaTipoAllegati(Long idUtente,
        ...
        if ((idDichiarazioneSpesa == null || idDichiarazioneSpesa == 0) && idProgetto != null) {
            List<PbandiWProgettoDocAllegVO> progettoDocAlleg = beanUtil.transformToArrayList(tipoAllegatiDTO,
                    PbandiWProgettoDocAllegVO.class);
            if (progettoDocAlleg != null && progettoDocAlleg.size() > 0) {
                try {
                    ...
                    List<TipoAllegatoProgettoVO> arrayFiltroVO = beanUtil.transformToArrayList(tipoAllegatiDTO,
                            TipoAllegatoProgettoVO.class);
                    List<TipoAllegatoProgettoVO> tipoAllegatiList = genericDAO
                            .findListWhere(Condition.filterBy(arrayFiltroVO));
            ...
            List<PbandiRDichSpesaDocAllegVO> dichSpesaDocAlleg = beanUtil.transformToArrayList(tipoAllegatiDTO,
                    PbandiRDichSpesaDocAllegVO.class);
    ...
    public DatiPiuGreenDTO findDatiPiuGreen(Long idUtente, String identitaDigitale, Long idProgettoFinanziamento)
        ...
        try {
            datiPiuGreenDTO
                    .setRegolaBR44Attiva(regolaManager.isRegolaApplicabileForProgetto(idProgettoFinanziamento, "BR44"));
        } catch (Exception e) {
            logger.warn("Errore nella verifica della regola BR44 per il progetto[" + idProgettoFinanziamento + "] "
                    + e.getMessage());
        }
        ...
        if (datiPiuGreenDTO.getRegolaBR44Attiva()) {
            ...
            // Verifica se in PBANDI_T_DICHIARAZIONE_SPESA esiste una DS Integrativa NON
            // CHIUSA associata al progetto contributo.
        ...
        logger.info("findDatiPiuGreen(): RegolaBR44Attiva = " + datiPiuGreenDTO.getRegolaBR44Attiva()
                + "; IdProgettoContributo:" + datiPiuGreenDTO.getIdProgettoContributo() + "; IdDichSpesaIntegrativa:"
                + datiPiuGreenDTO.getIdDichSpesaIntegrativa());
    ...
    // Dato l'id di una DS finanziamento, restituisce l'id della Ds Integrativa
    // contributo.
    public DichiarazioneSpesaDTO findDsIntegrativaContributo(Long idUtente, String identitaDigitale,
            Long idDichiarazioneSpesaFinanziamento)
            throws CSIException, SystemException, UnrecoverableException,
            GestioneDatiGeneraliException {
        ...
        logger.info("findDsIntegrativaContributo(): idDichiarazioneSpesaFinanziamento:"
                + idDichiarazioneSpesaFinanziamento);
    ...
    public NotificaDTO[] findElencoNotifiche(
            Long idUtente, String identitaIride, Long idBandoLineaIntervento, Long idSoggettoBeneficiario,
            long[] idProgetti, String codiceRuolo)
        ...
        ValidatorInput.verifyNullValue(nameParameter, idUtente, identitaIride, idBandoLineaIntervento,
                idSoggettoBeneficiario);
        ...
        List<PbandiTNotificaProcessoVO> listaNotifiche = progettoManager.caricaListaNotifiche(idBandoLineaIntervento,
                idSoggettoBeneficiario, idProgetti, codiceRuolo);
