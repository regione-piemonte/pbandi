...
public class PbandiTProgettoVO extends GenericVO {
	...
	public PbandiTProgettoVO(BigDecimal idUtenteIns, BigDecimal sogliaSpesaCalcErogazioni, String codiceVisualizzato,
			String acronimoProgetto, Date dtChiusuraProgetto, BigDecimal idGrVulnerabiliProg,
			BigDecimal idTipoLocalizzazione, BigDecimal idStatoProgetto, BigDecimal percentualePremialita,
			BigDecimal flagPrenotAvvio, BigDecimal idIndRisultatoProgram, String noteChiusuraProgetto,
			BigDecimal idDomanda, BigDecimal idProgettoFinanz, BigDecimal idTipoOperazione, String fascicoloActa,
			BigDecimal idTemaPrioritario, BigDecimal importoPremialita, String titoloProgetto,
			BigDecimal idGrandeProgetto, BigDecimal idUtenteAgg, BigDecimal idProgettoComplesso,
			BigDecimal idClassificazioneRa, Date dtInserimento, BigDecimal idSpecificaStato, BigDecimal idObiettivoTem,
			String abstractProgetto, Date dtComitato, BigDecimal idTipoStrumentoProgr, String idIstanzaProcesso,
			BigDecimal idCategoriaCipe, BigDecimal idProgetto, String cup, BigDecimal idObiettivoSpecifQsn,
			BigDecimal idProgettoPadre, Date dtConcessione, Date dtAggiornamento, String codiceProgetto,
			BigDecimal idStrumentoAttuativo, String flagArFlux, String flagProgettoMaster, BigDecimal idSettoreCpt,
			BigDecimal idIndicatoreQsn, BigDecimal idTipologiaCipe, BigDecimal idTipoProceduraOrig, Date dtFirmaAccordo, Date dtCompletamentoValutazione) {
	...
	public boolean isValid() {
		...
		if (isPKValid() && idUtenteIns != null && codiceVisualizzato != null && idStatoProgetto != null
				&& idDomanda != null && idTipoOperazione != null && titoloProgetto != null && dtInserimento != null
				&& idTipoStrumentoProgr != null && codiceProgetto != null && idStrumentoAttuativo != null
				&& flagArFlux != null && flagProgettoMaster != null) {
			isValid = true;
		}
		return isValid;
	}
	...
	public String toString() {
		...
		if (!DataFilter.isEmpty(temp))
			sb.append(" idObiettivoSpecifQsn: " + temp + "\t\n");

		temp = DataFilter.removeNull(idProgettoPadre);
		if (!DataFilter.isEmpty(temp))
			sb.append(" idProgettoPadre: " + temp + "\t\n");

		temp = DataFilter.removeNull(dtConcessione);
		if (!DataFilter.isEmpty(temp))
			sb.append(" dtConcessione: " + temp + "\t\n");

		temp = DataFilter.removeNull(dtAggiornamento);
		if (!DataFilter.isEmpty(temp))
			sb.append(" dtAggiornamento: " + temp + "\t\n");

		temp = DataFilter.removeNull(codiceProgetto);
		if (!DataFilter.isEmpty(temp))
			sb.append(" codiceProgetto: " + temp + "\t\n");

		temp = DataFilter.removeNull(idStrumentoAttuativo);
		if (!DataFilter.isEmpty(temp))
			sb.append(" idStrumentoAttuativo: " + temp + "\t\n");

		temp = DataFilter.removeNull(flagArFlux);
		if (!DataFilter.isEmpty(temp))
			sb.append(" flagArFlux: " + temp + "\t\n");

		temp = DataFilter.removeNull(flagProgettoMaster);
		if (!DataFilter.isEmpty(temp))
			sb.append(" flagProgettoMaster: " + temp + "\t\n");

		temp = DataFilter.removeNull(idSettoreCpt);
		if (!DataFilter.isEmpty(temp))
			sb.append(" idSettoreCpt: " + temp + "\t\n");

		temp = DataFilter.removeNull(idIndicatoreQsn);
		if (!DataFilter.isEmpty(temp))
			sb.append(" idIndicatoreQsn: " + temp + "\t\n");

		temp = DataFilter.removeNull(idTipologiaCipe);
		if (!DataFilter.isEmpty(temp))
			sb.append(" idTipologiaCipe: " + temp + "\t\n");

		temp = DataFilter.removeNull(idTipoProceduraOrig);
		if (!DataFilter.isEmpty(temp))
			sb.append(" idTipoProceduraOrig: " + temp + "\t\n");
		
		temp = DataFilter.removeNull(dtFirmaAccordo);
		if (!DataFilter.isEmpty(temp))
			sb.append(" dtFirmaAccordo: " + temp + "\t\n");
		
		temp = DataFilter.removeNull(dtCompletamentoValutazione);
		if (!DataFilter.isEmpty(temp))
			sb.append(" dtCompletamentoValutazione: " + temp + "\t\n");
