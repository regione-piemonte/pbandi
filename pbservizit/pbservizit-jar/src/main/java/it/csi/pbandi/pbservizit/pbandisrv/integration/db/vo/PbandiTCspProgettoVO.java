...
public class PbandiTCspProgettoVO extends GenericVO {
	...
	public PbandiTCspProgettoVO(BigDecimal idUtenteIns, String flagBeneficiarioCup, BigDecimal idAttivitaAteco,
			BigDecimal idIndRisultatoProgram, String flagGeneratoreEntrate, BigDecimal idTipoOperazione,
			BigDecimal idTemaPrioritario, String titoloProgetto, BigDecimal idGrandeProgetto, BigDecimal idUtenteAgg,
			BigDecimal idProgettoComplesso, BigDecimal idClassificazioneRa, Date dtElaborazione, String flagInvioMonit,
			BigDecimal idObiettivoTem, BigDecimal idTipoAiuto, BigDecimal idCspProgetto, BigDecimal idIstanzaDomanda,
			Date dtComitato, BigDecimal idTipoStrumentoProgr, BigDecimal idCategoriaCipe, String flagCardine,
			BigDecimal idProgetto, String flagLeggeObbiettivo, String cup, String flagRichiestaCup,
			BigDecimal idObiettivoSpecifQsn, String flagAvviabile, Date dtConcessione, Date dtInizioValidita,
			BigDecimal idStrumentoAttuativo, String flagDettaglioCup, String numeroDomanda,
			BigDecimal idDimensioneTerritor, BigDecimal idSettoreCpt, String flagAltroFondo, String statoProgramma,
			BigDecimal idIndicatoreQsn, BigDecimal progrBandoLineaIntervento, BigDecimal idTipologiaCipe,
			Date dtPresentazioneDomanda, String flagPPP, String flagStrategico, Date dtFirmaAccordo,
			Date dtCompletamentoValutazione) {
	...
	public boolean isValid() {
		...
		if (isPKValid() && idUtenteIns != null && flagBeneficiarioCup != null && flagGeneratoreEntrate != null
				&& idTipoOperazione != null && titoloProgetto != null && flagInvioMonit != null && flagCardine != null
				&& flagLeggeObbiettivo != null && flagRichiestaCup != null && flagAvviabile != null
				&& dtInizioValidita != null && flagDettaglioCup != null && flagAltroFondo != null
				&& statoProgramma != null && progrBandoLineaIntervento != null) {
			isValid = true;
		}
		return isValid;
	}
	...
	public String toString() {
		...
		if (!DataFilter.isEmpty(temp))
			sb.append(" flagBeneficiarioCup: " + temp + "\t\n");

		temp = DataFilter.removeNull(idAttivitaAteco);
		if (!DataFilter.isEmpty(temp))
			sb.append(" idAttivitaAteco: " + temp + "\t\n");
		...
		if (!DataFilter.isEmpty(temp))
			sb.append(" flagGeneratoreEntrate: " + temp + "\t\n");
		...
		if (!DataFilter.isEmpty(temp))
			sb.append(" dtElaborazione: " + temp + "\t\n");

		temp = DataFilter.removeNull(flagInvioMonit);
		if (!DataFilter.isEmpty(temp))
			sb.append(" flagInvioMonit: " + temp + "\t\n");
		...
		if (!DataFilter.isEmpty(temp))
			sb.append(" idTipoAiuto: " + temp + "\t\n");

		temp = DataFilter.removeNull(idCspProgetto);
		if (!DataFilter.isEmpty(temp))
			sb.append(" idCspProgetto: " + temp + "\t\n");

		temp = DataFilter.removeNull(idIstanzaDomanda);
		if (!DataFilter.isEmpty(temp))
			sb.append(" idIstanzaDomanda: " + temp + "\t\n");
		...
		if (!DataFilter.isEmpty(temp))
			sb.append(" flagLeggeObbiettivo: " + temp + "\t\n");
		...
		if (!DataFilter.isEmpty(temp))
			sb.append(" flagRichiestaCup: " + temp + "\t\n");
		...
		if (!DataFilter.isEmpty(temp))
			sb.append(" dtInizioValidita: " + temp + "\t\n");
		...
		if (!DataFilter.isEmpty(temp))
			sb.append(" flagDettaglioCup: " + temp + "\t\n");

		temp = DataFilter.removeNull(numeroDomanda);
		if (!DataFilter.isEmpty(temp))
			sb.append(" numeroDomanda: " + temp + "\t\n");

		temp = DataFilter.removeNull(idDimensioneTerritor);
		if (!DataFilter.isEmpty(temp))
			sb.append(" idDimensioneTerritor: " + temp + "\t\n");
		...
		if (!DataFilter.isEmpty(temp))
			sb.append(" flagAltroFondo: " + temp + "\t\n");

		temp = DataFilter.removeNull(statoProgramma);
		if (!DataFilter.isEmpty(temp))
			sb.append(" statoProgramma: " + temp + "\t\n");
		...
		if (!DataFilter.isEmpty(temp))
			sb.append(" progrBandoLineaIntervento: " + temp + "\t\n");
		...
		if (!DataFilter.isEmpty(temp))
			sb.append(" dtPresentazioneDomanda: " + temp + "\t\n");

		temp = DataFilter.removeNull(flagPPP);
		if (!DataFilter.isEmpty(temp))
			sb.append(" flagPPP: " + temp + "\t\n");

		temp = DataFilter.removeNull(flagStrategico);
		if (!DataFilter.isEmpty(temp))
			sb.append(" flagStrategico: " + temp + "\t\n");
