...
import it.csi.pbandi.pbservizit.pbandisrv.integration.db.jdbctemplate.vo.BandolineaEnteCompetenzaVO;
...
import it.csi.pbandi.pbservizit.pbandisrv.integration.db.vo.PbandiDStatoAttoVO;
...
public class BilancioManager {
	...
	public PbandiDStatoAttoVO statoAttoBozza() {
		...
		statoAttoVO.setDescBreveStatoAtto(it.csi.pbandi.pbservizit.pbandiutil.common.Constants.STATO_ATTO_LIQUIDAZIONE_BOZZA);
	...
	public PbandiDStatoAttoVO statoAttoCancellato() {
		PbandiDStatoAttoVO statoAttoVO = new PbandiDStatoAttoVO();
		statoAttoVO.setDescBreveStatoAtto(it.csi.pbandi.pbservizit.pbandiutil.common.Constants.STATO_ATTO_LIQUIDAZIONE_CANCELLATO);
		statoAttoVO = genericDAO.findSingleWhere(statoAttoVO);
		return statoAttoVO;
	}
	...
	public BigDecimal caricaAttoDiLiquidazione(AttoLiquidazione attoLiquidazione) {
		...
		dt.setTiporecord(it.csi.pbandi.pbservizit.pbandisrv.util.Constants.TIPO_RECORD_DT);
		...
		for (DettaglioConsultazioneAtto dettaglio : attoLiquidazione
			...
			dl.setTiporecord(it.csi.pbandi.pbservizit.pbandisrv.util.Constants.TIPO_RECORD_DL);
			...
			dm.setTiporecord(it.csi.pbandi.pbservizit.pbandisrv.util.Constants.TIPO_RECORD_DM);
	...
	*/
	...
	public Boolean isResultOk(
		...
		if(resultCode == it.csi.pbandi.pbservizit.pbandisrv.util.Constants.CONSULTAZIONE_ATTO_BILANCIO_CODICE_NESSUN_ERRORE) {
			return true;
		} else {
			String result = consultaAttoLiquidazione.getResultDesc();
			switch (resultCode) {
			case it.csi.pbandi.pbservizit.pbandisrv.util.Constants.CONSULTAZIONE_ATTO_BILANCIO_CODICE_PROBLEMA_TECNICO:
				logger.error("BILANCIO : PROBLEMA TECNICO : " + result);
				break;
			case it.csi.pbandi.pbservizit.pbandisrv.util.Constants.CONSULTAZIONE_ATTO_BILANCIO_CODICE_ANNO_ATTO_NON_VALIDO:
				logger.error("BILANCIO : ERRORE DATI : " + result);
				break;
			case it.csi.pbandi.pbservizit.pbandisrv.util.Constants.CONSULTAZIONE_ATTO_BILANCIO_CODICE_ATTO_INESISTENTE:
				logger.error("BILANCIO : ERRORE DATI : " + result);
				break;
			case it.csi.pbandi.pbservizit.pbandisrv.util.Constants.CONSULTAZIONE_ATTO_BILANCIO_CODICE_DIREZIONE_NON_VALIDA:
				logger.error("BILANCIO : ERRORE DATI : " + result);
				break;
			case it.csi.pbandi.pbservizit.pbandisrv.util.Constants.CONSULTAZIONE_ATTO_BILANCIO_CODICE_ERRORE_ORACLE:
				logger.error("BILANCIO : ERRORE ORACLE : " + result);
				break;
			case it.csi.pbandi.pbservizit.pbandisrv.util.Constants.CONSULTAZIONE_ATTO_BILANCIO_CODICE_NUMERO_ATTO_NON_VALIDO:
				logger.error("BILANCIO : ERRORE DATI : " + result);
				break;
			default:
