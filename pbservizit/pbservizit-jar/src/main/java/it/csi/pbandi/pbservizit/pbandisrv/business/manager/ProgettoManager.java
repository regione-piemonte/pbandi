...
import it.csi.pbandi.pbservizit.pbandisrv.integration.db.jdbctemplate.dao.PbandiDatiGeneraliDAOImpl;
import it.csi.pbandi.pbservizit.pbandisrv.integration.db.jdbctemplate.dao.PbandiDichiarazioneDiSpesaDAOImpl;
import it.csi.pbandi.pbservizit.pbandisrv.integration.db.jdbctemplate.dao.PbandiGestioneProgettoDAOImpl;
import it.csi.pbandi.pbservizit.pbandisrv.integration.db.jdbctemplate.dao.PbandiProfilazioneDAOImpl;
import it.csi.pbandi.pbservizit.pbandisrv.integration.db.jdbctemplate.vo.AcronimoVO;
...
import it.csi.pbandi.pbservizit.pbandisrv.integration.db.jdbctemplate.vo.ProgettoGefoVO;
import it.csi.pbandi.pbservizit.pbandisrv.integration.db.jdbctemplate.vo.ProgettoSoggettoPartnerVO;
...
import it.csi.pbandi.pbservizit.pbandisrv.integration.db.jdbctemplate.vo.TotaleRendicontatoProgettoVO;
import it.csi.pbandi.pbservizit.pbandisrv.integration.db.jdbctemplate.vo.TotaleRendicontazioneProgettoVO;
import it.csi.pbandi.pbservizit.pbandisrv.integration.db.jdbctemplate.vo.datigenerali.DatiGeneraliVO;
...
public class ProgettoManager {
	...
	@Autowired
	private PbandiGestioneProgettoDAOImpl gestioneProgettoDAOImpl;

	
	
	public PbandiGestioneProgettoDAOImpl getGestioneProgettoDAOImpl() {
		return gestioneProgettoDAOImpl;
	}

	public void setGestioneProgettoDAOImpl(PbandiGestioneProgettoDAOImpl gestioneProgettoDAOImpl) {
		this.gestioneProgettoDAOImpl = gestioneProgettoDAOImpl;
	}
	...
	public PbandiDLineaDiInterventoVO getLineaDiInterventoAsse(
		...
		String descTipoLinea = it.csi.pbandi.pbservizit.pbandisrv.util.Constants.DESC_TIPO_LINEA_ASSE;
	...
	public PbandiDLineaDiInterventoVO getLineaDiInterventoNormativa(
			BigDecimal progrBandoLineaIntervento) {
		String descTipoLinea = it.csi.pbandi.pbservizit.pbandisrv.util.Constants.DESC_TIPO_LINEA_NORMATIVA;
...
//				result = false;
	...
	public BeneficiarioDTO findBeneficiario(Long idSoggetto, Long idProgetto) {
		...
		it.csi.pbandi.pbservizit.pbandisrv.integration.db.jdbctemplate.vo.dichiarazionedispesa.BeneficiarioVO beneficiarioVO = getPbandiDichiarazioneDiSpesaDAO()
	...
	//
	// it.csi.pbandi.pbservizit.pbandisrv.integration.db.jdbctemplate.vo.dichiarazionedispesa.BeneficiarioVO
	...
	public it.csi.pbandi.pbservizit.pbandisrv.dto.manager.report.ProgettoDTO getProgetto(
		...
		it.csi.pbandi.pbservizit.pbandisrv.dto.manager.report.ProgettoDTO progettoDTO = beanUtil
				.transform(
						pbandiTProgettoVO,
		...
		ProgettoVO progettoVO = getPbandiDichiarazioneDiSpesaDAO().findDatiProgetto(
	...
	public BigDecimal getTotaleRecuperi(Long idProgetto) {
		BigDecimal totaleRecuperi = getTotaleRecuperi(idProgetto, it.csi.pbandi.pbservizit.pbandisrv.util.Constants.COD_FIDEIUSSIONI_TIPO_RECUPERO_RECUPERO);
	...
	public List<RecuperoProgettoVO> getTotaleRecuperiPerModalitaAgevolazione(Long idProgetto) {
		List<RecuperoProgettoVO> result = getTotaleRecuperiPerModalitaAgevolazione(idProgetto, it.csi.pbandi.pbservizit.pbandisrv.util.Constants.COD_FIDEIUSSIONI_TIPO_RECUPERO_RECUPERO);
		return result;
	}
	...
	public BigDecimal getTotalePreRecuperi(Long idProgetto) {
		BigDecimal totalePreRecuperi = getTotaleRecuperi(idProgetto, it.csi.pbandi.pbservizit.pbandisrv.util.Constants.COD_FIDEIUSSIONI_TIPO_RECUPERO_PRERECUPERO);
	...
	public List<RecuperoProgettoVO>  getTotalePreRecuperiPerModalitaAgevolazione(Long idProgetto) {
		List<RecuperoProgettoVO> result = getTotaleRecuperiPerModalitaAgevolazione(idProgetto, it.csi.pbandi.pbservizit.pbandisrv.util.Constants.COD_FIDEIUSSIONI_TIPO_RECUPERO_PRERECUPERO);
		return result;
	}
	...
	public BigDecimal getTotaleSoppressioni(Long idProgetto) {
		BigDecimal totaleSoppressioni = getTotaleRecuperi(idProgetto, it.csi.pbandi.pbservizit.pbandisrv.util.Constants.COD_FIDEIUSSIONI_TIPO_RECUPERO_SOPPRESSIONE); 
	...
	public List<RecuperoProgettoVO>  getTotaleSoppressioniPerModalitaAgevolazione(Long idProgetto) {
		List<RecuperoProgettoVO> result = getTotaleRecuperiPerModalitaAgevolazione(idProgetto, it.csi.pbandi.pbservizit.pbandisrv.util.Constants.COD_FIDEIUSSIONI_TIPO_RECUPERO_SOPPRESSIONE);
		return result;
	}
	...
	public List<ErogazioneCausaleModalitaAgevolazioneVO> getTotaleErogazioniPerModalitaAgevolazione(Long idProgetto) {
		...
		if(erogazioni!=null)
			logger.debug("trovate " +  erogazioni.size() + " erogazioni");
		else
			logger.debug("erogazioni null");
		...
		if (!ObjectUtil.isEmpty(erogazioni)) {
			 ...
			 for (ErogazioneCausaleModalitaAgevolazioneVO erog : erogazioni) {
				 ...
				 logger.debug("idErogazione ="+erog.getIdErogazione());
			 }
			 /*
	 ...
	 * @return
	 */
	// Alex: se non ho capito male, corrisponde al "rendicontabile".
	...
	// Alex: restituisce il "rendicontato"
	// Metodo creato sulla falsa riga di getTotaleRendicontazione(). 
	public BigDecimal getTotaleRendicontato(Long idProgetto) {
		TotaleRendicontatoProgettoVO filtroVO = new TotaleRendicontatoProgettoVO();
		filtroVO.setIdProgetto(NumberUtil.toBigDecimal(idProgetto));
		BigDecimal result = new BigDecimal(0D);
		try {
			TotaleRendicontatoProgettoVO totale = genericDAO.findSingleOrNoneWhere(filtroVO);
			if (totale != null)
				result = totale.getImportoRendicontato();
		} catch (Exception e) {
			logger.error("Eccezione gestita.",e);
		}
		return result;
	...
	public List<SoggettoFinanziatoreProgettoVO> findSoggettiFinanziatoriPrivati(Long idProgetto) {
		...
		soggettoPrivato.setDescBreveSoggFinanziatore(it.csi.pbandi.pbservizit.pbandisrv.util.Constants.COD_SOGGETTO_FINANZIATORE_PRIVATO);
		
		SoggettoFinanziatoreProgettoVO soggettoPrivatoFESR = new SoggettoFinanziatoreProgettoVO();
		soggettoPrivatoFESR.setDescBreveSoggFinanziatore(it.csi.pbandi.pbservizit.pbandisrv.util.Constants.COD_SOGGETTO_FINANZIATORE_PRIVATO_FESR);
	...
	public  List<RigoContoEconomicoProgettoVO> getRigheContoEconomicoMaster(Long idProgetto) {
		return getRigheContoEconomico(idProgetto, it.csi.pbandi.pbservizit.pbandisrv.util.Constants.TIPOLOGIA_CONTO_ECONOMICO_MASTER);
	}
	
	public  List<RigoContoEconomicoProgettoCulturaVO> getRigheContoEconomicoMasterCultura(Long idProgetto) {
		return getRigheContoEconomicoCultura(idProgetto, it.csi.pbandi.pbservizit.pbandisrv.util.Constants.TIPOLOGIA_CONTO_ECONOMICO_MASTER);
	}
	...
	public  List<RigoContoEconomicoProgettoVO> getRigheContoEconomicoCopyIst(Long idProgetto) {
		return getRigheContoEconomico(idProgetto, it.csi.pbandi.pbservizit.pbandisrv.util.Constants.TIPOLOGIA_CONTO_ECONOMICO_COPY_IST);
	}
	...
	public  List<RigoContoEconomicoProgettoVO> getRigheContoEconomicoCopyBen(Long idProgetto) {
		return getRigheContoEconomico(idProgetto, it.csi.pbandi.pbservizit.pbandisrv.util.Constants.TIPOLOGIA_CONTO_ECONOMICO_COPY_BEN);
	}
	...
	public RecapitoVO caricaRecapiti(Long idProgetto) {
		return getGestioneProgettoDAOImpl().caricaRecapiti(idProgetto);
	}

	public int updateSoggettoProgettoSede(Long idUtente, Long progrSoggettoProgettoSede, Long idRecapito) {
		return getGestioneProgettoDAOImpl().updateSoggettoProgettoSede(idUtente, progrSoggettoProgettoSede, idRecapito );
	}

	public int updateSoggettoProgettoSedePec(Long idUtente, Long progrSoggettoProgettoSede, Long idRecapito) {
		return getGestioneProgettoDAOImpl().updateSoggettoProgettoSedePec(idUtente, progrSoggettoProgettoSede, idRecapito );
	}

	public RecapitoVO getMailFromRecapiti(String email) {
		return getGestioneProgettoDAOImpl().getMailFromRecapiti(email);
	}

	public RecapitoVO getPecFromRecapiti(String pec) {
		return getGestioneProgettoDAOImpl().getPecFromRecapiti(pec);
	}
	
	// Copiato da GestioneDocumentiDiSpesaBusinessImpl al momento non presente su pbservizit.
	public PartnerDTO[] findPartnersPerVisualizzaContoEconomico(Long idProgettoPadre) {
		...
		if (genericDAO.count(progettoMasterVO) == 1) {
			ProgettoSoggettoPartnerVO vo = beanUtil.transform(progettoMasterVO,	ProgettoSoggettoPartnerVO.class);
		...
		try {
			...
			partnerDTO = beanUtil.transformList(listaVO, PartnerDTO.class,	beanMap);
		} catch (Exception e) {
			logger.error("Progettomanager.findPartnersPerVisualizzaContoEconomico(): Errore durante la copia dei " + listaVO.size() + " elementi trovati nel DTO", e);
		}
		return partnerDTO.toArray(new PartnerDTO[partnerDTO.size()]);
	}
}
