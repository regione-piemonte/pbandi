...
public class SoggettoManager {
	...
	public ContestoIdentificativoDTO caricaContestoIdentificativoNelContestoSpring(
		...
		if (anonymous) {
			contestoIdentificativo
					.setIdUtente(it.csi.pbandi.pbservizit.pbandiutil.common.Constants.ID_UTENTE_FITTIZIO);
		} else {
			...
			if (params.length > 1) {
				...
				logger.info("caricaContestoIdentificativoNelContestoSpring newCtx=" + newCtx);
		...
		if (it.csi.pbandi.pbservizit.pbandiutil.common.Constants.ID_UTENTE_FITTIZIO != idUtente
	...
	public EstremiBancariDTO getEstremiBancari(Long idProgetto, Long idSoggetto) {
		...
		pbandiRSoggettoProgettoVO
				.setIdTipoAnagrafica(new BigDecimal(
						decodificheManager
								.findDecodifica(
	...
	public Long findSoggettoRappresentanteByProgetto(Long idProgetto) {
		String sql = "SELECT DISTINCT s.ID_SOGGETTO \n" +
				"FROM \n" +
				"pbandi_t_progetto p, \n" +
				"pbandi_r_soggetto_progetto c, \n" +
				"pbandi_t_soggetto s, \n" +
				"pbandi_r_sogg_prog_sogg_correl sc, \n" +
				"pbandi_r_soggetti_correlati sr \n" +
				"WHERE\n" +
				"c.id_progetto = p.id_progetto \n" +
				"AND c.id_tipo_anagrafica = 16 \n" +
				"AND c.dt_fine_validita IS NULL \n" +
				"AND s.id_soggetto = c.id_soggetto \n" +
				"AND c.progr_soggetto_progetto = sc.progr_soggetto_progetto \n" +
				"AND sc.progr_soggetti_correlati = sr.progr_soggetti_correlati \n" +
				"AND sr.id_tipo_soggetto_correlato = 1 \n" +
				"AND sr.dt_fine_validita IS NULL \n" +
				"AND p.ID_PROGETTO = ? ";
		Object[] params = new Object[]{idProgetto};
		return genericDAO.getJdbcTemplate().queryForObject(sql, params, Long.class);
	}
}