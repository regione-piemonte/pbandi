...
public class PbandiDVoceDiSpesaVO extends GenericVO {
	...
	public PbandiDVoceDiSpesaVO(BigDecimal idVoceDiSpesa, Date dtFineValidita, String codTipoVoceDiSpesa,
			BigDecimal idVoceDiSpesaMonit, Date dtInizioValidita, String descVoceDiSpesa, BigDecimal idVoceDiSpesaPadre,
			Long idTipologiaVoceDiSpesa, String flagEdit, String flagSpeseGestione) {
	...
	public String toString() {
		...
		if (!DataFilter.isEmpty(temp))
			sb.append(" codTipoVoceDiSpesa: " + temp + "\t\n");

		temp = DataFilter.removeNull(idVoceDiSpesaMonit);
		if (!DataFilter.isEmpty(temp))
			sb.append(" idVoceDiSpesaMonit: " + temp + "\t\n");
		...
		if (!DataFilter.isEmpty(temp))
			sb.append(" descVoceDiSpesa: " + temp + "\t\n");

		temp = DataFilter.removeNull(idVoceDiSpesaPadre);
		if (!DataFilter.isEmpty(temp))
			sb.append(" idVoceDiSpesaPadre: " + temp + "\t\n");

		temp = DataFilter.removeNull(idTipologiaVoceDiSpesa);
		if (!DataFilter.isEmpty(temp))
			sb.append(" idTipologiaVoceDiSpesa: " + temp + "\t\n");

		temp = DataFilter.removeNull(flagEdit);
		if (!DataFilter.isEmpty(temp))
			sb.append(" flagEdit: " + temp + "\t\n");
		
		temp = DataFilter.removeNull(flagSpeseGestione);
		if (!DataFilter.isEmpty(temp))
			sb.append(" flagSpeseGestione: " + temp + "\t\n");
