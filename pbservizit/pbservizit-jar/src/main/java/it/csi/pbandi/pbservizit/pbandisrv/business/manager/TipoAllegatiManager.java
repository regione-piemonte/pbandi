...
public class TipoAllegatiManager {
	 ...
	 * 
	 */
	public void spostaTipoAllegati(Long idBando, Long idProgetto, Long idDichiarazione) throws Exception{
	...
	public void spostaTipoAllegatiCultura(Long idBando, Long idProgetto, Long idDichiarazione, String tipoDocIndex)
			throws Exception {
		...
		try {
			...
			filtroAllegatiProgetto.setProgrBandoLineaIntervento(BigDecimal.valueOf(idBando));
			filtroAllegatiProgetto.setIdTipoDocumentoIndex(idTipoDocumentoIndex);
			List<PbandiRDichSpesaDocAllegVO> dichSpesa = beanUtil
					.transformList(genericDAO.findListWhere(filtroAllegatiProgetto), PbandiRDichSpesaDocAllegVO.class);
			for (int i = 0; dichSpesa.size() > 0 && i < dichSpesa.size(); i++) {
				dichSpesa.get(i).setidDichiarazioneSpesa(BigDecimal.valueOf(idDichiarazione));
			}
			genericDAO.insert(dichSpesa);
