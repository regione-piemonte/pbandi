...
import it.csi.pbandi.pbservizit.dto.archivioFile.InizializzaArchivioFileDTO;
import it.csi.pbandi.pbservizit.dto.archivioFile.UserSpaceDTO;
...
import it.csi.pbandi.pbservizit.integration.vo.PbandiTDocumentoIndexLockVO;
import it.csi.pbandi.pbservizit.integration.vo.PbandiTDocumentoIndexVO;
import it.csi.pbandi.pbservizit.integration.vo.PbandiTFileVO;
...
public class ArchivioFileDAOImpl extends JdbcDaoSupport implements ArchivioFileDAO {
	...
	@Autowired
	protected it.csi.pbandi.pbservizit.pbandisrv.business.archivio.ArchivioBusinessImpl archivioBusinessImpl;
	...
	public InizializzaArchivioFileDTO inizializzaArchivioFile (long idSoggetto, long idSoggettoBeneficiario, String codiceRuolo, long idUtente, HttpServletRequest req) 
		...
		try {
			...
			LOG.error(prf+" ERRORE in inizializzaArchivioFile(): ", e);
	...
	public ArrayList<FolderDTO> leggiRoot(long idSoggetto, long idSoggettoBeneficiario, String codiceRuolo) 
		...
		try {
			...
			LOG.info(prf + " listaArchivioFileVO.size=" + listaArchivioFileVO.size());
	...
	private Map<String,FolderDTO> archivioFileToListFolderDTOOrdered(
		...
		LOG.info("addFileToFolder terminato");
		...
		for ( FolderDTO folderDTO : folders.values()) {
			if(folderDTO.getIdPadre()==null){
				...
				if(father!=null) {
					...
					LOG.warn(" Attenzione, alberatura inconsistente. Trovato folder senza padre");
					LOG.warn(" IdFolder="+folderDTO.getIdFolder()+", nomeFolder="+folderDTO.getNomeFolder()+ ", idPadre="+folderDTO.getIdPadre());
				}
			}
		}
	...
	public EsitoLeggiFile leggiFileFirmaAutografa(long idDocumentoIndex) throws InvalidParameterException, Exception {
		String prf = "[ArchivioFileDAOImpl::leggiFileFirmaAutografa] ";
		LOG.info(prf + " BEGIN");		
		...
		try {
			...
			FileDTO fileDTO = documentoManager.leggiFileFirmaAutografa(idDocumentoIndex);
		...
		*/
		...
		try {
			...
			for (FileDTO fileDTO : filesDTO) {				
				...
				BigDecimal idEntita = this.idDaDescrizione("PBANDI_C_ENTITA", "ID_ENTITA", "NOME_ENTITA", it.csi.pbandi.pbservizit.util.Constants.ENTITA_C_FILE);
	...
	public static String getMimeType(MultivaluedMap<String, String> header) {
		String[] contentType = header.getFirst(it.csi.pbandi.pbservizit.util.Constants.CONTENT_TYPE).split(";");
	...
	private void aggiornaTracciamentoEntita(Long idUtente, String descOperazione, Long target, Long valorePrec, Long valoreNuovo) {
		try {
			...
			if (descOperazione.equalsIgnoreCase(opMoveFile))
				nomeEntita = it.csi.pbandi.pbservizit.util.Constants.ENTITA_C_FILE;
			else if (descOperazione.equalsIgnoreCase(opMoveFolder))
				nomeEntita = it.csi.pbandi.pbservizit.util.Constants.ENTITA_C_FOLDER;
	...
	public FileInfoDTO infoFile(long idDocumentoIndex, long idFolder, long idUtente, HttpServletRequest request)
		...
		try {
			...
			UserInfoSec userInfo = (UserInfoSec) request.getSession().getAttribute(it.csi.pbandi.pbservizit.util.Constants.USERINFO_SESSIONATTR);
			
			result = archivioBusinessImpl.getFileInfo(idUtente, userInfo.getIdIride(), idDocumentoIndex, idFolder);
	...
	public EsitoAssociaFilesDTO associaFiles(AssociaFilesRequest associaFilesRequest, Long idEntita, Long idUtente)
		...
		try {
			...
			BigDecimal idEntitaFornitore = this.idDaDescrizione("PBANDI_C_ENTITA", "ID_ENTITA", "NOME_ENTITA", it.csi.pbandi.pbservizit.util.Constants.ENTITA_T_FORNITORE);
				...
				if (fileGiaAssociato) {
					...
					try {
						...
						if (this.idProgettoComeIdTarget(idEntita)) {
							...
							sqlInsertTFileEntita = "INSERT INTO PBANDI_T_FILE_ENTITA (ID_FILE_ENTITA, ID_FILE, ID_ENTITA, ID_TARGET, DT_ASSOCIAZIONE) VALUES ("+idTFileEntita+","+idFile+","+idEntita+","+idProgetto+",SYSDATE)";
						} else {
							// Caso normale.
							sqlInsertTFileEntita = "INSERT INTO PBANDI_T_FILE_ENTITA (ID_FILE_ENTITA, ID_FILE, ID_ENTITA, ID_TARGET, ID_PROGETTO, DT_ASSOCIAZIONE) VALUES ("+idTFileEntita+","+idFile+","+idEntita+","+idTarget+","+idProgetto+",SYSDATE)";
						}
	...
	// Associa più file ad una qualche entità (dich di spesa, pagamento, etc).	
	// Ex pbandiweb ArchivioFileAction.associate() + pbandisrv ArchivioBusinessImpl.associateFile()
	// NOTA: ad oggi questo non è un servizio.
	public EsitoAssociaFilesDTO associaFiles(ArrayList<Long> elencoIdDocumentoIndex, Long idProgetto, Long idEntita, Long idTarget, Long idUtente)
			throws InvalidParameterException, Exception {
		String prf = "[ArchivioFileDAOImpl::associaFiles] ";
		LOG.info(prf + " BEGIN");
		...
		try {			
			...
			BigDecimal idEntitaFornitore = decodificheDAOImpl.idDaDescrizione("PBANDI_C_ENTITA", "ID_ENTITA", "NOME_ENTITA", Constants.ENTITA_T_FORNITORE);
			...
			for (Long idDocIndex : elencoIdDocumentoIndex) {
				...
				if (fileGiaAssociato) {
					...
					try {
						...
						// Aggiorna DtAggiornamentoIndex e IdUtenteAgg nella TDocumentoIndex.
						String sqlUpdateTIndex = "UPDATE PBANDI_T_DOCUMENTO_INDEX SET ID_UTENTE_AGG = "+idUtente+", DT_AGGIORNAMENTO_INDEX = SYSDATE";
						logger.info(prf + "\n"+sqlUpdateTIndex);					
						getJdbcTemplate().update(sqlUpdateTIndex);
