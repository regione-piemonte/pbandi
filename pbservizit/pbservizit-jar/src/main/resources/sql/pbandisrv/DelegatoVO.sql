...
SELECT DISTINCT pf.cognome as cognome     ,
      ...
      i.desc_indirizzo||  nvl2(i.civico,' '||i.civico,'') indirizzo_residenza,
    ...
    WHERE rsp.id_tipo_anagrafica                            = (select id_tipo_anagrafica from PBANDI_D_TIPO_ANAGRAFICA where desc_breve_tipo_anagrafica = 'PERSONA-FISICA')
      ...
      AND rsc.id_tipo_soggetto_correlato                    = (select id_tipo_soggetto_correlato from PBANDI_D_TIPO_SOGG_CORRELATO where upper(PBANDI_D_TIPO_SOGG_CORRELATO.desc_breve_tipo_sogg_correlato) = 'DELF')
