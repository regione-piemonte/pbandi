...
( 
    SELECT codice_fiscale_soggetto,
           ...
           ROW_NUMBER() OVER (PARTITION BY codice_fiscale_BENEFICIARIO ORDER BY codice_fiscale_BENEFICIARIO ASC) AS rn
    FROM (
        ...
        FROM (SELECT rsp.id_soggetto id_soggetto_beneficiario,
              ...
              FROM pbandi_r_soggetto_progetto rsp,
                   (SELECT soggetto.id_soggetto,
                           ...
                           NVL(eg.denominazione_ente_giuridico,pf.denominazione_persona_fisica) AS denominazione_beneficiario
                    FROM (SELECT pbandi_t_soggetto.id_soggetto,
                          ...
                          AND   pbandi_d_tipo_soggetto.id_tipo_soggetto = pbandi_t_soggetto.id_tipo_soggetto) soggetto,
                         (SELECT DISTINCT rsp.id_soggetto,
                          ...
                          AND   rsp.ID_TIPO_ANAGRAFICA = (SELECT dta.id_tipo_anagrafica
                                                          FROM pbandi_d_tipo_anagrafica dta
                                                          WHERE dta.desc_breve_tipo_anagrafica = 'BENEFICIARIO')
                          AND   NVL(rsp.id_tipo_beneficiario,'-1') <>(SELECT dtb.id_tipo_beneficiario
                                                                      FROM pbandi_d_tipo_beneficiario dtb
                                                                      WHERE dtb.desc_breve_tipo_beneficiario = 'BEN-ASSOCIATO')
                          AND   NVL(TRUNC(rsp.dt_fine_validita),TRUNC(SYSDATE +1)) > TRUNC(SYSDATE)) eg,
                         (SELECT DISTINCT rsp.id_soggetto,
                                 FIRST_VALUE(tpf.cognome || ' ' || tpf.nome) OVER (PARTITION BY tpf.id_soggetto ORDER BY tpf.dt_inizio_validita DESC,tpf.id_persona_fisica DESC) denominazione_persona_fisica
                          ...
                          AND   NVL(rsp.id_tipo_beneficiario,'-1') <>(SELECT dtb.id_tipo_beneficiario
                                                                      FROM pbandi_d_tipo_beneficiario dtb
                                                                      WHERE dtb.desc_breve_tipo_beneficiario = 'BEN-ASSOCIATO')
                          AND   NVL(TRUNC(rsp.dt_fine_validita),TRUNC(SYSDATE +1)) > TRUNC(SYSDATE)) pf
              ...
              AND   NVL(TRUNC(rsp.dt_fine_validita),TRUNC(SYSDATE +1)) > TRUNC(SYSDATE)) ben,
             (SELECT DISTINCT tp.id_progetto,
              ...
              FROM (SELECT ts1.*,
                    ...
                    AND   rsta.id_tipo_anagrafica NOT IN (SELECT dta.id_tipo_anagrafica
                                                          FROM pbandi_d_tipo_anagrafica dta
                                                          WHERE dta.desc_breve_tipo_anagrafica IN ('PERSONA-FISICA','OI-ISTRUTTORE','ADG-ISTRUTTORE', 'ISTR-AFFIDAMENTI'))
                    AND   (rsta.id_tipo_anagrafica NOT IN (SELECT dta.id_tipo_anagrafica
                                                           FROM pbandi_d_tipo_anagrafica dta
                                                           WHERE dta.desc_breve_tipo_anagrafica IN ('BEN-MASTER','OI-IST-MASTER','ADG-IST-MASTER')) OR (NOT EXISTS (SELECT 'x'
                                                                                                                                                                    FROM pbandi_r_ente_competenza_sogg recs
                                                                                                                                                                    WHERE recs.id_soggetto = ts1.id_soggetto) OR EXISTS (SELECT 'x'
                                                                                                                                                                                                                         FROM pbandi_r_ente_competenza_sogg recs
                                                                                                                                                                                                                         WHERE recs.id_soggetto = ts1.id_soggetto
                                                                                                                                                                                                                         AND   recs.id_ente_competenza = tec.id_ente_competenza)))) ts,
              ...
              AND   rble.id_ruolo_ente_competenza = (SELECT dre.id_ruolo_ente_competenza
             ...
             SELECT   p.id_progetto,
              ...
              WHERE   ts.id_soggetto = sbli.id_soggetto
                      ...
                      AND rbli.progr_bando_linea_intervento =
                            sbli.progr_bando_linea_intervento
                      AND p.id_domanda = d.id_domanda) m,
             pbandi_d_tipo_anagrafica dta
        WHERE m.id_tipo_anagrafica = dta.id_tipo_anagrafica
        AND   ben.id_progetto = m.id_progetto
        AND   (dta.desc_breve_tipo_anagrafica <> 'PERSONA-FISICA' OR ben.id_soggetto_beneficiario IN (SELECT rsc.id_soggetto_ente_giuridico
                                                                                                      ...
                                                                                                      AND   rsc.id_tipo_soggetto_correlato IN (SELECT rrta.id_tipo_soggetto_correlato
                                                                                                                                               FROM pbandi_r_ruolo_tipo_anagrafica rrta
                                                                                                                                               WHERE rrta.id_tipo_anagrafica = m.id_tipo_anagrafica)))
        AND   id_soggetto = ?
        AND   desc_breve_tipo_anagrafica = ?
    )
)
SELECT *
FROM tab
WHERE rn=1 AND  ROWNUM <= 50 AND (lower(denominazione_beneficiario) LIKE lower(?) OR lower(codice_fiscale_beneficiario) LIKE lower(?))
ORDER BY denominazione_beneficiario , codice_fiscale_beneficiario