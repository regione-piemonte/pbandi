...
@Component({
  selector: 'app-storico-blocchi-domanda-dialog',
  templateUrl: './storico-blocchi-domanda-dialog.component.html',
  styleUrls: ['./storico-blocchi-domanda-dialog.component.scss']
})
export class StoricoBlocchiDomandaDialogComponent implements OnInit {
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<StoricoBlocchiDomandaDialogComponent>,
  ) { }
  ...
  getStoricoBlocchi() {
    this.subscribers.getStoricoBlocchi = this.anagBeneService.getStoricoBlocchiDomanda(this.idSoggetto, this.numeroDomanda).subscribe(data => {
      if (data.length > 0) {
