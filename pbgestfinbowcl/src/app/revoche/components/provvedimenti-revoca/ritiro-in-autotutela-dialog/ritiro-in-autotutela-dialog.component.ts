...
@Component({
  selector: 'app-ritiro-in-autotutela-dialog',
  templateUrl: './ritiro-in-autotutela-dialog.component.html',
  styleUrls: ['./ritiro-in-autotutela-dialog.component.scss']
})
export class RitiroInAutotutelaDialogComponent implements OnInit {
  ...
  constructor(
    public dialogRef: MatDialogRef<RitiroInAutotutelaDialogComponent>,
  ...
  getDocumenti() {
    this.provvedimentiRevocaResponseService.getDocumentiProvvedimentoRevoca(this.data.numeroGestioneRevoca == undefined ? "" : this.data.numeroGestioneRevoca).subscribe(data => {
      this.letteraAccompagnatoria = data.find(element => element.idTipoDocumento == '48');
      this.altriAllegati = data.filter(x => x.idTipoDocumento == '54');
  ...
  salvaAltriAllegati() {
    if (this.newAltriAllegati.length > 0) {
      for (let i = 0; i < this.newAltriAllegati.length; i++) {
        this.provvedimentiRevocaResponseService.aggiungiAllegatoProvvedimento(this.newAltriAllegati[i], "altro", "ritiro", this.data.numeroGestioneRevoca).subscribe(data => {
  ...
  ritiroInAutotutelaCall() {
    if (this.erroreAggiuntaAllegati == false) {
      this.provvedimentiRevocaResponseService.ritiroInAutotutelaRevoca(this.data?.numeroGestioneRevoca).subscribe(data => {
  ...
  salvaDocumenti() {
    ...
    if(this.newLetteraAccompagnatoria) {
      ...
      this.provvedimentiRevocaResponseService.aggiungiAllegatoProvvedimento(this.newLetteraAccompagnatoria, "letteraAccompagnatoria", "ritiro", this.data.numeroGestioneRevoca).subscribe(data => {
