...
@Component({
  selector: 'app-gestione-checklist',
  templateUrl: './gestione-checklist.component.html',
  styleUrls: ['./gestione-checklist.component.scss']
})
@AutoUnsubscribe({ objectName: 'subscribers' })
export class GestioneChecklistComponent implements OnInit {
  i: number;
  idProposta: number;
  idDettPropostaCertif: number;
    ...
    this.subscribers.router = this.activatedRoute.params.subscribe(params => {
      ...
      this.subscribers.userInfo$ = this.userService.userInfo$.subscribe(data => {
        if (data) {
          ...
          if (this.activatedRoute.snapshot.paramMap.get('action') != null) {
            this.idDettPropostaCertif = +this.activatedRoute.snapshot.paramMap.get('action');

            this.subscribers.propostaProgetto = this.certificazioneService.getDettaglioProposta(this.idDettPropostaCertif).subscribe(data => {
              if (data) {
                this.proposta = data;
                this.stati = new Array<string>();
                this.stati.push(Constants.DESC_BREVE_TIPO_DOC_CHECKLIST_CERTIF_PROGETTO);
                this.getAllegati(this.stati);
            ...
            this.subscribers.proposta = this.certificazioneService.getProposta(this.idProposta).subscribe(data => {
              if (data) {
                ...
                this.stati = new Array<string>();
                this.stati.push(Constants.DESC_BREVE_TIPO_DOC_CHECKLIST_CERTIF_PROPOSTA);
                this.getAllegati(this.stati);
  ...
  aggiungiAllegato() {
    ...
    this.subscribers.allega = this.certificazioneService.allegaFileProposta(this.idProposta, this.proposta.idProgetto, this.file,
      this.idDettPropostaCertif != null ? Constants.DESC_BREVE_TIPO_DOC_CHECKLIST_CERTIF_PROGETTO : Constants.DESC_BREVE_TIPO_DOC_CHECKLIST_CERTIF_PROPOSTA).subscribe(data => {
        if (data) {
          if (data.esito) {
  ...
  isLoading() {
    if (!this.loadedProposta || !this.loadedAggiungi || !this.loadedAllegati || !this.loadedElimina || !this.loadedModifica) {
      return true;
    }
    return false;
  }
