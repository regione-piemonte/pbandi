/*******************************************************************************
* Copyright Regione Piemonte - 2024
* SPDX-License-Identifier: EUPL-1.2
******************************************************************************/

package it.csi.pbandi.pbworkspace.dto;

public class EsitoAvviaProgettiDTO implements java.io.Serializable {

	static final long serialVersionUID = 1;

	//private java.lang.Boolean esito = null;
	//private java.lang.Long id = null;
	private java.util.ArrayList<String> messaggi = new java.util.ArrayList<String>();
	private java.util.ArrayList<String> errori = new java.util.ArrayList<String>();
	
	public java.util.ArrayList<String> getMessaggi() {
		return messaggi;
	}
	public void setMessaggi(java.util.ArrayList<String> messaggi) {
		this.messaggi = messaggi;
	}
	
	public java.util.ArrayList<String> getErrori() {
		return errori;
	}
	public void setErrori(java.util.ArrayList<String> errori) {
		this.errori = errori;
	}
	/*
	public java.lang.Boolean getEsito() {
		return esito;
	}
	public void setEsito(java.lang.Boolean esito) {
		this.esito = esito;
	}
	public java.lang.Long getId() {
		return id;
	}
	public void setId(java.lang.Long id) {
		this.id = id;
	}
	*/
	
	@Override
	public String toString() {
		StringBuffer sb = new StringBuffer();
		sb.append("\nEsitoOperazioneDTO: ");
		try {
			//sb.append("\n esito = "+esito);
			//sb.append("\n id = "+id);
			if (messaggi == null) {
				sb.append("\n num messaggi = 0");
			} else {
				sb.append("\n num messaggi = "+messaggi.size());
				for (String s : messaggi)
					sb.append("\n msg = "+s);
			}
			if (errori == null) {
				sb.append("\n num errori = 0");
			} else {
				sb.append("\n num errori = "+errori.size());
				for (String s : errori)
					sb.append("\n msg = "+s);
			}
		} catch (Exception e) {
		}
		return sb.toString();
	}

}
