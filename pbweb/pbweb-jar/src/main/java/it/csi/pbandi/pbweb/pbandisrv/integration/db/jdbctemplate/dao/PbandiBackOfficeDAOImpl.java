...
public class PbandiBackOfficeDAOImpl extends PbandiDAO {
	...
	public void setSeqpbandiRSoggettoProgetto(
			AbstractDataFieldMaxValueIncrementer seqpbandiRSoggettoProgetto) {
	...
	public boolean eliminaModelloAssociato(Long progrBandoLineaIntervento, Long idTipoDocumentoIndex) {
		...
		try {
			...
			StringBuilder sqlDelete = new StringBuilder(
					"DELETE FROM PBANDI_R_BL_TIPO_DOC_MICRO_SEZ "+
					"WHERE PROGR_BL_TIPO_DOC_SEZ_MOD IN "+
			...
			sqlDelete = new StringBuilder(
					"DELETE FROM PBANDI_R_BL_TIPO_DOC_SEZ_MOD "+
					"WHERE ID_TIPO_DOCUMENTO_INDEX = :idTipoDocumentoIndex "+
					" AND PROGR_BANDO_LINEA_INTERVENTO = :progrBandoLineaIntervento ");
			elimina(sqlDelete.toString(), params);
	...
	public List<PbandiCTipoDocumentoIndexVO> findModelliAssociati(
		...
		try {
			...
			StringBuilder sqlSelect = new StringBuilder(
				...
				"and t.progr_bando_linea_intervento= :progrBandoLineaIntervento "+
				"order by 1,2 ");
	...
	public CheckListBandoLineaDTO[] findCheckListAssociabili() {
		...
		try {
			...
			MapSqlParameterSource params = new MapSqlParameterSource();
			List<CheckListBandoLineaDTO> result = query(sqlSelect.toString(), params, new CheckListBandoLineaDTORowMapper());
			...
			// Popolo il campo idRecord: serve come id univoco nella tabella che visualizzar� i record.
			Long i = new Long(1);
	...
	public CheckListBandoLineaDTO[] findCheckListAssociate(Long progrBandoLineaIntervento) {
		...
		try {
			...
			StringBuilder sqlSelect = new StringBuilder(
				"select tp.id_tipo_documento_index as idTipoDocumentoIndex, "+
				"mo.id_modello as idModello, mo.nome_modello as nomeModello, "+
				"mo.versione_modello as versioneModello, "+
				"tp.desc_breve_tipo_doc_index as descBreveTipoDocIndex, "+
				"tp.desc_tipo_doc_index as descTipoDocIndex "+
	...
	public DocPagamBandoLineaDTO[] findDocPagamAssociati(			
		...
		try {
			...
			List<DocPagamBandoLineaDTO> result = query(sqlSelect.toString(), params, new DocPagamBandoLineaDTORowMapper());
	...
	public boolean inserisciTipoDocumentoSpesa(			
	...
	public boolean inserisciModalitaPagamento(			
			String descrizione, String descrizioneBreve)
			throws CSIException, SystemException, UnrecoverableException,
			GestioneBackOfficeException {
		getLogger().begin();
		try {
			...
			StringBuilder sqlSelect = new StringBuilder(
				"insert into PBANDI_D_MODALITA_PAGAMENTO "+
	...
	public boolean eliminaDocPagamAssociato(Long progrEccezBanLinDocPag) {
		...
		try {
			...
			MapSqlParameterSource params = new MapSqlParameterSource();
			StringBuilder sqlDelete = new StringBuilder(
				"delete from PBANDI_R_ECCEZ_BAN_LIN_DOC_PAG "+
	...
	public boolean eliminaDocPagamAssociatoATuttiBL(Long idTipoDocumento, Long idModalitaPagamento) {
		...
		try {
			...
			MapSqlParameterSource params = new MapSqlParameterSource();
			StringBuilder sqlDelete = new StringBuilder(
				"delete from PBANDI_R_TIPO_DOC_MODALITA_PAG "+
				"where ID_TIPO_DOCUMENTO_SPESA = :idTipoDocumento "+
	...
	public List<IstruttoreVO> findIstruttoriSempliciForIstruttoreMaster(
			Long idSoggettoIstruttoreMaster, IstruttoreDTO filtro) {
		getLogger().begin();
		try {
			List<IstruttoreVO> result = new ArrayList<IstruttoreVO>();
			MapSqlParameterSource params = new MapSqlParameterSource();
			/*
			 * Costruzione della query per la selezione dell' ente di competenza
			 * dell' istruttore MASTER
			 */
			StringBuilder sqlSelectEnte = new StringBuilder(
					"SELECT  PBANDI_R_ENTE_COMPETENZA_SOGG.id_ente_competenza");
			StringBuilder tablesEnte = new StringBuilder(
					"PBANDI_R_ENTE_COMPETENZA_SOGG");
			sqlSelectEnte.append(" FROM ").append(tablesEnte);

			List<StringBuilder> conditionListEnte = new ArrayList<StringBuilder>();
			conditionListEnte
					.add(new StringBuilder(
							"PBANDI_R_ENTE_COMPETENZA_SOGG.id_soggetto = :idSoggettoMaster"));
			...
			StringBuilder sqlSelectIstruttore = new StringBuilder(
					...
					.append("PBANDI_T_PERSONA_FISICA.nome as nome,").append(
							"PBANDI_T_SOGGETTO.id_soggetto as idSoggetto");

			StringBuilder tablesIstruttore = new StringBuilder(
					"PBANDI_R_ENTE_COMPETENZA_SOGG,").append(
					"PBANDI_T_SOGGETTO").append(",PBANDI_T_PERSONA_FISICA");

			List<StringBuilder> conditionListIstruttore = new ArrayList<StringBuilder>();
			conditionListIstruttore
					.add(new StringBuilder(
							"PBANDI_T_SOGGETTO.id_soggetto = PBANDI_R_ENTE_COMPETENZA_SOGG.id_soggetto"));
			conditionListIstruttore
					.add(new StringBuilder(
							"PBANDI_T_PERSONA_FISICA.id_soggetto = PBANDI_T_SOGGETTO.id_soggetto"));

			conditionListIstruttore.add(new StringBuilder(
					"PBANDI_R_ENTE_COMPETENZA_SOGG.id_ente_competenza IN ("
							+ sqlSelectEnte + " )"));
			/*
			 * Gli istruttori semplici non hanno piu' il tipo di anagrafica
			 * valorizzata su PBANDI_T_SOGGETTO
			 */

			if (filtro.getIdBando() != null) {
				tablesIstruttore.append(",PBANDI_T_PROGETTO").append(
				...
				conditionListIstruttore
						.add(new StringBuilder(
								" PBANDI_T_DOMANDA.id_domanda = PBANDI_T_PROGETTO.id_domanda"));
				conditionListIstruttore
						.add(new StringBuilder(
				...
				conditionListIstruttore.add(new StringBuilder(
						" PBANDI_R_BANDO_LINEA_INTERVENT.id_bando = :idBando"));
			...
			if (filtro.getIdProgetto() != null) {
				...
				params.addValue("idProgetto", filtro.getIdProgetto(),
						Types.NUMERIC);
			}
			...
			if (filtro.getCognome() != null
					&& filtro.getCognome().trim().length() > 0) {
				conditionListIstruttore.add(new StringBuilder(
						" UPPER(PBANDI_T_PERSONA_FISICA.cognome) LIKE '%"
								+ filtro.getCognome().toUpperCase() + "%'"));
			}
			if (filtro.getNome() != null
					&& filtro.getNome().trim().length() > 0) {
				conditionListIstruttore.add(new StringBuilder(
						" UPPER(PBANDI_T_PERSONA_FISICA.nome) LIKE '%"
								+ filtro.getNome().toUpperCase() + "%'"));
			}
			/*
			 * FIX Pbandi 625: ricerca parziale per codice fiscale
			 */
			if (filtro.getCodiceFiscale() != null
					&& filtro.getCodiceFiscale().trim().length() > 0) {
				conditionListIstruttore.add(new StringBuilder(
						" UPPER(PBANDI_T_SOGGETTO.codice_fiscale_soggetto) LIKE '%"
			...
			if (filtro.getIdProgetto() != null || filtro.getIdBando() != null) {
				...
				conditionListIstruttore
						.add(new StringBuilder(
								"PBANDI_R_SOGGETTO_PROGETTO.id_soggetto = PBANDI_T_SOGGETTO.id_soggetto"));
			}

			/*
			 * JIRA-1097
			 */
			tablesIstruttore.append(",PBANDI_R_SOGG_TIPO_ANAGRAFICA");
			conditionListIstruttore
					.add(new StringBuilder(
							"PBANDI_R_SOGG_TIPO_ANAGRAFICA.id_tipo_anagrafica in (select id_tipo_anagrafica from PBANDI_D_TIPO_ANAGRAFICA where desc_breve_tipo_anagrafica IN ('OI-ISTRUTTORE','ADG-ISTRUTTORE'))"));
			conditionListIstruttore
					.add(new StringBuilder(
							"PBANDI_T_SOGGETTO.id_soggetto = PBANDI_R_SOGG_TIPO_ANAGRAFICA.id_soggetto"));

			sqlSelectIstruttore.append(" FROM ").append(tablesIstruttore);
			setWhereClause(conditionListIstruttore, sqlSelectIstruttore,
					tablesIstruttore);

			params.addValue("idSoggettoMaster", idSoggettoIstruttoreMaster,
					Types.NUMERIC);

			sqlSelectIstruttore.append(
					" GROUP BY (PBANDI_T_SOGGETTO.codice_fiscale_soggetto,")
					.append("PBANDI_T_PERSONA_FISICA.cognome,").append(
							"PBANDI_T_PERSONA_FISICA.nome,").append(
							"PBANDI_T_SOGGETTO.id_soggetto )").append(
							" ORDER BY PBANDI_T_PERSONA_FISICA.cognome,")
					.append("PBANDI_T_PERSONA_FISICA.nome");

			/*
			 * Costruzione della quey complessiva
			 */
			StringBuilder sqlSelect = new StringBuilder(
					"SELECT codiceFiscale as codiceFiscale,").append(
					"cognome as cognome,").append("nome as nome,").append(
					"idSoggetto as idSoggetto");
	...
	public List<ProgettoVO> findProgettiAssociatiIstruttore(
			Long idSoggettoIstruttore, boolean checkDataFineValidita) {
		getLogger().begin();
		try {
			...
			StringBuilder sqlSelect = new StringBuilder(
					...
					.append(
							"PBANDI_R_SOGGETTO_PROGETTO.PROGR_SOGGETTO_PROGETTO AS progrSoggettoProgetto,")
					.append(
							"PBANDI_T_PROGETTO.codice_visualizzato AS codiceVisualizzato");
			StringBuilder tables = new StringBuilder(
					"PBANDI_R_SOGGETTO_PROGETTO,").append(
					"PBANDI_D_TIPO_ANAGRAFICA,").append("PBANDI_T_PROGETTO");
			...
			conditionList
					.add(new StringBuilder(
							"PBANDI_R_SOGGETTO_PROGETTO.id_soggetto = :idSoggettoIstruttore"));
			...
			MapSqlParameterSource params = new MapSqlParameterSource();
			params.addValue("idSoggettoIstruttore", idSoggettoIstruttore,
					Types.NUMERIC);

			result = query(sqlSelect.toString(), params,
					new ProgettoVORowMapper());
			return result;
	...
	public List<ProgettoVO> verificaProgettiAssociatiIstruttore(
			Long idSoggettoIstruttore, List<String> idProgetti,
		...
		try {
			...
			StringBuilder sqlSelect = new StringBuilder(
					...
					.append(
							",PBANDI_R_SOGGETTO_PROGETTO.PROGR_SOGGETTO_PROGETTO AS progrSoggettoProgetto")
					.append(
							",PBANDI_T_PROGETTO.ID_ISTANZA_PROCESSO as idIstanzaProcesso")
					.append(
							",PBANDI_T_PROGETTO.codice_visualizzato AS codiceVisualizzato");
			StringBuilder tables = new StringBuilder(
					"PBANDI_R_SOGGETTO_PROGETTO,").append(
			...
			if (idProgetti != null && idProgetti.size() > 0) {
				...
				progetti = temp.toString().substring(0,
			...
			conditionList.add(new StringBuilder(
					"PBANDI_R_SOGGETTO_PROGETTO.id_progetto IN (" + progetti
							+ ")"));
			if (checkDataFineValidita)
				setWhereClause(conditionList, sqlSelect, tables);
			else
	...
	public List<ProgettoVO> findDettaglioProgettiAssociatiIstruttore(
			Long idSoggettoIstruttore) {
		getLogger().begin();
		try {
			List<ProgettoVO> result = new ArrayList<ProgettoVO>();

			/**
			 * Costruzione della query per la ricerca degli progetti associati
			 * all' istruttore
			 */
			...
			StringBuilder tablesProgettiIstruttore = new StringBuilder(
					"PBANDI_R_SOGGETTO_PROGETTO,")
					.append("PBANDI_D_TIPO_ANAGRAFICA");
			sqlSelectProgettiIstruttore.append(" FROM ").append(
					tablesProgettiIstruttore);
			...
			conditionListIstruttori
					.add(new StringBuilder(
							"PBANDI_R_SOGGETTO_PROGETTO.id_soggetto = :idSoggettoIstruttore"));
			setWhereClause(conditionListIstruttori,
					sqlSelectProgettiIstruttore, tablesProgettiIstruttore);
			...
			StringBuilder sqlCase = new StringBuilder(" CASE ")
					...
					.append(
							"    THEN ( SELECT PBANDI_T_ENTE_GIURIDICO.denominazione_ente_giuridico")
			 ...
			 */
			StringBuilder sqlSelect = new StringBuilder(
					"SELECT PBANDI_T_PROGETTO.id_progetto as idProgetto,")
					.append(
							"PBANDI_T_PROGETTO.codice_visualizzato as codiceVisualizzato,")
					.append("PBANDI_T_BANDO.titolo_bando as titoloBando,")
					.append(
							"PROGETTI_ISTRUTTORE.progrSoggettoProgetto as progrSoggettoProgetto,")
			...
			StringBuilder tables = new StringBuilder(
					...
					"PBANDI_D_TIPO_BENEFICIARIO,").append("PBANDI_T_PROGETTO,")
					.append("PBANDI_T_DOMANDA,").append(
							"PBANDI_R_BANDO_LINEA_INTERVENT,").append(
							"PBANDI_T_BANDO,").append(
							"(" + sqlSelectProgettiIstruttore
									+ ") PROGETTI_ISTRUTTORE");
			sqlSelect.append(" FROM ").append(tables);
			List<StringBuilder> conditionList = new ArrayList<StringBuilder>();
			conditionList
			...
			conditionList
					.add(new StringBuilder(
							"PBANDI_D_TIPO_BENEFICIARIO.id_tipo_beneficiario <> 4"));
			conditionList
					.add(new StringBuilder(
			...
			conditionList
					.add(new StringBuilder(
							"PBANDI_R_SOGGETTO_PROGETTO.id_progetto = PROGETTI_ISTRUTTORE.idProgetto"));
	...
	public List<BandoVO> findBandi() {
		...
		try {
			...
			StringBuilder sqlSelect = new StringBuilder(
					"SELECT PBANDI_R_BANDO_LINEA_INTERVENT.ID_BANDO as idBando,")
					.append("PBANDI_R_BANDO_LINEA_INTERVENT.nome_bando_linea as titoloBando");

			StringBuilder tables = new StringBuilder(
					"PBANDI_R_BANDO_LINEA_INTERVENT, PBANDI_T_BANDO");
			sqlSelect.append(" FROM ").append(tables);
			List<StringBuilder> conditionList = new ArrayList<StringBuilder>();
			conditionList
					.add(new StringBuilder(
	...
	public List<ProgettoVO> findProgettiByBando(Long idBando) {
		getLogger().begin();
		try {
			MapSqlParameterSource params = new MapSqlParameterSource();
			List<ProgettoVO> result = new ArrayList<ProgettoVO>();
			...
			StringBuilder tables = new StringBuilder(
					"PBANDI_R_BANDO_LINEA_INTERVENT,").append(
					"PBANDI_T_DOMANDA,").append("PBANDI_T_PROGETTO");
			sqlSelect.append(" FROM ").append(tables);
			List<StringBuilder> conditionList = new ArrayList<StringBuilder>();
			conditionList
					.add(new StringBuilder(
	...
	public List<BeneficiarioVO> findBeneficiariProgetto(ProgettoDTO filtro) {
		getLogger().begin();
		try {
			...
			StringBuilder sqlCase = new StringBuilder(" CASE ")
					.append(
							"WHEN PBANDI_R_SOGGETTO_PROGETTO.id_persona_fisica is null ")
					.append(
							"   THEN ( SELECT DISTINCT PBANDI_T_ENTE_GIURIDICO.denominazione_ente_giuridico")
					.append("          FROM PBANDI_T_ENTE_GIURIDICO")
					.append(
							"          WHERE PBANDI_T_ENTE_GIURIDICO.id_ente_giuridico = PBANDI_R_SOGGETTO_PROGETTO.id_ente_giuridico")
					.append("         )")
					.append(
							"    ELSE ( SELECT DISTINCT PBANDI_T_PERSONA_FISICA.cognome || ' ' || PBANDI_T_PERSONA_FISICA.nome")
					.append("           FROM PBANDI_T_PERSONA_FISICA")
					.append(
							"           WHERE PBANDI_T_PERSONA_FISICA.id_persona_fisica = PBANDI_R_SOGGETTO_PROGETTO.id_persona_fisica")
					.append("          )").append("END as beneficiario");
			...
			conditionList
					.add(new StringBuilder(
							"PBANDI_T_PROGETTO.id_progetto = PBANDI_R_SOGGETTO_PROGETTO.id_progetto"));
			if (filtro.getIdBando() != null) {
				tables.append(",PBANDI_T_DOMANDA,").append(
						"PBANDI_R_BANDO_LINEA_INTERVENT,").append(
						" PBANDI_T_BANDO");
			...
			if (filtro.getIdSoggettoBeneficiario() != null) {
				conditionList
						.add(new StringBuilder(
								"PBANDI_R_SOGGETTO_PROGETTO.id_soggetto = :idSoggettoBeneficiario"));
				params.addValue("idSoggettoBeneficiario", filtro
						.getIdSoggettoBeneficiario(), Types.NUMERIC);
			}
			sqlSelect.append(" FROM ").append(tables);
			setWhereClause(conditionList, sqlSelect, tables);
			sqlSelect.append(" ORDER BY 2");
			result = query(sqlSelect.toString(), params,
					new BeneficiarioVORowMapper());
			return result;
	...
	public List<ProgettoVO> findProgettiByBeneficiarioBando(
			Long idSoggettoBeneficiario, Long idBando) {
		getLogger().begin();
		try {
			List<ProgettoVO> result = new ArrayList<ProgettoVO>();
			MapSqlParameterSource params = new MapSqlParameterSource();
			...
			StringBuilder tables = new StringBuilder(
					...
					.append("PBANDI_T_DOMANDA,")
					.append(
							"PBANDI_T_PROGETTO, PBANDI_R_SOGGETTO_PROGETTO, PBANDI_D_TIPO_ANAGRAFICA, PBANDI_D_TIPO_BENEFICIARIO ");
			sqlSelect.append(" FROM ").append(tables);
			List<StringBuilder> conditionList = new ArrayList<StringBuilder>();
			conditionList
					.add(new StringBuilder(
	...
	public List<ProgettoVO> findProgettiDaAssociare(Long idBando,
			Long idProgetto, boolean isIstruttoriAssociati) {
		getLogger().begin();
		try {
			List<ProgettoVO> result = new ArrayList<ProgettoVO>();
			// /**
			...
			// "  WHERE PBANDI_R_SOGGETTO_PROGETTO.id_tipo_anagrafica = PBANDI_D_TIPO_ANAGRAFICA.id_tipo_anagrafica"
			// +
			// "    AND PBANDI_D_TIPO_ANAGRAFICA.desc_breve_tipo_anagrafica  IN ('OI-ISTRUTTORE','ADG-ISTRUTTORE')"
			// +
			// " ) "));
			// conditionListProgettiNonAssociati.add(new
			...
			// conditionListProgettiAssociati.add(new
			// StringBuilder(" PBANDI_D_TIPO_ANAGRAFICA.desc_breve_tipo_anagrafica  IN ('OI-ISTRUTTORE','ADG-ISTRUTTORE')  "));
			//			
			...
			//			
			// sqlSelectProgettiAssociati.append(" GROUP BY PBANDI_T_PROGETTO.id_progetto, PBANDI_T_PROGETTO.CODICE_VISUALIZZATO ");
			//			
			// /**
			...
			// conditionListBeneficiari.add(new
			// StringBuilder(" PBANDI_D_TIPO_BENEFICIARIO.desc_breve_tipo_beneficiario IN ('BEN-CAPOFILA','BEN-SINGOLO','BEN-PARTNER','BEN-PROPONENTE') "));
			...
			// conditionList.add(new
			// StringBuilder(" PBANDI_R_BANDO_LINEA_INTERVENT.id_bando =PBANDI_T_BANDO.id_bando"));
			// conditionList.add(new
			...
			sqlSelectProgettiAssociati.append(" FROM ").append(
					tablesProgettiAssociati);
			List<StringBuilder> conditionListProgettiAssociati = new ArrayList<StringBuilder>();
			conditionListProgettiAssociati
					.add(new StringBuilder(
							"PBANDI_R_SOGGETTO_PROGETTO.id_tipo_anagrafica = PBANDI_D_TIPO_ANAGRAFICA.id_tipo_anagrafica"));
			conditionListProgettiAssociati
					.add(new StringBuilder(
							"PBANDI_D_TIPO_ANAGRAFICA.desc_breve_tipo_anagrafica IN ('OI-ISTRUTTORE','ADG-ISTRUTTORE')"));
			setWhereClause(conditionListProgettiAssociati,
					sqlSelectProgettiAssociati, tablesProgettiAssociati);

			/*
			 * Query per i progetti associati
			 */
			StringBuilder sqlSelectProgettiNonAssociati = new StringBuilder(
					"SELECT PBANDI_T_PROGETTO.id_progetto");
			StringBuilder tablesProgettiNonAssociati = new StringBuilder(
					"PBANDI_T_PROGETTO");
			sqlSelectProgettiNonAssociati.append(" FROM ").append(
					tablesProgettiNonAssociati);
			List<StringBuilder> conditionListProgettiNonAssociati = new ArrayList<StringBuilder>();
			conditionListProgettiNonAssociati.add(new StringBuilder(
					"PBANDI_T_PROGETTO.id_progetto NOT IN ("
							+ sqlSelectProgettiAssociati + ")"));
			setWhereClause(conditionListProgettiNonAssociati,
					sqlSelectProgettiNonAssociati, tablesProgettiNonAssociati);
			...
			if (isIstruttoriAssociati)
				conditionList.add(new StringBuilder(
						"PBANDI_T_PROGETTO.id_progetto IN ("
								+ sqlSelectProgettiAssociati + ")"));
			else
				conditionList.add(new StringBuilder(
						"PBANDI_T_PROGETTO.id_progetto IN ("
								+ sqlSelectProgettiNonAssociati + ")"));

			if (idBando != null) {
				sqlSelect
						.append(",PBANDI_R_BANDO_LINEA_INTERVENT.nome_bando_linea as titoloBando ");
				tables.append(",PBANDI_R_BANDO_LINEA_INTERVENT").append(
						",PBANDI_T_DOMANDA ").append(",PBANDI_T_BANDO");
				conditionList
						.add(new StringBuilder(
								"PBANDI_R_BANDO_LINEA_INTERVENT.progr_bando_linea_intervento = PBANDI_T_DOMANDA.progr_bando_linea_intervento"));
				conditionList
	...
	public List<IstruttoreVO> findIstruttoriAssociatiAProgetto(
			Long idSoggettoMaster, Long idBando, Long idProgetto) {
		getLogger().begin();
		try {
			List<IstruttoreVO> result = new ArrayList<IstruttoreVO>();
			MapSqlParameterSource params = new MapSqlParameterSource();
			params
					.addValue("idSoggettoMaster", idSoggettoMaster,
							Types.NUMERIC);
			/**
			 * Costruzione della query per la selezione dell' ente di competenza
			 * dell' istruttore MASTER
			...
			StringBuilder sqlSelect = new StringBuilder(
					...
					.append(
							",PBANDI_R_SOGGETTO_PROGETTO.id_soggetto as idSoggetto ")
					.append(",PBANDI_T_PERSONA_FISICA.cognome as cognome")
			...
			StringBuilder tables = new StringBuilder(
					" PBANDI_R_SOGGETTO_PROGETTO ").append(
					",PBANDI_T_PROGETTO ").append(",PBANDI_D_TIPO_ANAGRAFICA ")
					.append(",PBANDI_R_BANDO_LINEA_INTERVENT ").append(
							",PBANDI_T_DOMANDA ").append(
							",PBANDI_T_PERSONA_FISICA ").append(
							",PBANDI_T_BANDO").append(
							",PBANDI_R_ENTE_COMPETENZA_SOGG").append(
							",PBANDI_T_SOGGETTO");
	...
	public List<IstruttoreVO> findDettaglioIstruttore(Long idSoggettoIstruttore) {
		...
		try {
			...
			MapSqlParameterSource params = new MapSqlParameterSource();
			StringBuilder sqlSelect = new StringBuilder(
					"SELECT DISTINCT PBANDI_T_PERSONA_FISICA.cognome as cognome,")
					.append("PBANDI_T_PERSONA_FISICA.nome as nome,")
					.append(
							"PBANDI_T_SOGGETTO.codice_fiscale_soggetto as codiceFiscale,")
					.append(
							"PBANDI_T_PERSONA_FISICA.id_persona_fisica as idPersonaFisica,")
					.append("PBANDI_T_SOGGETTO.id_soggetto as idSoggetto");

			StringBuilder tables = new StringBuilder("PBANDI_T_SOGGETTO,")
			...
			conditionList
					.add(new StringBuilder(
							"PBANDI_T_SOGGETTO.id_soggetto = PBANDI_T_PERSONA_FISICA.id_soggetto"));
			conditionList.add(new StringBuilder(
					" PBANDI_T_SOGGETTO.id_soggetto = :idSoggettoIstruttore"));
			params.addValue("idSoggettoIstruttore", idSoggettoIstruttore,
					Types.NUMERIC);
	...
	public ProgettoVO findProgettoByProgressivoSoggettoProgetto(
			Long progrSoggettoProgetto, boolean checkDataFine) {
		getLogger().begin();
		try {
			...
			StringBuilder sqlSelect = new StringBuilder(
					...
					.append(
							",PBANDI_T_PROGETTO.CODICE_VISUALIZZATO as codiceVisualizzato");
			StringBuilder tables = new StringBuilder(
					"PBANDI_R_SOGGETTO_PROGETTO, PBANDI_T_PROGETTO");
			...
			conditionList
					.add(new StringBuilder(
							"PBANDI_R_SOGGETTO_PROGETTO.PROGR_SOGGETTO_PROGETTO = :progrSoggettoProgetto"));
			...
			MapSqlParameterSource params = new MapSqlParameterSource();
			params.addValue("progrSoggettoProgetto", progrSoggettoProgetto,
					Types.NUMERIC);
		...
		try {
			...
			StringBuilder sqlSelect = new StringBuilder(
					...
					.append(
							",PBANDI_T_PROGETTO.ID_ISTANZA_PROCESSO as idIstanzaProcesso");
			StringBuilder tables = new StringBuilder(
					"PBANDI_T_PROGETTO, PBANDI_R_SOGGETTO_PROGETTO");
	...
	public List<ProgettoVO> findProgetti(List<String> idProgetti) {
		...
		try {
			...
			List<StringBuilder> conditionList = new ArrayList<StringBuilder>();
			conditionList.add(new StringBuilder(
					"PBANDI_T_PROGETTO.ID_PROGETTO IN ( " + progetti + " )"));
	...
	public String findRuoloIstruttoreMaster(Long idIstruttoreMaster,
			String codRuolo, boolean checkDataFine) {
		getLogger().begin();
		try {
			List<Map> list = null;
			String result = null;
			MapSqlParameterSource params = new MapSqlParameterSource();
			params.addValue("idIstruttoreMaster", idIstruttoreMaster,
					Types.NUMERIC);
			params.addValue("codRuolo", codRuolo);
			StringBuilder sqlSelect = new StringBuilder(
					"SELECT DISTINCT PBANDI_D_TIPO_ANAGRAFICA.desc_breve_tipo_anagrafica as codice");
			StringBuilder tables = new StringBuilder(
					"PBANDI_R_SOGG_TIPO_ANAGRAFICA, PBANDI_D_TIPO_ANAGRAFICA");
			sqlSelect.append(" FROM ").append(tables);
			List<StringBuilder> conditionList = new ArrayList<StringBuilder>();
			conditionList
					.add(new StringBuilder(
			...
			conditionList
					.add(new StringBuilder(
							"PBANDI_R_SOGG_TIPO_ANAGRAFICA.id_soggetto = :idIstruttoreMaster"));
			conditionList
					.add(new StringBuilder(
							"PBANDI_D_TIPO_ANAGRAFICA.desc_breve_tipo_anagrafica = :codRuolo"));
	...
	public List<BandoVO> findBandiConfigurabili(BandoVO filter) {
		...
		StringBuilder sqlSelect = new StringBuilder("SELECT")
				...
				.append(
						", PBANDI_R_BANDO_LINEA_INTERVENT.PROGR_BANDO_LINEA_INTERVENTO as idBandoLinea")
				.append(", PBANDI_T_BANDO.TITOLO_BANDO as titoloBando")
				.append(", PBANDI_D_MATERIA.DESC_MATERIA as descMateria")
				.append(
						", PBANDI_R_BANDO_LINEA_INTERVENT.NOME_BANDO_LINEA as nomeBandoLinea");

		StringBuilder tables = new StringBuilder("PBANDI_T_BANDO").append(
				", PBANDI_R_BANDO_LINEA_INTERVENT")
		...
		List<StringBuilder> conditionList = new ArrayList<StringBuilder>();
		conditionList.add(new StringBuilder(
				"PBANDI_T_BANDO.ID_MATERIA = PBANDI_D_MATERIA.ID_MATERIA"));
		conditionList
				.add(new StringBuilder(
						"PBANDI_T_BANDO.ID_BANDO = PBANDI_R_BANDO_LINEA_INTERVENT.ID_BANDO(+)"));
		...
		if (filter.getIdMateria() != null) {
			conditionList.add(new StringBuilder(
					"PBANDI_D_MATERIA.ID_MATERIA = :idMateria"));
			params.addValue("idMateria", filter.getIdMateria(), Types.NUMERIC);
		}
		if (filter.getTitoloBando() != null) {
			conditionList.add(new StringBuilder(
					"upper(PBANDI_T_BANDO.TITOLO_BANDO) LIKE :titoloBando"
							+ escapeCommand));
			params.addValue("titoloBando", "%"+StringUtil.escapeWildcards(filter
					.getTitoloBando(), escapeChar)
					+ "%", Types.VARCHAR);
		}
		if (filter.getNomeBandoLinea() != null) {
			conditionList.add(new StringBuilder(
					"upper(PBANDI_R_BANDO_LINEA_INTERVENT.NOME_BANDO_LINEA) LIKE :nomeBandoLinea"
							+ escapeCommand));
			params.addValue("nomeBandoLinea", "%"+StringUtil.escapeWildcards(filter
					.getNomeBandoLinea(), escapeChar)
					+ "%", Types.VARCHAR);
		}
		if (filter.getIdLineaDiIntervento() != null) {
			conditionList.add(new StringBuilder(
					"PBANDI_T_BANDO.ID_LINEA_DI_INTERVENTO = :idLineaDiIntervento"));
			params.addValue("idLineaDiIntervento", filter.getIdLineaDiIntervento(), Types.NUMERIC);
		}

		setWhereClause(conditionList, sqlSelect, tables);

		List<BandoVO> result = query(sqlSelect.toString(), params,
				new BandoVORowMapper());

		return result;
	}

	private class IstruttoreVORowMapper
			implements
			RowMapper<it.csi.pbandi.pbweb.pbandisrv.integration.db.jdbctemplate.vo.gestionebackoffice.IstruttoreVO> {
		public it.csi.pbandi.pbweb.pbandisrv.integration.db.jdbctemplate.vo.gestionebackoffice.IstruttoreVO mapRow(
				ResultSet rs, int row) throws SQLException {

			BeanMapper beanMapper = new BeanMapper();
			it.csi.pbandi.pbweb.pbandisrv.integration.db.jdbctemplate.vo.gestionebackoffice.IstruttoreVO vo = (it.csi.pbandi.pbweb.pbandisrv.integration.db.jdbctemplate.vo.gestionebackoffice.IstruttoreVO) beanMapper
					.toBean(
							rs,
							it.csi.pbandi.pbweb.pbandisrv.integration.db.jdbctemplate.vo.gestionebackoffice.IstruttoreVO.class);
			return vo;
		}
	}

	private class BandoVORowMapper
			implements
			RowMapper<it.csi.pbandi.pbweb.pbandisrv.integration.db.jdbctemplate.vo.gestionebackoffice.BandoVO> {

		public it.csi.pbandi.pbweb.pbandisrv.integration.db.jdbctemplate.vo.gestionebackoffice.BandoVO mapRow(
				ResultSet rs, int row) throws SQLException {

			BeanMapper beanMapper = new BeanMapper();
			it.csi.pbandi.pbweb.pbandisrv.integration.db.jdbctemplate.vo.gestionebackoffice.BandoVO vo = (it.csi.pbandi.pbweb.pbandisrv.integration.db.jdbctemplate.vo.gestionebackoffice.BandoVO) beanMapper
					.toBean(
							rs,
							it.csi.pbandi.pbweb.pbandisrv.integration.db.jdbctemplate.vo.gestionebackoffice.BandoVO.class);
			return vo;
		}
	}
	...
	private class ProgettoVORowMapper
			implements
			RowMapper<it.csi.pbandi.pbweb.pbandisrv.integration.db.jdbctemplate.vo.ProgettoVO> {

		public it.csi.pbandi.pbweb.pbandisrv.integration.db.jdbctemplate.vo.ProgettoVO mapRow(
				ResultSet rs, int row) throws SQLException {

			BeanMapper beanMapper = new BeanMapper();
			it.csi.pbandi.pbweb.pbandisrv.integration.db.jdbctemplate.vo.ProgettoVO vo = (it.csi.pbandi.pbweb.pbandisrv.integration.db.jdbctemplate.vo.ProgettoVO) beanMapper
					.toBean(
							rs,
							it.csi.pbandi.pbweb.pbandisrv.integration.db.jdbctemplate.vo.ProgettoVO.class);
			return vo;
		}
	}

	private class BeneficiarioVORowMapper
			implements
			RowMapper<it.csi.pbandi.pbweb.pbandisrv.integration.db.jdbctemplate.vo.gestionebackoffice.BeneficiarioVO> {

		public it.csi.pbandi.pbweb.pbandisrv.integration.db.jdbctemplate.vo.gestionebackoffice.BeneficiarioVO mapRow(
				ResultSet rs, int row) throws SQLException {

			BeanMapper beanMapper = new BeanMapper();
			it.csi.pbandi.pbweb.pbandisrv.integration.db.jdbctemplate.vo.gestionebackoffice.BeneficiarioVO vo = (it.csi.pbandi.pbweb.pbandisrv.integration.db.jdbctemplate.vo.gestionebackoffice.BeneficiarioVO) beanMapper
					.toBean(
							rs,
							it.csi.pbandi.pbweb.pbandisrv.integration.db.jdbctemplate.vo.gestionebackoffice.BeneficiarioVO.class);
			return vo;
		}
	}
	...
	private class PbandiCTipoDocumentoIndexVORowMapper implements
		RowMapper<PbandiCTipoDocumentoIndexVO> {

		public PbandiCTipoDocumentoIndexVO mapRow(
				ResultSet rs, int row) throws SQLException {

			BeanMapper beanMapper = new BeanMapper();
			PbandiCTipoDocumentoIndexVO vo = (PbandiCTipoDocumentoIndexVO) beanMapper.toBean(
					rs,
	...
	private class CheckListBandoLineaDTORowMapper implements
		RowMapper<CheckListBandoLineaDTO> {
	
		public CheckListBandoLineaDTO mapRow(
				ResultSet rs, int row) throws SQLException {
	
			BeanMapper beanMapper = new BeanMapper();
			CheckListBandoLineaDTO dto = (CheckListBandoLineaDTO) beanMapper.toBean(
					rs,
					CheckListBandoLineaDTO.class);
			return dto;
		}
	}
	
	private class DocPagamBandoLineaDTORowMapper implements
	RowMapper<DocPagamBandoLineaDTO> {

	public DocPagamBandoLineaDTO mapRow(
			ResultSet rs, int row) throws SQLException {

		BeanMapper beanMapper = new BeanMapper();
		DocPagamBandoLineaDTO dto = (DocPagamBandoLineaDTO) beanMapper.toBean(
				rs,
				DocPagamBandoLineaDTO.class);
		return dto;
	}
}
