...
public class LoggerUtil {
	...
	// PK uso questo perche altrimenti i log che passano da questa classe (tipicamente quelli provenienti da pbandisrc es it.csi.pbandi.pbweb.pbandisrv.*)
	// non finiscono con quelli di pbweb
	String className = Constants.COMPONENT_NAME;
	
	// PK : ora scrive 
	// 2022-04-07 16:00:52 [pbweb] <INFO> <pbweb> <[it.csi.pbandi.pbweb.pbandisrv.business.gestionedatigenerali.GestioneDatiGeneraliBusinessImpl::findDatiPiuGreen]
	// sarebbe meglio
	// 2022-04-07 16:00:52 [pbweb] <INFO> <pbweb> <[GestioneDatiGeneraliBusinessImpl::findDatiPiuGreen]
	// TODO sistemare
...
//		String className = traceElement.getClassName();
		...
		if (logger.isDebugEnabled()) {
			...
			logger.debug("[" + traceElement.getClassName() + "::" + methodName + "] " + message);
		}
	}
...
//		String className = traceElement.getClassName();
		...
		logger.info("[" + traceElement.getClassName() + "::" + methodName + "] " + message);
...
//		String className = traceElement.getClassName();
		...
		logger.error("[" + traceElement.getClassName() + "::" + methodName + "] " + message, ex);
...
//		String className = traceElement.getClassName();
		...
		logger.fatal("[" + traceElement.getClassName() + "::" + methodName + "] " + message, ex);
...
//		String className = traceElement.getClassName();
		...
		logger.warn("[" + traceElement.getClassName() + "::" + methodName + "] " + message);
...
//		String className = traceElement.getClassName();
		String methodName = traceElement.getMethodName();
		Logger logger = Logger.getLogger(className);
		logger.debug("[" + traceElement.getClassName() + "::" + methodName + "] BEGIN");
	}
...
//		String className = traceElement.getClassName();
		String methodName = traceElement.getMethodName();
		Logger logger = Logger.getLogger(className);
		logger.debug("[" + traceElement.getClassName() + "::" + methodName + "] END\n");
	}
...
//			logger.error("[" + className + "::" + methodName + "] " + doDump(o));
			logger.error("[" + traceElement.getClassName() + "::" + methodName + "] " + doDump(o));
		} catch (Throwable t) {
...
//			String className = traceElement.getClassName();
			...
			if (logger.isDebugEnabled()) {
				String methodName = traceElement.getMethodName();
//				logger.debug("[" + className + "::" + methodName + "] "	+ doDump(o));
				logger.debug("[" + traceElement.getClassName() + "::" + methodName + "] "	+ doDump(o));
			}
		} catch (Throwable t) {
			this.error("Dump fallito: " + t.getMessage(), t);
		}
...
//		logger.error("[" + className + "::" + methodName + "] " + message);
		logger.error("[" + traceElement.getClassName() + "::" + methodName + "] " + message);
	}
...
//			String className = traceElement.getClassName();
			...
			if(level!=null && level.length>0 ){
				if(level[0].equalsIgnoreCase("error"))
		 			logger.error("[" + traceElement.getClassName() + "::" + methodName + "] " + sb.toString());
				else if(level[0].equalsIgnoreCase("warn"))
					logger.warn("[" + traceElement.getClassName() + "::" + methodName + "] " + sb.toString());
				else if(level[0].equalsIgnoreCase("info"))
					logger.info("[" + traceElement.getClassName() + "::" + methodName + "] " + sb.toString());
				else  
					logger.debug("[" + traceElement.getClassName() + "::" + methodName + "] " + sb.toString());
			}else{
				logger.debug("[" + traceElement.getClassName() + "::" + methodName + "] " + sb.toString()+"\n\n");
			}
		} catch (Exception e) {
