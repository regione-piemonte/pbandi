	...
	public java.util.List<PagamentoQuotePartiVO> findPagamentiAssociati(Long idDocumentoDiSpesa, Long idProgetto, String dataPagamento, boolean visualizzaImportoRendicPagato){
		...
		java.util.List<PagamentoQuotePartiVO> result = new ArrayList<PagamentoQuotePartiVO>();
			...
			sqlSelect.append("" +
					...
					", pagamento.flag_pagamento         							   AS flagPagamento" +
...
// PBANDI-2314					"PBANDI_D_STATO_VALIDAZ_SPESA, " +
			...
			StringBuilder sqlSelect = (new StringBuilder("SELECT DISTINCT "+
					...
					" ,PBANDI_T_PAGAMENTO.IMPORTO_PAGAMENTO as importoPagamento" +
					" ,PBANDI_T_PAGAMENTO.DT_PAGAMENTO as dtPagamento" +
					" ,PBANDI_T_PAGAMENTO.FLAG_PAGAMENTO as flagPagamento" +
					" ,PBANDI_D_MODALITA_PAGAMENTO.ID_MODALITA_PAGAMENTO as idModalitaPagamento" + 
					" ,PBANDI_D_MODALITA_PAGAMENTO.DESC_BREVE_MODALITA_PAGAMENTO as descBreveModalitaPagamento" ));
// PBANDI-2314					" ,PBANDI_D_STATO_VALIDAZ_SPESA.DESC_BREVE_STATO_VALIDAZ_SPESA as descBreveStatoValidazioneSpesa"));
...
//			int count = getNamedJdbcTemplate().queryForInt(sqlSelect.toString(), params);
	...
	public Long inserisciPagamento(Long idModalitaPagamento,java.sql.Date dataPagamento,Double importoTotalePagamento,
			...
			Long idCausalePagamento,String rifPagamento, String flagPagamento
			)
	{
		...
		sqlInsert.append(tipoDataPagamento);

		if(importoTotalePagamento!=null)
			sqlInsert.append(",IMPORTO_PAGAMENTO");
		...
		if(flagPagamento!=null)
			sqlInsert.append(",FLAG_PAGAMENTO");
		...
		if(idModalitaPagamento!=null)
			sqlInsert.append(" :idModalitaPagamento,");
		sqlInsert.append(":dataPagamento");
		if(importoTotalePagamento!=null)
			sqlInsert.append(",:importoTotalePagamento");
		sqlInsert.append(",:idPagamento");
		...
		if(flagPagamento!=null)
			sqlInsert.append(",:flagPagamento");
		sqlInsert.append(")");
		...
		// Alex: sostituito con select sulla sequence poichè getSeqpbandipagamenti() restituisce un oggetto nullo. 
		//long idNuovoPagamento = getSeqpbandipagamenti().nextLongValue();		
		String sqlSeq = "select SEQ_PBANDI_T_PAGAMENTO.nextval from dual ";
		getLogger().info("\n"+sqlSeq.toString());
		Long idNuovoPagamento = queryForLong(sqlSeq.toString(), new MapSqlParameterSource());
		getLogger().info("idNuovoPagamento = "+idNuovoPagamento);
		...
		if(flagPagamento!=null)
			params.addValue("flagPagamento", flagPagamento, Types.VARCHAR);
...
//JIRA23-14		params.addValue("idProgetto", idProgetto, Types.BIGINT);
			...
			Long idCausalePagamento,String rifPagamento, String flagPagamento)
	{
			...
			sqlUpdate
					.append("ID_MODALITA_PAGAMENTO").append("=:idModalitaPagamento")
			...
			sqlUpdate.append(",FLAG_PAGAMENTO=:flagPagamento");
