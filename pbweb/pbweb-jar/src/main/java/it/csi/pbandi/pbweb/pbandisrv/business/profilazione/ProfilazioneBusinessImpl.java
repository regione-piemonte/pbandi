...
package it.csi.pbandi.pbweb.pbandisrv.business.profilazione;
...
import it.csi.pbandi.pbweb.pbandisrv.dto.profilazione.ConsensoInvioComunicazioneDTO;
import it.csi.pbandi.pbweb.pbandisrv.dto.profilazione.EsitoControlloAccessoDTO;
import it.csi.pbandi.pbweb.pbandisrv.dto.profilazione.FiltroBeneficiarioProgettoSoggettoRuoloDTO;
import it.csi.pbandi.pbweb.pbandisrv.dto.profilazione.ProgettoDTO;
import it.csi.pbandi.pbweb.pbandisrv.dto.profilazione.Ruolo;
import it.csi.pbandi.pbweb.pbandisrv.dto.profilazione.UserInfoDTO;
import it.csi.pbandi.pbweb.pbandisrv.dto.profilazione.UserParamsDTO;
import it.csi.pbandi.pbweb.pbandisrv.exception.profilazione.ProfilazioneException;
import it.csi.pbandi.pbweb.pbandisrv.exception.profilazione.UtenteException;
...
import it.csi.pbandi.pbweb.pbandisrv.integration.db.jdbctemplate.vo.BeneficiarioProgettoBandolineaVO;
...
import it.csi.pbandi.pbweb.pbandisrv.integration.db.jdbctemplate.vo.gestioneincarichi.RappresentanteLegaleGestioneIncarichiVO;
...
import it.csi.pbandi.pbweb.pbandisrv.integration.db.vo.PbandiDRuoloHelpVO;
...
import it.csi.pbandi.pbweb.pbandisrv.integration.db.vo.PbandiTAccessoIrideVO;
import it.csi.pbandi.pbweb.pbandisrv.integration.db.vo.PbandiTAccessoUppVO;
import it.csi.pbandi.pbweb.pbandisrv.integration.db.vo.PbandiTSoggettoVO;
import it.csi.pbandi.pbweb.pbandisrv.integration.db.vo.PbandiTStoricoAccessoVO;
import it.csi.pbandi.pbweb.pbandisrv.integration.db.vo.PbandiTUtenteVO;
import it.csi.pbandi.pbweb.pbandisrv.interfacecsi.profilazione.ProfilazioneSrv;
...
public class ProfilazioneBusinessImpl extends BusinessImpl implements
		ProfilazioneSrv {
	...
	static {
		...
		TIPO_ANAGRAFICA_VO_TO_DTO.put("descBreveTipoAnagrafica",
				"descrizioneBreve");
		...
		BENEFICIARIO_VO_TO_DTO
				.put("codiceFiscaleBeneficiario", "codiceFiscale");
	...
	public java.lang.Boolean hasPermesso(Long idUtente,
			String identitaDigitale, String descBreveRuolo,
			String descBrevePermesso) throws CSIException, SystemException,
			UnrecoverableException, UtenteException {
		ContestoIdentificativoDTO contestoIdentificativo = soggettoManager
				.caricaContestoIdentificativo(idUtente, identitaDigitale);
		boolean autorizzato = soggettoManager.hasPermesso(
				contestoIdentificativo, descBreveRuolo, descBrevePermesso);
		return autorizzato;
	}

	public Boolean isRegolaApplicabileForBandoLinea(java.lang.Long idUtente,
			java.lang.String identitaDigitale, Long idBandoLinea,
			String codiceRegola) throws CSIException, SystemException,
			UnrecoverableException {
		boolean result = false;

			result = regolaManager.isRegolaApplicabileForBandoLinea(
					idBandoLinea, codiceRegola);

		return result;
	}

	public Boolean isRegolaApplicabileForProgetto(java.lang.Long idUtente,
			java.lang.String identitaDigitale, Long idProgetto,
			String codiceRegola) throws CSIException, SystemException,
			UnrecoverableException {
		boolean result = false;

			result = regolaManager.isRegolaApplicabileForProgetto(
					idProgetto, codiceRegola);

		return result;
	}

	public UserInfoDTO getInfoUtente(Long idUtente, String identitaDigitale)
			throws CSIException {

			UserInfoDTO userInfo = new UserInfoDTO();

			ContestoIdentificativoDTO identita = soggettoManager
					.caricaContestoIdentificativo(idUtente, identitaDigitale);
	...
	public UserInfoDTO getMiniAppInfoUtente(Long idUtenteCorrente,
			String identitaDigitaleCorrente, UserParamsDTO userParams)
			throws CSIException, SystemException, UnrecoverableException,
			ProfilazioneException {
		...
		try {
			...
			ContestoIdentificativoDTO identita = soggettoManager
					.caricaContestoIdentificativo(idUtente,
							identitaDigitaleCorrente);
		/*	logger.info("\n\n\nidentita.getDescBreveTipoAnagrafica(): "+identita.getDescBreveTipoAnagrafica());
			...
			*/
			...
			/*
			 * 2) Controllo che l' idSoggetto restituito sia uguale all'
			 * idSoggetto di userParams. Questo controllo vale anche in caso di
			 * accesso di utente incaricato poiche' sia l' idSoggetto che l'
			 * identita iride devono essere relativi al soggetto delegante.
			 */
			if (NumberUtil.compare(soggetto.getIdSoggetto(), new BigDecimal(
					userParams.getIdSoggetto())) != 0) {
				logger.warn("Soggetto[" + userParams.getIdSoggetto() + " - "
						+ identita.getCodFiscale()
						+ "] non presente in PBandi_T_Soggetto");
				UtenteException ue = new UtenteException(
						ErrorConstants.ERRORE_UTENTE_NON_AUTORIZZATO);
				throw ue;
			}

			/*
			 * 3) Nel caso di accesso di utente incaricato verifico che il
			 * delegante (idSoggetto di userParams) sia Rappresentante Legale
			 * per l' azienda (idSoggettoBeneficiario) Altrimenti verifico che
			 * esiste l' utente in PBandi_t_utente con idUtente e codice fiscale
			 * dell' identita iride
			 */
			if (userParams.getFlagIncaricato() != null
					&& userParams.getFlagIncaricato()) {
				RappresentanteLegaleGestioneIncarichiVO rappLegaleVO = new RappresentanteLegaleGestioneIncarichiVO();
				rappLegaleVO.setIdSoggettoDelegante(new BigDecimal(userParams
						.getIdSoggetto()));
				rappLegaleVO.setIdSoggettoBeneficiario(new BigDecimal(
						userParams.getIdSoggettoBeneficiario()));
				List<RappresentanteLegaleGestioneIncarichiVO> listRapp = genericDAO
						.findListWhere(rappLegaleVO);
				if (listRapp == null || listRapp.size() == 0) {
					logger.warn("Delegate["
							+ identita.getCodFiscale()
				...
				if (utenteVO == null) {
					logger.warn("Utente[" + idUtente + " - "
							+ identita.getCodFiscale()
							+ "] non censito in PBANDI");
					UtenteException ue = new UtenteException(
							ErrorConstants.ERRORE_UTENTE_NON_AUTORIZZATO);
					throw ue;
				}
			...
			/*
			 * Nel caso di incaricato: - nome, cognome e codice fiscale sono
			 * quelli dell' incaricato - idSoggetto e' quello del delegante
			 */
			...
			if (userParams.getFlagIncaricato() != null
				...
				utenteIncaricatoVO = genericDAO
						.findSingleWhere(utenteIncaricatoVO);
				PersonaFisicaVO personaFisicaVO = new PersonaFisicaVO();
				personaFisicaVO.setIdSoggetto(utenteIncaricatoVO
						.getIdSoggetto());
	...
	private PbandiTSoggettoVO eseguiControlliComuni(UserInfoDTO userInfo,
			ContestoIdentificativoDTO identita) throws UtenteException {
		...
		try {
			if (identita.getCodFiscale() == null) {
				UtenteException e = new UtenteException(
						"Codice fiscale null letto dal contesto identificativo.");
				logger.error(
						"Codice fiscale non valorizzato nel contesto identificativo.",
						e);
				throw e;
			}
			...
			try {
				...
				if (isEmpty(tipiAnagrafica)) {
					logger.warn("Utente[" + identita.getCodFiscale()
							+ "] non possiede ruoli, quindi non pu� accedere");
					autorizzato = false;
				}

			} catch (RecordNotFoundException e) {
				logger.warn("Utente[" + identita.getCodFiscale()
						+ "] non censito in PBandi_T_utente");
				autorizzato = false;
			}
		} catch (RecordNotFoundException e) {
			logger.warn("Utente[" + identita.getCodFiscale()
		...
		if (!autorizzato) {
			...
			userInfo.setRuoli(beanUtil.transform(tipiAnagrafica, Ruolo.class,
					TIPO_ANAGRAFICA_VO_TO_DTO));

			userInfo.setIdUtente(beanUtil.transform(utente.getIdUtente(),
					Long.class));
		}
	...
	public Beneficiario[] findBeneficiari(Long idUtente,
			String identitaDigitale, String codiceFiscale, String ruoloIride)
			throws CSIException, SystemException, UnrecoverableException,
			ProfilazioneException {
			PbandiTSoggettoVO soggettoVO = soggettoManager
					.findSoggetto(codiceFiscale);
			...
			List<BeneficiarioUtenteVO> beneficiariVO = getSoggettoManager()
					.findBeneficiariBySoggettoRuolo(filtro);

			return beanUtil.transform(beneficiariVO, Beneficiario.class,
					BENEFICIARIO_VO_TO_DTO);
		 
	}

	public Beneficiario[] findBeneficiariProgetto(Long idUtente,
			String identitaDigitale, Long idProgetto) throws CSIException,
			SystemException, UnrecoverableException, ProfilazioneException {
	...
	public Beneficiario[] findBeneficiario(Long idUtente,
			String identitaDigitale, Long idSoggettoBeneficiario)
			throws CSIException, SystemException, UnrecoverableException,
			ProfilazioneException {
		...
		List<BeneficiarioVO> listBeneficiari = pbandiProfilazioneDAO
				.findBeneficiario(idSoggettoBeneficiario);
		return beanUtil.transform(listBeneficiari, Beneficiario.class);

	}

	public Beneficiario[] findBeneficiariByProgettoSoggettoRuolo(Long idUtente,
		...
		List<BeneficiarioUtenteVO> beneficiariVO = getSoggettoManager()
				.findBeneficiariBySoggettoRuolo(filter);

		return beanUtil.transform(beneficiariVO, Beneficiario.class,
				BENEFICIARIO_VO_TO_DTO);
	}

	public ProgettoDTO[] findProgettiByBeneficiarioSoggettoRuolo(Long idUtente,
			String identitaDigitale,
			FiltroBeneficiarioProgettoSoggettoRuoloDTO filtro)
			throws CSIException, SystemException, UnrecoverableException,
			UtenteException {
		...
		List<ProgettoUtenteVO> progetti = getSoggettoManager()
				.findProgettiBySoggettoRuolo(filter);

		return beanUtil.transform(progetti, ProgettoDTO.class);
	}

	public EsitoControlloAccessoDTO controllaStoricizzaAccesso(Long idUtente,
			String identitaDigitale, Long codTipoAccesso, Boolean isSpid) throws CSIException,
			SystemException, UnrecoverableException, UtenteException {
	 
			String[] nameParameter = { "idUtente", "identitaDigitale",
					"codTipoAccesso", "isSpid" };
			ValidatorInput.verifyNullValue(nameParameter, idUtente,
					identitaDigitale, codTipoAccesso, isSpid);
			...
			tipoAccessoVO.setCodTipoAccesso(beanUtil.transform(codTipoAccesso,
					String.class));
			try {
				tipoAccessoVO = genericDAO.findSingleWhere(tipoAccessoVO);
			} catch (RecordNotFoundException rnfe) {
				logger.warn("Errore durante la ricerca del tipo di accesso ("
						+ codTipoAccesso + ")" + rnfe.getMessage());
				throw rnfe;
			}
			...
			storicoVO.setIdUtente(beanUtil
			...
			try {
				...
				logger.error(
						"Errore durante insert in PBANDI_T_STORICO_ACCESSO [idUtente="
								+ idUtente, e);
				throw new UtenteException(
						"Errore durante la storicizzazione dell' accesso.", e);
			}

			/* ATTENZIONE: VALE SOLO PER SHIBBOLETH, NON PER SPID.  (Alex 17/7/2018)
			 * Se l'utente accede con il livello di autenticazione User/Psw/Pin
			 * devo: 1) al primo accesso: - inserisco il record nella
			 * PBANDI_T_ACCESSO_UPP - calcolo la possibile data di blocco dell'
			 * accesso, aggiugendo 30 gg alla data odierna. Se questa data e'
			 * antecedente alla data di blocco di tutti gli accessi con
			 * usr/psw/pin allora la setto come data di blocco accesso per l'
			 * utente, mentre se la data e' successiva alla data di blocco di
			 * tutti gli accessi con usr/psw/pin allora setto quest'ultima come
			 * data di blocco accesso per l'utente.
			 * 
			 * 2) dal secondo accesso: - controllo la data odierna con la data
			 * di blocco accesso per l' utente, se e' valida restituisco il msg
			 * con la data di blocco, altrimenti restituisco il messggio che le
			 * credenziali non sono piu' valide.
			 */
			...
			if (codTipoAccesso == Identita.AUTENTICAZIONE_USERNAME_PASSWORD_PIN && !isSpid) {
				 ...
				 */
				Date dtBloccoAccessiUPP = configurationManager
						.getConfiguration().getDtBloccoAccessoUPP();

				/*
				 * Numero di gg per il calcolo della data di blocco
				 */
				Long gg = configurationManager.getConfiguration()
						.getGgCalcoloDtBloccoAccessoUPP();

				PbandiTAccessoUppVO accessoVO = new PbandiTAccessoUppVO();
				accessoVO.setIdUtente(beanUtil.transform(idUtente,
						BigDecimal.class));
				List<PbandiTAccessoUppVO> accessiUPP = genericDAO
						.findListWhere(accessoVO);

				if (ObjectUtil.isEmpty(accessiUPP)) {
					accessoVO.setDtPrimoAccesso(DateUtil.getSysdate());
					/*
					 * Calcolo la data di blocco dell' accesso dell' utente con
					 * user/psw/pin come la data minima tra la (dataOdierna+30)
					 * e la data di blocco di tutti gli accessi con
					 * user/psw/pin.
					 */
					Date dtBloccoUtente = DateUtil.addGiorni(currentDate,
							beanUtil.transform(gg, Integer.class));
					try {
						if (DateUtil.after(dtBloccoUtente, dtBloccoAccessiUPP))
							accessoVO.setDtBloccoAccesso(DateUtil
									.utilToSqlDate(dtBloccoAccessiUPP));
						else
							accessoVO.setDtBloccoAccesso(DateUtil
									.utilToSqlDate(dtBloccoUtente));
					} catch (Exception e1) {
					...
					try {
						...
						logger.error(
								"Errore durante insert in PBANDI_T_ACCESSO_UPP [idUtente="
										+ idUtente, e);
						throw new UtenteException(
								"Errore durante l' inserimento dell' accesso con usr/pin/psw.",
								e);
					}
				} else {
					/*
				...
				/*
				 * Se la data odierna e' successiva alla data minima tra la data
				 * di blocco dell' utente e la data di blocco di tutti gli
				 * accessi con user/psw/pin allora blocco l'utente.
				 */
				try {
					Date dataMinimaBlocco = null;
					if (DateUtil.before(accessoVO.getDtBloccoAccesso(),
							dtBloccoAccessiUPP))
					...
					if (DateUtil.before(dataMinimaBlocco, currentDate)) {
						...
						msgAvviso = configurationManager.getConfiguration()
								.getMsgAccessoBloccato();
						msgAvviso = msgAvviso.replace("${dtBlocco}",
								DateUtil.getData(dataMinimaBlocco.getTime()));
					} else {
						msgAvviso = configurationManager.getConfiguration()
								.getMsgDataBloccoAccesso();
						msgAvviso = msgAvviso.replace("${dtBlocco}",
								DateUtil.getData(dataMinimaBlocco.getTime()));
					}
	...
	public void memorizzaIride(Long idUtente, String identitaDigitale,
			String idShib, String idIride, String sessionId, String msg)
			throws CSIException, SystemException, UnrecoverableException,
			UtenteException {
			PbandiTAccessoIrideVO vo = new PbandiTAccessoIrideVO();
			vo.setAccessDate(DateUtil.getSysdate());
			vo.setIrideId(idIride == null ? null : idIride.substring(0,
					Math.min(244, idIride.length())));
			vo.setMsg(msg == null ? null : msg.substring(0,
					Math.min(244, msg.length())));
			vo.setShibId(idShib == null ? null : idShib.substring(0,
					Math.min(244, idShib.length())));
			vo.setSessionId(sessionId == null ? null : sessionId.substring(0,
					Math.min(244, sessionId.length())));
			try {
				genericDAO.insert(vo);
			} catch (Exception e) {
				/*
				 * Attenzione. In caso di errore il sistema deve continuare a
				 * funzionare.
				 */
				logger.error(
						"Errore durante inserimento su PBandi_t_Accesso_Iride",
						e);
				logger.dump(vo);
			}
	}
	 ...
	 * (non-Javadoc)
	 * 
	 * @see it.csi.pbandi.pbweb.pbandisrv.interfacecsi.profilazione.ProfilazioneSrv#
	 * tracciaControlloCookie(java.lang.Long, java.lang.String,
	 * java.lang.String, java.lang.String)
	 */
	public void tracciaControlloCookie(Long idUtente, String identitaDigitale,
			String msg) throws CSIException, SystemException,
			UnrecoverableException, UtenteException {
	...
	public Boolean existBandolineaWithRegolaForBeneficiario(Long idUtente, String identitaDigitale,Long idSoggettoBeneficiario, String codiceRegola)throws CSIException, SystemException,
	UnrecoverableException, UtenteException {
		boolean result = false;
		...
		if (!ObjectUtil.isEmpty(bandolinea)) {
			...
			List<RegolaAssociataBandoLineaVO> bandolineaFilter = beanUtil.transformList(bandolinea, RegolaAssociataBandoLineaVO.class, map);
			RegolaAssociataBandoLineaVO regolaFilter = new RegolaAssociataBandoLineaVO();
			regolaFilter.setDescBreveRegola(codiceRegola);
			
			AndCondition<RegolaAssociataBandoLineaVO> andCondition = new AndCondition<RegolaAssociataBandoLineaVO>(Condition.filterBy(bandolineaFilter),Condition.filterBy(regolaFilter));
	...
	public ConsensoInvioComunicazioneDTO findConsensoInvioComunicazione(Long idUtente, String identitaDigitale) throws CSIException, SystemException,
	UnrecoverableException, UtenteException {
	...
	public Boolean salvaConsensoInvioComunicazione(Long idUtente, String identitaDigitale, ConsensoInvioComunicazioneDTO consensoInvioComunicazioneDTO) throws CSIException, SystemException,
	UnrecoverableException, UtenteException {
		try {
