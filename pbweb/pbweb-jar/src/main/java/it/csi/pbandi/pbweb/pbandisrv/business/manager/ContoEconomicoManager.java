...
import it.csi.pbandi.pbweb.pbandisrv.exception.manager.ContoEconomicoAggiornamentoStatoFallitoException;
...
import it.csi.pbandi.pbweb.pbandisrv.integration.db.jdbctemplate.vo.ModalitaDiAgevolazioneContoEconomicoPerBilancioVO;
...
import it.csi.pbandi.pbweb.pbandisrv.integration.db.jdbctemplate.vo.VoceDiEntrataCulturaVO;
...
import it.csi.pbandi.pbweb.pbandisrv.integration.db.vo.PbandiDStatoContoEconomicoVO;
...
public class ContoEconomicoManager {
	...
	private ContoEconomicoConStatoVO findContoEconomicoConStatoValidDate(
		...
		contoEconomicoConStatoVO.setDescBreveTipologiaContoEco(tipologiaContoEconomico);
	...
	private List<SoggettiFinanziatoriPerProgettoVO> getSoggettiFinanziatoriPrivati() {
		...
		f1VO
				.setDescBreveTipoSoggFinanziat(it.csi.pbandi.pbweb.pbandiutil.common.Constants.CODICE_TIPO_SOGG_FINANZIATORE_COFPOR);
		...
		f2VO
				.setDescBreveTipoSoggFinanziat(it.csi.pbandi.pbweb.pbandiutil.common.Constants.CODICE_TIPO_SOGG_FINANZIATORE_OTHFIN);
...
//		}
	...
	public ContoEconomicoDTO loadContoEconomico(BigDecimal idContoEconomico)
		...
		if (ribassi != null && !ribassi.isEmpty() && ribassi.size() > 1) {
			...
			contoRisultante
					.setFlagRibassoAsta(it.csi.pbandi.pbweb.pbandiutil.common.Constants.FLAG_FALSE);
		} else {
			ribassoAstaVO = ribassi.get(0);
			contoRisultante
					.setFlagRibassoAsta(it.csi.pbandi.pbweb.pbandiutil.common.Constants.FLAG_TRUE);
	...
	public BigDecimal getQuotaImportoAgevolato(BigDecimal idProgetto,
		...
		modalitaDiAgevolazioneContoEconomicoVO
				.setFlagLvlprj(it.csi.pbandi.pbweb.pbandiutil.common.Constants.FLAG_TRUE);
	...
	public Long chiudiInvioPropostaRimodulazione(BigDecimal idProgetto,
		...
		contoEconomicoVO = new PbandiTContoEconomicoVO();
		contoEconomicoVO.setNoteContoEconomico(noteContoEconomico);
		BigDecimal idContoEconomico = findContoEconomicoConStato(idProgetto,
	...
	public List<ContoEconomicoItemDTO> mappaContoEconomicoPerReport(
		...
		importo = calcolaImportoPerVisualizzazione(contoEconomicoItemList,
				isRimodulazione);
		mapImporto.put(IMPORTO, importo);
	...
	private void calcolaImportiMacroVociPerVisualizzazione(
		...
		for (ContoEconomicoItemDTO contoEconomicoItem : contoEconomicoItemList) {
			if (contoEconomicoItem.getIdVoce() != null) {
				if (mapMacroVoci.containsKey(contoEconomicoItem.getIdVoce()
						.toString())) {
					Double importo = mapMacroVoci.get(contoEconomicoItem
							.getIdVoce().toString());
					if (isRimodulazione) {
						contoEconomicoItem
	...
	public ContoEconomicoDTO createMain(BigDecimal idProgetto, Long idUtente)
		...
		it.csi.pbandi.pbweb.pbandisrv.dto.manager.ContoEconomicoDTO contoMain = loadContoEconomico(contoDaCreareVO
				.getIdContoEconomico());
	...
	public VoceDiSpesaConAmmessoVO[] getVociDiSpesaConAmmesso(Long idProgetto) {
		logger.debug("getVociDiSpesaConAmmesso idProgetto="+idProgetto);
