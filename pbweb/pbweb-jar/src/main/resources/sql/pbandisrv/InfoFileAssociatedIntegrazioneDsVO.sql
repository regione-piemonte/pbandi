...
DICHIARAZIONE.DT_DICHIARAZIONE , 
PBANDI_D_TIPO_DICHIARAZ_SPESA.DESC_TIPO_DICHIARAZIONE_SPESA DESC_TIPO_DICHIARAZIONE_SPESA,
...
JOIN PBANDI_T_FILE_ENTITA FILE_ENTITA ON FILE_ENTITA.ID_FILE=PBANDI_T_FILE.ID_FILE AND 
 FILE_ENTITA.ID_ENTITA=(SELECT ID_ENTITA FROM PBANDI_C_ENTITA WHERE UPPER(NOME_ENTITA)=UPPER('PBANDI_T_INTEGRAZIONE_SPESA'))
JOIN PBANDI_T_INTEGRAZIONE_SPESA  INTEGRAZIONE
ON (INTEGRAZIONE.ID_INTEGRAZIONE_SPESA = FILE_ENTITA.ID_TARGET)
JOIN PBANDI_T_DICHIARAZIONE_SPESA  DICHIARAZIONE
ON (DICHIARAZIONE.ID_DICHIARAZIONE_SPESA = INTEGRAZIONE.ID_DICHIARAZIONE_SPESA)
JOIN PBANDI_D_TIPO_DICHIARAZ_SPESA USING(ID_TIPO_DICHIARAZ_SPESA)
