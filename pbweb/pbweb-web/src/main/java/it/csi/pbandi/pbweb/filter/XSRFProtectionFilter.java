...
public class XSRFProtectionFilter implements Filter {
	 ...
	 */
	private static final String XSRF_HEADER_NAME = "X-XSRF-TOKEN-WEB";

	/*
	 * nome del cookie XSRF
	 */
	private static final String XSRF_COOKIE_NAME = "XSRF-TOKEN-COOKIE-WEB";

	/**
	 * nome dell'attributo di sessione che mantiene il token XSRF 
	 */
	private static final String XSRF_INTERNAL_TOKEN_NAME = "XSRF_SESSION_TOKEN-WEB";
	...
	public void doFilter(ServletRequest req, ServletResponse resp, FilterChain chain)
		...
		LOG.info("[XSRFProtectionFilter::dofilter] BEGIN");
		LOG.info("[XSRFProtectionFilter::dofilter] isDisabled="+isDisabled);
		LOG.info("[XSRFProtectionFilter::dofilter] RequestURI="+hreq.getRequestURI());
			...
			if (!isRestRequest(hreq)) {
				...
				if (!isXSRFSessionAlive(session)){
					...
					try {
						...
						LOG.info("[XSRFProtectionFilter::dofilter()  newToken=["+newToken+"]");
				...
				if (isXSRFSessionAlive(session)) {
					...
					LOG.info("[XSRFProtectionFilter::dofilter] TOKEN session: " + actualActiveToken);
					LOG.info("[XSRFProtectionFilter::dofilter] TOKEN header: " + actualRequestHeader);
					LOG.info("[XSRFProtectionFilter::dofilter] TOKEN cookie: " + actualRequestCookie);
	...
	private boolean validXSRFCookieAndHeader(HttpServletRequest hreq, HttpSession session) {
		...
		String actualRequestHeader = getActualXSRFHeader(hreq);
		String actualRequestCookie = getActualXSRFCookie(hreq);
		LOG.info("[XSRFProtectionFilter::validXSRFCookieAndHeader] TOKEN session: " + actualActiveToken);
		LOG.info("[XSRFProtectionFilter::validXSRFCookieAndHeader] TOKEN header: " + actualRequestHeader);
		LOG.info("[XSRFProtectionFilter::validXSRFCookieAndHeader] TOKEN cookie: " + actualRequestCookie);
	...
	public void init(FilterConfig cfg) throws ServletException {
		...
		LOG.info("[XSRFProtectionFilter::init] sDisabled="+sDisabled);
