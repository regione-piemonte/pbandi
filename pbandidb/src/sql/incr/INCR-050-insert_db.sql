...
update pbandi_t_impegno t
set t.codice_fiscale = (select cod_fiscale from pbandi_w_impegni w
where t.anno_impegno= w.annoimp
and t.numero_impegno= w.nroimp
and t.anno_esercizio = w.annoesercizio); 

update pbandi_t_impegno t
set t.ragsoc = (select w.ragsoc from pbandi_w_impegni w
where t.anno_impegno= w.annoimp
and t.numero_impegno= w.nroimp
and t.anno_esercizio = w.annoesercizio);
