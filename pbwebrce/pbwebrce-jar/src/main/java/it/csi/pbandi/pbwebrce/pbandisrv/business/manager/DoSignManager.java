...
package it.csi.pbandi.pbwebrce.pbandisrv.business.manager;
...
import it.csi.pbandi.pbwebrce.pbandisrv.business.digitalsign.DigitalSignException;
...
import it.csi.pbandi.pbwebrce.pbandisrv.integration.db.vo.PbandiTLogValidazFirmaVO;
import it.csi.pbandi.pbwebrce.pbandiutil.common.LoggerUtil;
import it.csi.pbandi.pbwebrce.util.DateUtil;
import it.csi.wso2.apiman.oauth2.helper.OauthHelper;
import it.doqui.dosign.dosign.business.session.dosign.signaturevalidation.DosignSignatureValidation_PortType;
import it.doqui.dosign.dosign.business.session.dosign.signaturevalidation.DosignSignatureValidation_ServiceLocator;
import it.doqui.dosign.dosign.business.session.dosign.signaturevalidation.SignerDto;
import it.doqui.dosign.dosign.business.session.dosign.signaturevalidation.VerifyDto;
import it.doqui.dosign.dosign.business.session.dosign.signaturevalidation.VerifyInfoDto;
import it.doqui.dosign.dosign.business.session.dosign.signaturevalidation.VerifyReportDto;
import it.doqui.dosign.dosign.business.session.dosign.timestamping.DosignTimestamping_PortType;
import it.doqui.dosign.dosign.business.session.dosign.timestamping.DosignTimestamping_ServiceLocator;
	    ...
	    try {
	    	...
	    	//String bypassCheckCFDigitalSign = getConfigurationManager().getConfiguration().getBypassCheckCFDigitalSign();
			  ...
			  if(bypassCheckDigitalSign!=null && bypassCheckDigitalSign.equalsIgnoreCase("S")){
				  logger.warn(prf + "************ATTENZIONE! BYPASS DEI CONTROLLI SULLA FIRMA DIGITALE , controllare il flag sul db************\n\n\n\n");
				  return Boolean.TRUE;
			  }else{
	    	
				 logger.info(prf + "begin verify for idDocIndex "+idDocIndex+" , bytesSignedFile: "+bytesSignedFile+" ,bytesSignedFile.length: "+bytesSignedFile.length);
				
//				 DosignServiceDto dosignService = initializeDosignService();
				 logger.info(prf + "calling DosignServiceClient.getSignatureValidationService...");
...
//			     logger.info("\n*** DosignServiceClient.getSignatureValidationService(serviceDosignService) executed in ms:  "+ (System.currentTimeMillis()-start));
				 ...
				 DosignSignatureValidation_PortType dosignWS = initializeDosignServiceWS();
				 logger.info(prf + "*** initializeDosignServiceWS executed in ms:  "+ (System.currentTimeMillis()-start));
...
//		         logger.info("\n***DosignSignatureValidation.verify executed in ms : "+(System.currentTimeMillis()-start));
				 ...
				 logger.info(prf + "***dosignWS.verify executed in ms : "+(System.currentTimeMillis()-start));
				 
//		         List<VerifyInfoDto> listOfVerifyInfo = verifyReport.getVerifyInfo();
				 VerifyInfoDto[] listOfVerifyInfo = verifyReport.getVerifyInfo();
				 logger.info(prf + "listOfVerifyInfo: "+listOfVerifyInfo);
				 
		         StringBuilder sb=new StringBuilder();
				 if(listOfVerifyInfo != null) {
					 int numberVerifyInfo = listOfVerifyInfo.length; //listOfVerifyInfo.size();
					 sb.append("numberVerifyInfo: "+numberVerifyInfo);
					  
					 for(int indiceVerifyInfo = 0; indiceVerifyInfo < numberVerifyInfo; indiceVerifyInfo++) {
					      VerifyInfoDto verifyInfo = listOfVerifyInfo[indiceVerifyInfo]; //listOfVerifyInfo.get(indiceVerifyInfo);
...
//						  List<SignerDto> listOfSigners = verifyInfo.getSigner();
					      SignerDto[] listOfSigners = verifyInfo.getSigner();
					      
						  List<String> fiscalCodes=new ArrayList<String>();
						  if(listOfSigners != null) {
						     int numberSigners = listOfSigners.length; //listOfSigners.size();
						     sb.append("\nnumberSigners: "+numberSigners);
		 					 for(int indiceSigner = 0; indiceSigner < numberSigners; indiceSigner++) {
								SignerDto signer = listOfSigners[indiceSigner]; //listOfSigners.get(indiceSigner);
								sb.append("\nsigner: "+signer);
						  ...
						  for (String fiscalCode : fiscalCodes) {
							  ...
							  for (String cfLegaleRapp : cfLegaleRapps) { 
								 ...
								 if(fiscalCode.equals(cfLegaleRapp)   ){
									 logger.info(prf + "--- CF MATCH! --"+fiscalCode);
									 listCfMatched.add(fiscalCode);
									 foundCf=true;
								 }
							  }
						  ...
						  if(bypassCheckCFDigitalSign==null || bypassCheckCFDigitalSign.equalsIgnoreCase("N")){
							  if(!foundCf){
								  ...
								  logger.info(prf + "Verify KO: "+msgCf+"\n\n\n\n\n\n");
		    	...
		    	logger.info(prf + "System.currentTimeMillis() = "+System.currentTimeMillis()+"; start = "+start);
				  logger.error(prf + "KO doSign verify, executed in ms: "+(System.currentTimeMillis()-start),ex);
...
//        DosignServiceDto dosignService = initializeTimeStampingService();
        ...
        try {
        	 DosignTimestamping_PortType timestampingService = initializeTimeStampingServiceWS();
...
//	}
	...
	public boolean testResources() {
		try {
			logger.info("Eseguo initializeDosignServiceWS()");
			DosignSignatureValidation_PortType dosignWS = initializeDosignServiceWS();
		...
		try {
			logger.info("Eseguo initializeTimeStampingServiceWS()");
			DosignTimestamping_PortType dosigWS = initializeTimeStampingServiceWS();
			logger.info("Eseguo dosigWS.testResources()");
			boolean esito = dosigWS.testResources();
			logger.info("Esito = "+esito);
			if (!esito)
				return false;
		} catch (Exception e) {
