...
import it.csi.pbandi.pbwebcert.util.tablewriter.TableAccessor.RowAccessor;
...
public class TableWriter {
	...
	public static byte[] writeTableToByteArrayCC(String key,
			ExcelDataWriter dw, List<ProgettoCertificazioneIntermediaFinaleDTO> i, ArrayList<Long> linesToJump)  throws Exception{
		ByteArrayOutputStream byteArrayOutputStream = new ByteArrayOutputStream();
		new TableWriter(dw, new LabelTableAccessorDecorator(key, new BeanTableAccessor(i)), linesToJump).writeTo(byteArrayOutputStream);
		return byteArrayOutputStream.toByteArray();
	}
